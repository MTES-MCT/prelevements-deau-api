...
import {getDocument, getDocumentByObjectKey} from './models/documents.js'
...
export async function handleDocument(req, res, next) {
  let documentId = mongo.parseObjectId(req.params.documentId)
  ...
  if (documentId && !req.document) {
    req.document = await getDocumentByObjectKey(documentId)
  }
