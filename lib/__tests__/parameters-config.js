...
test('getAvailableOperators - volume prélevé spatial', t => {
  ...
  t.deepEqual(operators, ['sum', 'mean', 'min', 'max'])
})
...
test('validateOperatorForParameter - message liste les opérateurs disponibles', t => {
  ...
  t.regex(error.message, /Opérateurs disponibles: sum, mean, min, max/)
})
...
test('paramètres instantanés non-débits ne supportent jamais sum', t => {
  ...
  for (const param of nonDebits) {
    t.false(isOperatorValidForParameter(param, 'sum', 'spatial'), `${param} ne devrait pas supporter sum en spatial`)
    t.false(isOperatorValidForParameter(param, 'sum', 'temporal'), `${param} ne devrait pas supporter sum en temporal`)
  }
})

test('tous les paramètres supportent mean, min, max en spatial (sauf débits qui ont sum only)', t => {
  const allParams = [
    'volume prélevé',
    'volume restitué',
    'température',
  ...
  for (const param of allParams) {
    t.true(isOperatorValidForParameter(param, 'mean', 'spatial'), `${param} devrait supporter mean en spatial`)
    t.true(isOperatorValidForParameter(param, 'min', 'spatial'), `${param} devrait supporter min en spatial`)
    t.true(isOperatorValidForParameter(param, 'max', 'spatial'), `${param} devrait supporter max en spatial`)
  }
})
