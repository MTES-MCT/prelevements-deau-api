...
test('la route de debug Sentry est absente de tous les préfixes', async t => {
  const app = express()
  app.use((req, res, next) => {
    req.auth = {type: 'USER_SESSION'}
    next()
  })
  app.use(createRoutes({authMethods: ['magic_link']}))
  app.use('/api', createRoutes({authMethods: ['magic_link']}))

  for (const path of ['/debug-sentry', '/api/debug-sentry']) {
