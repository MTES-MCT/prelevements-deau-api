...
export async function checkPermissionOnDocument(req, res, next) {
  if (req.territoire.code !== req.document.territoire) {
...
export async function checkPermissionOnRegle(req, res, next) {
  if (req.territoire.code !== req.regle.territoire) {
    return next(createHttpError(403, 'Vous n\'avez pas de droit sur cette règle.'))
  }

  next()
}

export async function checkPermissionOnSeries(req, res, next) {
  if (req.territoire.code !== req.series.territoire) {
    return next(createHttpError(403, 'Vous n\'avez pas de droit sur cette série.'))
  }
