  ...
  return async (req, _res, next) => {
    try {
      ...
      const declarantZoneIds = declaration.source
        ? null
        : await Promise.all([
          getDeclarantZoneIds(declaration.declarantUserId),
          getDeclarantZoneIds(declaration.createdByDeclarantUserId)
        ])
      const zoneIds = declaration.source
        ? await getSourceZoneIds(declaration.source.id)
...
export function authorizeDeclarant(
  ...
  return async (req, res, next) => {
    try {
      ...
      if (role === 'INSTRUCTOR') {
        const zoneIds = await getDeclarantZoneIds(declarantId)
        if (!await hasZonePermission(req.user, permission, zoneIds)) {
...
export function authorizeSource(
  ...
  return async (req, res, next) => {
    try {
      ...
      if (role === 'INSTRUCTOR') {
        const zoneIds = await getSourceZoneIds(sourceId)
        if (!await hasZonePermission(req.user, permission, zoneIds)) {
          return next(createHttpError(403, 'Vous ne disposez pas de ce droit pour cette déclaration.'))
        }
...
export function authorizeChunk(
  ...
  return async (req, res, next) => {
    try {
      ...
      if (role === 'INSTRUCTOR') {
        const zoneIds = await getChunkZoneIds(chunkId)
        if (!await hasZonePermission(req.user, permission, zoneIds)) {
