...
import {canCreateDeclarant} from '../services/resource-permissions.js'
import Joi from 'joi'
...
export async function authorizeDeclarantCreation(req, res, next) {
  try {
    ...
    if (!await canCreateDeclarant(req.user)) {
      return next(createHttpError(403, 'Droits insuffisants. Vous devez être administrateur d’au moins une zone.'))
    }
    ...
    try {
      ...
      if (role === 'INSTRUCTOR') {
        ...
        if (pointPrelevementsCount + collecteurRightsCount === 0) {
          if (attribute === 'read' || await canCreateDeclarant(req.user)) {
            return next()
          }
