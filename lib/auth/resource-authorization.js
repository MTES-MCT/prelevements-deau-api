...
async function getExploitationIdsZoneIds(exploitationIds, {client = prisma} = {}) {
  const zoneIds = await Promise.all(
    exploitationIds.map(exploitationId => getExploitationZoneIds(exploitationId, {client}))
  )
    ...
    try {
      ...
      if (user.role === 'INSTRUCTOR') {
        ...
        const zoneIds = exploitationIds.length > 0
          ? await getExploitationIdsZoneIds(exploitationIds, {client})
          : await getDeclarantZoneIds(req.regle.declarantUserId, {client})
  ...
  return async (req, _res, next) => {
    try {
      ...
      if (user.role === 'INSTRUCTOR') {
        ...
        const exploitationIds = [...new Set([
          req.document.declarantPointPrelevementId,
          ...(req.document.exploitations ?? []).map(link => link.declarantPointPrelevementId)
        ].filter(Boolean))]

        if (exploitationIds.length > 0) {
          zoneIds = await getExploitationIdsZoneIds(exploitationIds, {client})
        } else if (req.document.declarantUserId) {
          zoneIds = await getDeclarantZoneIds(req.document.declarantUserId, {client})
        }
