...
const SECURITY_CONTACT_EMAIL = 'contact@partageonsleau.beta.gouv.fr'
...
function renderSecurityContactLink() {
  const email = escapeHtml(SECURITY_CONTACT_EMAIL)
  ...
  <mj-body background-color="#f4f4f4">
    <mj-section background-color="#ffffff" padding="40px 20px">
      <mj-column>
        ...
        <mj-text font-size="12px" color="#666666">
          Si vous n'avez pas demandé cette modification, ignorez cet email, vérifiez la sécurité de votre compte et contactez ${renderSecurityContactLink()}.
        </mj-text>
      </mj-column>
    </mj-section>
  </mj-body>
  ...
  <mj-body background-color="#f4f4f4">
    <mj-section background-color="#ffffff" padding="40px 20px">
      <mj-column>
        ...
        <mj-text font-size="12px" color="#666666">
          Si vous n'êtes pas à l'origine de cette demande, connectez-vous à votre compte pour l'annuler, changez votre mot de passe si vous en utilisez un et contactez ${renderSecurityContactLink()}.
        </mj-text>
      </mj-column>
    </mj-section>
  </mj-body>
  ...
  <mj-body background-color="#f4f4f4">
    <mj-section background-color="#ffffff" padding="40px 20px">
      <mj-column>
        ...
        <mj-text font-size="12px" color="#666666">
          Si vous n'êtes pas à l'origine de ce changement, contactez immédiatement ${renderSecurityContactLink()}.
        </mj-text>
      </mj-column>
    </mj-section>
  </mj-body>
...
async function renderAgentAccountSecurityAlertEmail(user, {
  description,
  title
}) {
  const mjmlTemplate = `
<mjml>
  <mj-head>
    <mj-title>${escapeHtml(title)} - Partageons l'eau</mj-title>
    <mj-attributes>
      <mj-all font-family="Arial, sans-serif" />
      <mj-text font-size="14px" color="#333333" line-height="1.6" />
    </mj-attributes>
  ...
  <mj-body background-color="#f4f4f4">
    <mj-section background-color="#ffffff" padding="40px 20px">
      <mj-column>
        ...
        <mj-text font-size="18px" font-weight="bold" padding-bottom="12px">
          ${escapeHtml(title)}
        </mj-text>
        ...
        <mj-text font-size="12px" color="#666666">
          Si vous n’êtes pas à l’origine de cette demande, contactez immédiatement ${renderSecurityContactLink()}.
        </mj-text>
      </mj-column>
    </mj-section>
  </mj-body>
...
export async function renderAdminAgentDeactivatedAlertEmail(user) {
  return renderAgentAccountSecurityAlertEmail(user, {
    title: 'Votre compte agent a été désactivé',
    description: [
      'Un administrateur vient de désactiver votre compte agent Partageons l’Eau.',
      'Vous ne pouvez plus vous connecter.',
      'Vos habilitations de zone sont conservées mais restent inactives tant que votre compte n’est pas restauré.'
    ].join(' ')
  })
}

export async function renderAdminAgentRestoredAlertEmail(user) {
  const email = escapeHtml(user?.email)

  return renderAgentAccountSecurityAlertEmail(user, {
    title: 'Votre compte agent a été réactivé',
    description: [
      'Un administrateur vient de réactiver votre compte agent Partageons l’Eau.',
      `Vous pouvez de nouveau vous connecter avec l’adresse <strong>${email}</strong>.`,
      'Vos habilitations encore valides sont de nouveau actives.',
      'Votre éventuel accès par mot de passe n’a pas été restauré.'
    ].join(' ')
  })
}

async function renderPasswordSecurityAlertEmail(user, {
  description,
  title,
  warning
}) {
  ...
  <mj-body background-color="#f4f4f4">
    <mj-section background-color="#ffffff" padding="40px 20px">
      <mj-column>
        ...
        <mj-text font-size="12px" color="#666666">
          ${escapeHtml(warning)} contactez immédiatement ${renderSecurityContactLink()}.
        </mj-text>
      </mj-column>
    </mj-section>
  </mj-body>
...
export async function renderPasswordActivatedAlertEmail(user) {
  return renderPasswordSecurityAlertEmail(user, {
    title: 'Votre mot de passe a été défini',
    description: 'Un mot de passe vient d’être défini pour votre compte Partageons l’Eau.',
    warning: 'Si vous n’êtes pas à l’origine de cette opération,'
  })
}

export async function renderPasswordChangedAlertEmail(user) {
  return renderPasswordSecurityAlertEmail(user, {
    title: 'Votre mot de passe a été modifié',
    description: 'Le mot de passe de votre compte Partageons l’Eau vient d’être modifié. Toutes vos autres sessions ont été fermées.',
    warning: 'Si vous n’êtes pas à l’origine de cette modification,'
  })
}

export async function renderPasswordResetAlertEmail(user) {
  return renderPasswordSecurityAlertEmail(user, {
    title: 'Votre accès par mot de passe a été réinitialisé',
    description: 'Un administrateur vient de réinitialiser votre accès par mot de passe. Votre ancien mot de passe et toutes vos sessions ne sont plus valides.',
    warning: 'Si vous n’avez pas demandé cette réinitialisation,'
  })
}

export async function renderCampaignNotificationEmail({user, title, description, context = [], message, actionUrl, actionLabel, footer}) {
  const link = new URL(actionUrl)
  if (!['https:', 'http:'].includes(link.protocol) || link.username || link.password) {
    throw new Error('URL de campagne invalide.')
  }
...
<mjml>
  <mj-head>
    <mj-title>${escapeHtml(title)} - Partageons l'eau</mj-title>
    <mj-preview>${escapeHtml(description)}</mj-preview>
    <mj-attributes>
      <mj-all font-family="Arial, sans-serif" />
      <mj-text font-size="14px" color="#333333" line-height="1.6" />
      <mj-button background-color="#0063CB" color="#ffffff" border-radius="4px" />
  ...
  <mj-body background-color="#f4f4f4">
    <mj-section background-color="#ffffff" padding="40px 20px">
      <mj-column>
        ...
        ${context.map(({label, value}) => renderContextRow(label, value)).join('')}
        ${message
          ? `
        <mj-text font-weight="bold" padding="20px 0 8px 0">Message du collecteur</mj-text>
        <mj-text padding="0 0 20px 0">${escapeHtml(message).replaceAll(/\r\n|\r|\n/g, '<br />')}</mj-text>
        `
          : ''}
        <mj-button href="${escapeHtml(link.toString())}" padding="20px 0 10px 0">
