...
function getClient() {
  client ||= createClient()
  if (!client) {
    throw new Error('S3 disabled (test env)')
  }
...
export function createDownloadDisposition(filename) {
  // Content-Disposition 2+ no longer strips paths from the supplied filename.
  return contentDisposition(path.basename(filename))
}
...
class StorageClient {
  ...
  async uploadObject(objectKey, buffer, options = {}) {
    ...
    if (options.filename) {
      params.ContentDisposition = createDownloadDisposition(options.filename)
    }
  ...
  async getPresignedUrl(objectKey, options = {}) {
    const params = {
      Bucket: this.fullBucketName,
      Key: normalizeKey(objectKey)
    }
    ...
    if (options.filename) {
      params.ResponseContentDisposition = createDownloadDisposition(options.filename)
    }

    if (options.type) {
      params.ResponseContentType = options.type
    }

    const command = new GetObjectCommand(params)
    const expiresIn = Number.isInteger(options.expiresIn) && options.expiresIn > 0
      ? Math.min(options.expiresIn, SIGNED_URL_DURATION)
      : SIGNED_URL_DURATION
