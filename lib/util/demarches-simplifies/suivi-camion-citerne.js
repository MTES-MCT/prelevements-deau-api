...
export default function runCamionCiterneTests(workbook, formData) {
  const errors = []
  ...
  try {
    ...
    validateDataRows(sheet, formData, errors)
  } catch (error) {
    // Erreur inattendue
    errors.push({
...
function validateDataRows(sheet, formData, errors) {
  ...
  for (let rowNum = firstDataRowIndex; rowNum <= range.e.r; rowNum++) {
    ...
    if (dateCell) {
      ...
      if (dateValue) {
        ...
        validateDateInPeriod(dateValue, rowIndex, {formData, errors})
...
function validateNonNullValues(rowContext, errors) {
  ...
  for (let colNum = 1; colNum <= 10; colNum++) {
    const cellValue = getCellValue(sheet, rowNum, colNum)
    if (cellValue !== null && cellValue !== '' && Number.parseFloat(cellValue) !== 0) {
  ...
  if (!hasNonNullValue) {
    errors.push({
      message: `Ligne ${rowIndex}: La date est renseignée, mais aucune valeur non nulle n'est indiquée dans les colonnes B à K.`,
      destinataire: 'déclarant'
    })
  }
}
