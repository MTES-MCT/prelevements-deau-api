...
import {runTests} from './validation.js'
...
function logMemoryUsage(stage) {
  ...
  console.log(`[${stage}] Mémoire utilisée :
...
async function validateFile(file, buffer, champs) {
  const errors = []

  try {
    const fileErrors = await runTests(buffer, file.fileType, champs)
...
export async function processDossier(dossier) {
  logMemoryUsage(`Début du traitement du dossier ${dossier.number}:`)
  ...
  try {
    ...
    await (existingDossier ? updateDossier(existingDossier._id, dossier, processedFiles, isValid) : createDossier(dossier, processedFiles, isValid))
  } catch (error) {
    console.error(`Erreur lors du traitement du dossier ${dossier.number}:`, error)
  }

  logMemoryUsage(`Fin du traitement du dossier ${dossier.number}:`)
}
