...
export async function processDossier(dossier) {
  try {
    ...
    const validationReport = await validateDossier(dossier, downloadedFiles)

    if (Object.keys(validationReport).length > 0) {
      // Uploader le fichier sur S3
      const fileUploadPromises = downloadedFiles.map(async file => {
        uploadFile(`dossiers/${dossier.number}/${file.filename}`, file.buffer)
      })

      await Promise.all(fileUploadPromises)
    }

    const fileToSave = downloadedFiles.map(file => omit(file, ['buffer']))

    // 4. Sauvegarder le résultat dans MongoDB
    const existingDossier = await getDossierByNumber(dossier.number)
    await (existingDossier
      ? updateDossier(existingDossier._id, dossier, fileToSave, validationReport)
      : createDossier(dossier, fileToSave, validationReport)
    )
  } catch (error) {
