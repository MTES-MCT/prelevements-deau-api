  ...
  getGroupedErrors() {
    ...
    for (const [type, sheets] of Object.entries(this.errors)) {
      for (const [sheet, errors] of Object.entries(sheets)) {
        ...
        switch (type) {
          case 'invalidDates': {
            message = `Les dates dans l'onglet '${sheet}' ne sont pas valides pour les cellules ${cells.join(', ')}.`
            break
          }

          case 'invalidTimes': {
            message = `Les heures dans l'onglet '${sheet}' ne sont pas valides pour les cellules ${cells.join(', ')}.`
            break
          }

          case 'missingDate': {
            message = `Le champ 'date' est obligatoire dans l'onglet '${sheet}' pour les cellules ${cells.join(', ')}.`
            break
          }

          case 'missingHeure': {
            const {frequence} = errors[0]
            message = `Le champ 'heure' est obligatoire dans l'onglet '${sheet}' à la fréquence '${frequence}' pour les cellules ${cells.join(', ')}.`
            break
          }

          case 'invalidDateTime': {
            message = `Les dates et heures dans l'onglet '${sheet}' ne sont pas valides pour les cellules ${cells.join(', ')}.`
            break
          }

          case 'missingRemarque': {
            const {paramName} = errors[0]
            message = `Le champ 'Remarque' doit être renseigné si la valeur est manquante pour le paramètre '${paramName}' dans l'onglet '${sheet}', cellules ${cells.join(', ')}.`
            break
          }

          default: {
            message = `Erreur inconnue de type '${type}' dans l'onglet '${sheet}' pour les cellules ${cells.join(', ')}.`
            break
          }
        }
...
export default function runMultiParamTests(workbook, formData) {
  const errors = []
...
function validateParameterData(dataRows, paramIndex, paramName, context, sheetName) {
  ...
  for (const row of dataRows) {
    ...
    // Valider que 'valeur' est un nombre réel positif
    validateNumericValue(valeur, rowNum + 1, paramIndex, context)
