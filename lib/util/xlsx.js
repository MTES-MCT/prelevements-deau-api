...
export function validateDateInPeriod(dateValue, rowIndex, context) {
  ...
  if (formData && formData.date_debut && formData.date_fin) {
    ...
    if (dateValue < dateDebutForm || dateValue > dateFinForm) {
      errors.push({
        message: `Ligne ${rowIndex}: La date ${formatSimpleDate(dateValue)} n'est pas comprise entre le ${formatSimpleDate(dateDebutForm)} et le ${formatSimpleDate(dateFinForm)} (période du formulaire).`,
