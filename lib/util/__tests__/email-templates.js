...
test('les alertes de compte agent décrivent les effets de sécurité', async t => {
  const user = {
    email: 'camille@example.test',
    firstName: 'Camille',
    lastName: 'Rivière'
  }
  const [deactivated, restored] = await Promise.all([
    renderAdminAgentDeactivatedAlertEmail(user),
    renderAdminAgentRestoredAlertEmail(user)
  ])

  t.true(deactivated.includes('compte agent a été désactivé'))
  t.true(deactivated.includes('habilitations de zone sont conservées'))
  t.true(restored.includes('compte agent a été réactivé'))
  t.true(restored.includes('camille@example.test'))
  t.true(restored.includes('accès par mot de passe n’a pas été restauré'))
  t.true(deactivated.includes(SECURITY_CONTACT_LINK))
  t.true(restored.includes(SECURITY_CONTACT_LINK))
})
