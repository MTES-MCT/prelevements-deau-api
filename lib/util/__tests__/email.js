...
import {normalizeEmail, prefixEmailSubject} from '../email.js'

test('prefixEmailSubject / conserve le sujet sans préfixe', t => {
  t.is(prefixEmailSubject('Connexion à Partageons l’eau'), 'Connexion à Partageons l’eau')
})

test('prefixEmailSubject / ajoute le préfixe testing', t => {
  t.is(prefixEmailSubject('Connexion à Partageons l’eau', '[TESTING] '), '[TESTING] Connexion à Partageons l’eau')
})

test('prefixEmailSubject / ajoute le préfixe demo', t => {
  t.is(prefixEmailSubject('Connexion à Partageons l’eau', '[DEMO] '), '[DEMO] Connexion à Partageons l’eau')
})
...
test('normalizeEmail / rejette une adresse de plus de 320 caractères', t => {
  t.throws(
    () => normalizeEmail(`${'a'.repeat(309)}@example.com`),
