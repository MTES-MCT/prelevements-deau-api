...
import runCamionCiterneTests from '../demarches-simplifies/suivi-camion-citerne.js'
...
test('runCamionCiterneTests avec un fichier Excel valide, mais dont la période ne correspond pas à celle indiqué dans le formulaire doit renvoyer une erreur', t => {
  ...
  // Vérification que l'erreur spécifique est présente
  const expectedErrorMessage = 'Ligne 4: La date 01/01/1970 n\'est pas comprise entre le 01/01/2021 et le 31/12/2021 (période du formulaire).'
  const errorMessages = errors.map(error => error.message)
