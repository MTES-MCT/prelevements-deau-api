...
export async function searchDeclarantsHandler(req, res) {
  const result = await searchDeclarants(req.user, parseDeclarantsSearchQuery(req.query))

  res.send({
    ...result,
    items: await decorateDeclarantsRights(result.items, req.user)
  })
}
...
export async function searchCollecteurPreleveursHandler(req, res) {
  ...
  const result = await searchCollecteurPreleveurs(
    req.user.id,
    parseDeclarantsSearchQuery(req.query)
  )
