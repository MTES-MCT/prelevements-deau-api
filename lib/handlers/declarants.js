...
import {serializeDeclarantContactEmails} from '../services/declarant-contact-emails.js'
...
export function toCompactDeclarantSearchItem(item, {
  ...
  const pointCount = item.searchSummary?.pointCount ?? (declarantRole === 'COLLECTEUR'
...
function withDeclarantContactFields(item) {
  return {
    ...item,
    loginEmail: item.email ?? null,
    contactEmails: serializeDeclarantContactEmails(item)
...
export async function listDeclarants(req, res) {
  ...
  const items = await decorateDeclarantsRights(declarants, req.user)
  res.send(items.map(withDeclarantContactFields))
}
...
export async function searchDeclarantsHandler(req, res) {
  ...
  res.send({
    ...
    items: filters.format === 'compact'
      ...
      : items.map(withDeclarantContactFields)
  })
}
...
export async function getCollecteurPreleveursHandler(req, res) {
  ...
  const items = await decorateDeclarantsRights(preleveurs, req.user)
  res.send(items.map(withDeclarantContactFields))
}
