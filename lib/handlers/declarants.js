...
export async function sendDeclarationReminderHandler(req, res) {
  ...
  if (!declarant?.user?.email) {
    throw createHttpError(400, 'Impossible d’envoyer un rappel : ce déclarant n’a pas d’adresse email.')
  }

  if (declarant.declarationNotificationsEnabled === false) {
    throw createHttpError(400, 'Impossible d’envoyer un rappel : ce déclarant est exclu des rappels et relances.')
  }

  const html = await renderDeclarationReminderEmail(declarant.user, FRONT_URL)
