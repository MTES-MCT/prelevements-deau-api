...
export async function reconsolidateDossier(req, res) {
  const dossier = await Dossier.markDossierForReconsolidation(req.dossier._id)
...
export async function reprocessAttachment(req, res) {
  const attachment = await Dossier.markAttachmentForReprocessing(req.attachment._id)
