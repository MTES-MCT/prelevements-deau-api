...
export async function buildSeriesListForAttachment(attachment, territoireCode, {withPoint = false} = {}) {
  const series = await listSeries({territoire: territoireCode, attachmentId: attachment._id, includeExtras: true})
  const mapped = series.map(s => ({
    ...
    minDate: s.minDate,
    maxDate: s.maxDate,
    hasSubDaily: s.hasSubDaily === undefined ? (SUB_DAILY_FREQUENCIES.includes(s.frequency) || undefined) : s.hasSubDaily,
...
export async function buildSeriesValuesPayload(seriesIdRaw, {startDate, endDate, withPoint = false}) {
  const seriesId = parseObjectId(seriesIdRaw)
  if (!seriesId) {
...
export async function getSeriesMetadataHandler(req, res) {
  ...
  res.send({
    series: {
      ...
      minDate: series.minDate,
      maxDate: series.maxDate,
      hasSubDaily: series.hasSubDaily === undefined ? (SUB_DAILY_FREQUENCIES.includes(series.frequency) || undefined) : series.hasSubDaily,
      pointPrelevement: series.pointPrelevement || null,
...
export async function listSeriesMetadataSearch(req, res) {
  ...
  const preleveurOid = preleveurId ? parseObjectId(preleveurId) : null
  const pointOid = pointId ? parseObjectId(pointId) : null
