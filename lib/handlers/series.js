...
export async function buildSeriesListForAttachment(attachment, territoireCode, {withPoint = false} = {}) {
  ...
  const mapped = series.map(s => ({
    ...
    minDate: s.minDate,
    maxDate: s.maxDate,
    hasSubDaily: s.hasSubDaily !== undefined ? s.hasSubDaily : (SUB_DAILY_FREQUENCIES.includes(s.frequency) || undefined),
...
export async function getSeriesMetadataHandler(req, res) {
  ...
  res.send({
    series: {
      ...
      minDate: series.minDate,
      maxDate: series.maxDate,
      hasSubDaily: series.hasSubDaily !== undefined ? series.hasSubDaily : (SUB_DAILY_FREQUENCIES.includes(series.frequency) || undefined),
      pointPrelevement: series.pointPrelevement || null,
