...
import {getReadableTelemetrySourceWhere, getVisibleTelemetryChunksWhere, hydrateMeterSourcePeriods, scopeMeterSource} from '../services/telemetry-source-access.js'
import {getCoordsByPointIds} from '../models/point-prelevement.js'
import {
  exploitationAtDateWhere,
  exploitationOverlapsPeriodWhere,
  findUniqueExploitationForPeriod,
  selectUniqueExploitation
...
import {campaignDeclarationAccessWhere, campaignReceiptAccessWhere} from '../services/collection-campaigns.js'
...
import {assertManualCollectionAccess, assertManualVolumePublication, manualCollectionAccessWhere} from '../services/manual-collection.js'
import {
...
export const createDeclarationSchema = Joi.object({
  ...
  declarantUserId: resourceIdSchema.optional(),
  preleveurUserId: resourceIdSchema.optional(),
  targetDeclarantUserId: resourceIdSchema.optional(),
...
export const reconcileDeclarationChunkSchema = Joi.object({
...
export const quickDeclarationContextSchema = Joi.object({
  declarantUserId: resourceIdSchema.optional(),
  preleveurUserId: resourceIdSchema.optional(),
  targetDeclarantUserId: resourceIdSchema.optional()
}).unknown(true)
...
const quickDeclarationEntrySchema = Joi.object({
  pointPrelevementId: resourceIdSchema.required(),
  exploitationId: resourceIdSchema.optional(),
  countingCode: Joi.string().trim().allow('', null).optional(),
...
export const createQuickDeclarationSchema = Joi.object({
...
export function canReadDeclarationWhere(userId, preleveurIds = []) {
  return {
    ...
    ]}, campaignDeclarationAccessWhere(userId)]
  }
}
...
export function canReadTelemetrySourceWhere(declarantUserIds, options) {
  return getReadableTelemetrySourceWhere(declarantUserIds, options)
}
...
function getTelemetrySourceDeclarant(source, declarantUserIds) {
  const meteredPreleveur = source.chunks?.find(chunk => chunk.calculationStrategy === 'METER' && chunk.preleveur)?.preleveur
  if (meteredPreleveur) return getDeclarantDisplay(meteredPreleveur)
...
async function decorateTelemetrySourcesForDeclarant(sources, declarantUserIds) {
  return (await hydrateMeterSourcePeriods(sources)).map(scopeMeterSource).map(source => ({
    ...source,
...
async function getLastIndexReadingsByPoint({declarantUserId, pointPrelevementIds}) {
  ...
  const values = await prisma.chunkValue.findMany({
    where: {
      ...
      chunk: {
        ...
        // This legacy single-index form has no meter selector. Do not suggest
        // a physical meter's index as an unidentified exploitation index.
        compteurId: null,
        calculationStrategy: 'GENERIC',
  ...
  for (const value of values) {
    const pointPrelevementId = value.chunk?.exploitationId ?? value.chunk?.pointPrelevementId
...
async function getLatestIndexReadingsByPoint({
  ...
  const hiddenReceipts = await prisma.declaration.findMany({where: {
    declarantUserId,
    collectionResponse: {is: {NOT: campaignReceiptAccessWhere(viewerUserId)}},
    source: {chunks: {some: {pointPrelevementId: {in: uniquePointIds}}}}
  }, select: {id: true}})
  const receiptFilter = hiddenReceipts.length
    ? Prisma.sql`AND d.id NOT IN (${Prisma.join(hiddenReceipts.map(row => Prisma.sql`${row.id}::uuid`))})`
    : Prisma.empty
  ...
  const values = await prisma.$queryRaw`
    WITH ranked_index_values AS (
      SELECT
        ...
        s."declarationId",
        ROW_NUMBER() OVER (
          PARTITION BY c."pointPrelevementId", c."exploitationId", c."compteurId"
          ORDER BY v."periodEnd" DESC, v."createdAt" DESC, v.id DESC
        ) AS "pointRowNumber",
        ROW_NUMBER() OVER (
          PARTITION BY c."pointPrelevementId", c."exploitationId", c."compteurId", v."metricTypeCode", v."periodStart", v."periodEnd"
      ...
      LEFT JOIN "Compteur" meter ON meter.id = c."compteurId"
  ...
  for (const value of values) {
    const pointPrelevementId = value.exploitationId ?? value.pointPrelevementId
...
function collectQuickDeclarationDeclaredValues(chunks) {
  ...
  for (const chunk of chunks ?? []) {
    for (const value of chunk.chunkValues ?? []) {
      ...
      values.push({
        ...value,
        compteurId: chunk.compteurId,
        exploitationId: chunk.exploitationId,
...
async function getQuickDeclarationValueReplacementStatuses({declarantUserId, viewerUserId, chunks}) {
      ...
      chunk: {
        ...
        source: {
          ...
          declaration: {
            ...
            ...campaignDeclarationAccessWhere(viewerUserId)
          }
        }
      }
    },
  ...
  for (const row of rows) {
    const key = getQuickDeclarationValueReplacementKey({
      ...
      exploitationId: row.chunk?.exploitationId,
...
async function getQuickDeclarationReplacementAuditValuesByChunk(chunks, viewerUserId) {
  ...
  const replacementValues = replacementValueIds.length === 0
    ...
    : await prisma.chunkValue.findMany({
      where: {
        ...
        chunk: {source: {OR: [{declaration: null}, {declaration: campaignDeclarationAccessWhere(viewerUserId)}]}}
      },
      select: {
        id: true,
        periodStart: true,
...
function decorateQuickDeclarationChunksWithLatestIndexReadings(declaration, latestIndexReadingsByPoint) {
  ...
  return {
    ...
    source: {
      ...
      chunks: declaration.source.chunks.map(chunk => ({
        ...chunk,
        latestIndexReadings: (latestIndexReadingsByPoint.get(chunk.exploitationId ?? chunk.pointPrelevementId) ?? [])
          .filter(reading => (reading.compteurId ?? null) === (chunk.compteurId ?? null))
      }))
    }
  }
}
...
async function getLastKnownUsagesByPoint({declarantUserId, pointPrelevementIds}) {
  ...
  for (const chunk of chunks) {
    const pointPrelevementId = chunk.exploitationId ?? chunk.pointPrelevementId
...
export function serializeQuickDeclarationPoint({
  ...
  return {
    ...
    lastKnownUsage: lastKnownUsagesByPointId.get(exploitation.id) ?? lastKnownUsagesByPointId.get(pointId) ?? null,
    lastVolumePeriods: lastVolumePeriodsByPointId.get(exploitation.id) ?? lastVolumePeriodsByPointId.get(pointId) ?? null,
    lastReading: serializeQuickDeclarationLastReading(lastReadingsByPointId.get(exploitation.id) ?? lastReadingsByPointId.get(pointId))
  }
}

async function getQuickDeclarationPoints(declarantUserId, actorDeclarantUserId = declarantUserId) {
  const exploitations = await prisma.declarantPointPrelevement.findMany({
    where: {
      ...manualCollectionAccessWhere({actorDeclarantUserId, declarantUserId}),
      ...exploitationAtDateWhere()
    },
    include: {
...
function ensureUniquePointEntries(entries) {
  ...
  for (const entry of entries) {
    const identity = entry.exploitationId ?? `${entry.pointPrelevementId}:${entry.countingCode ?? ''}`
...
async function getLinkedExploitationsByPointId({declarantUserId, actorDeclarantUserId = declarantUserId, pointPrelevementIds, client = prisma}) {
  ...
  // The point map is used only for point-level access and naming. Selection of
  // the declaration beneficiary always goes through resolveQuickExploitation.
  const byPoint = new Map(exploitations.map(exploitation => [exploitation.pointPrelevementId, exploitation]))
  byPoint.exploitations = exploitations
  return byPoint
}

export function resolveQuickExploitation(exploitationsByPoint, entry) {
  const candidates = (exploitationsByPoint.exploitations ?? [...exploitationsByPoint.values()])
    .filter(exploitation => exploitation.pointPrelevementId === entry.pointPrelevementId
      && (!entry.exploitationId || exploitation.id === entry.exploitationId)
      && (entry.countingCode == null || exploitation.countingCode === entry.countingCode.trim()))
  const exploitation = selectUniqueExploitation(candidates, {pointPrelevementId: entry.pointPrelevementId})
  if (!exploitation) throw createHttpError(403, 'Cette exploitation n’est pas autorisée pour cette saisie.')
  return exploitation
}
...
async function getReconciliationConflicts({
  ...
  return prisma.chunk.findMany({
    where: {
      id: {not: chunkId},
      pointPrelevementId,
      ...(exploitationId ? {OR: [{exploitationId}, {exploitationId: null}]} : {}),
    ...
    try {
      ...
      if (!filesPersisted) {
        try {
          ...
          // Cleanup must not replace the original upload or persistence error.
        }

        try {
          await Promise.all(uploadedKeys.map(async k => storage.deleteObject(k, true)))
        } catch {
          // Best-effort cleanup; preserve the original error for the caller.
        }
      }
...
export async function getQuickDeclarationContextHandler(req, res, next) {
  try {
    ...
    const [points, waterUses] = await Promise.all([
      getQuickDeclarationPoints(targetDeclarantUserId, actorDeclarantUserId),
      listSandreWaterUses()
    ])
...
export async function previewQuickDeclarationConflictsHandler(req, res, next) {
  try {
    ...
    const requestedPointIds = [...new Set([
      ...value.entries.map(entry => entry.pointPrelevementId),
      ...value.pointUsageNames.map(entry => entry.pointPrelevementId)
    ])]
    const authorizedExploitations = await getLinkedExploitationsByPointId({
      declarantUserId,
      actorDeclarantUserId: createdByDeclarantUserId,
      pointPrelevementIds: requestedPointIds
    })
    assertManualCollectionAccess(authorizedExploitations, requestedPointIds)
    const resolvedEntries = value.entries.map(entry => ({...entry, exploitationId: resolveQuickExploitation(authorizedExploitations, entry).id}))
    ensureUniquePointEntries(resolvedEntries)
    ...
    const exploitationsByPointId = authorizedExploitations
    ...
    for (const entry of entries) {
      const exploitation = resolveQuickExploitation(exploitationsByPointId, entry)
      const valueRows = [{
        metricTypeCode,
  ...
  try {
    ...
    const entries = await Promise.all(value.entries.map(async entry => {
      ...
      return {
        pointPrelevementId: entry.pointPrelevementId,
        exploitationId: entry.exploitationId,
        countingCode: entry.countingCode,
        value: Number(entry.value ?? entry.index),
        usageId: usage.id,
        usage
      }
    ...
    assertManualCollectionAccess(exploitationsByPointId, requestedPointPrelevementIds)
    for (const entry of entries) entry.exploitationId = resolveQuickExploitation(exploitationsByPointId, entry).id
    ensureUniquePointEntries(entries)
    ...
    const declaration = await prisma.$transaction(async tx => {
      const currentExploitations = await getLinkedExploitationsByPointId({
        declarantUserId,
        actorDeclarantUserId: createdByDeclarantUserId,
        pointPrelevementIds: requestedPointPrelevementIds,
        client: tx
      })
      assertManualCollectionAccess(currentExploitations, requestedPointPrelevementIds)
      for (const entry of entries) resolveQuickExploitation(currentExploitations, entry)
      ...
      if (!isIndexMeasurement) {
        for (const {chunkId, entry, valueRow} of quickDeclarationChunks) {
          // eslint-disable-next-line no-await-in-loop
          const conflictResult = await applyConflictPolicyForIncomingChunkValues({
            pointPrelevementId: entry.pointPrelevementId,
          ...
          assertManualVolumePublication(conflictResult, 1)
        }
      }
        ...
        source: {
          include: {
            chunks: {
              include: {
                pointPrelevement: true,
                exploitation: {select: {id: true, countingCode: true}},
                usage: true,
                _count: {
                  select: {
                    chunkValues: true
...
export async function listMyTelemetrySourcesHandler(req, res, next) {
  try {
    ...
    const sources = await prisma.source.findMany({
      where: canReadTelemetrySourceWhere(declarantUserIds, {user: req.user}),
      orderBy: {createdAt: 'desc'},
      include: {
        chunks: {
          where: getVisibleTelemetryChunksWhere({user: req.user}),
          orderBy: [{minDate: 'asc'}, {createdAt: 'asc'}],
          include: {
            preleveur: {include: {user: true}},
            usage: true,
            _count: {
              select: {
                chunkValues: true
    ...
    return res.json({
      success: true,
      data: await decorateTelemetrySourcesForDeclarant(sources, declarantUserIds)
    })
  } catch (error) {
    next(error)
  }
...
export async function getMyTelemetrySourceHandler(req, res, next) {
  try {
    ...
    const source = await prisma.source.findFirst({
      where: {
        id: sourceId,
        ...canReadTelemetrySourceWhere(declarantUserIds, {user: req.user})
      },
      include: {
        chunks: {
          where: getVisibleTelemetryChunksWhere({user: req.user}),
    ...
    const [decorated] = await decorateTelemetrySourcesForDeclarant([source], declarantUserIds)
...
export async function getDeclarationDetailHandler(req, res, next) {
  try {
    ...
    const replacementAuditValuesByChunkId = isManualQuickDeclaration
      ? await getQuickDeclarationReplacementAuditValuesByChunk(declaration.source?.chunks ?? [], userId)
      : new Map()
    ...
    const storage = declaration.files.length > 0 ? createStorageClient(DECLARATIONS_BUCKET) : null
...
export async function requestDeclarationPointsChangeHandler(req, res, next) {
  try {
    ...
    const declaration = await prisma.declaration.findFirst({
      ...
      include: {
        collectionResponse: {select: {id: true}},
        files: true,
        source: {
          include: {
            chunks: {
    ...
    if (declaration.collectionResponse) {
      throw createHttpError(409, 'Cette déclaration appartient à une campagne. Corrigez la réponse depuis la campagne.')
    }
...
export async function deleteDeclarationHandler(req, res, next) {
  try {
    ...
    if (await prisma.collectionResponse.findUnique({where: {declarationId}, select: {id: true}})) {
      throw createHttpError(409, 'Cette déclaration appartient à une campagne. Corrigez la réponse depuis la campagne.')
    }
...
export async function reconcileDeclarationChunkHandler(req, res, next) {
  try {
    ...
    if (declaration.collectionResponse || chunk.metadata?.collectionResponseId) {
      throw createHttpError(409, 'Cette déclaration appartient à une campagne. Corrigez la réponse depuis la campagne.')
    }

    if (chunk.calculationStrategy && chunk.calculationStrategy !== 'GENERIC') {
    ...
    const targetExploitationId = value.exploitationId ?? null
    if (!targetPointPrelevementId && targetExploitationId) throw createHttpError(400, 'Une exploitation nécessite un point de prélèvement.')
    const previousPointPrelevementId = chunk.pointPrelevementId
    const exploitationChanged = targetExploitationId && chunk.exploitationId !== targetExploitationId
    const pointAssociationChanged = previousPointPrelevementId !== targetPointPrelevementId || exploitationChanged

    if (!isChunkPointAssociationChangeAllowed(chunk, targetPointPrelevementId)
      || (exploitationChanged && !isChunkPointAssociationChangeAllowed(chunk, null))) {
    ...
    const shouldRemovePreviousAlias = Boolean(previousPointPrelevementId) && pointAssociationChanged

    const previousExploitation = shouldRemovePreviousAlias
      ? await getActiveExploitationForReconciliation({
        declarantUserId: declaration.declarantUserId,
        pointPrelevementId: previousPointPrelevementId,
        exploitationId: chunk.exploitationId ?? undefined,
    ...
    if (!isDetaching) {
      exploitation = await getActiveExploitationForReconciliation({
        ...
        exploitationId: targetExploitationId ?? (previousPointPrelevementId === targetPointPrelevementId ? chunk.exploitationId : undefined),
...
export async function getAvailablePointsPrelevementsForDeclarationHandler(req, res, next) {
  try {
    ...
    if (isDeclarant) {
      ...
      if (canRead === 0) {
        if (declaration.collectionResponse) throw createHttpError(404, 'Déclaration introuvable')
...
export async function getAvailablePointsPrelevementsForDeclaration({
  ...
  const period = declarationId ? await prisma.chunk.aggregate({
    where: {source: {declarationId}}, _min: {minDate: true}, _max: {maxDate: true}
  }) : null
  const periodWhere = period?._min.minDate && period?._max.maxDate
    ? exploitationOverlapsPeriodWhere(period._min.minDate, period._max.maxDate)
  ...
  const exploitations = await prisma.declarantPointPrelevement.findMany({
    ...
    select: {
      ...
      startDate: true,
      endDate: true,
      declarant: {select: {socialReason: true, user: {select: {firstName: true, lastName: true}}}},
      usage: true,
      secondaryUsageLinks: {
        include: {usage: true}
      },
  ...
  return points.map(point => ({
    ...point,
    exploitations: exploitations.filter(exploitation => exploitation.pointPrelevementId === point.id)
      .map(({id, countingCode, declarantUserId, startDate, endDate, declarant}) => ({
        id, countingCode, declarantUserId, startDate, endDate,
        declarantLabel: getDeclarantLabelForPointsChangeRequest(declarant)
      })),
