...
export async function getAuthorizedConnector({declarantId, pointId, connectorType, sourcePointId, connectorId, exploitationId, countingCode, client = prisma}) {
  const exploitations = await client.declarantPointPrelevement.findMany({
    where: {
      ...
      ...(countingCode == null ? {} : {countingCode: countingCode.trim()}),
      connectors: {
  ...
  const candidates = exploitations.flatMap(exploitation => exploitation.connectors.filter(candidate => {
  ...
  }).map(connector => ({exploitation, connector})))
  ...
  const {connector, exploitation} = candidates[0]
...
export async function ingestServiceAccountConnectorOutputHandler(req, res) {
  ...
  const exploitationId = metadata.exploitation_id
  const countingCode = metadata.counting_code
  if ((exploitationId != null && typeof exploitationId !== 'string')
    || (countingCode != null && typeof countingCode !== 'string')) {
    throw createHttpError(400, 'Identité d’exploitation invalide.')
  }
  ...
  if (metrics.some(metric => (metric.exploitationId && metric.exploitationId !== authorizedConnector.exploitationId)
    || (metric.countingCode != null && (typeof metric.countingCode !== 'string' || metric.countingCode.trim() !== authorizedConnector.countingCode)))) {
    throw createHttpError(409, 'Le comptage du fichier ne correspond pas à l’exploitation de ce connecteur.')
  }
  ...
  for (const {metric, analysis} of metricAnalyses) {
    ...
    const conflictResolution = await applyConflictPolicyForIncomingChunkValues({
      ...
      exploitationId: authorizedConnector.exploitationId,
      valueRows: values,
  ...
  for (const {metric, usageId, values} of importableMetricAnalyses) {
    ...
    const chunk = await prisma.chunk.create({
      data: {
        ...
        metadata: {
          connector,
          countingCode: authorizedConnector.countingCode,
          connectorId: authorizedConnector.connector.id,
          connectorRate,
          flowType,
          sourceFlowType,
  ...
  await prisma.$transaction(async tx => {
    ...
    for (const usageId of declaredRootUsageIds) {
      // eslint-disable-next-line no-await-in-loop
      await addExploitationSecondaryUsage(authorizedConnector.exploitationId, usageId, {client: tx})
    }
