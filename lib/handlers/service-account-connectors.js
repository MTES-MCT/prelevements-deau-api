...
function getMetricUsage(metric) {
  return getUsageEauValue(metric?.usage)
}

function appendMissingUsageValues(usages = [], usagesToAppend = []) {
  const existingUsages = Array.isArray(usages) ? usages : []

  return [
    ...new Set([
      ...existingUsages,
      ...usagesToAppend.filter(Boolean)
    ])
  ]
}
...
export async function ingestServiceAccountConnectorOutputHandler(req, res) {
  ...
  console.log(
    `[service-account-connectors] ingest summary pointId=${pointId} declarantId=${declarantId} connectorId=${authorizedConnector.connector.id} connectorRate=${connectorRate} metrics=${metrics.length} values=${allValues.length} hasIndexMetric=${shouldReconstructVolumesFromIndex}`
  )
  ...
  for (const metric of metrics) {
    ...
    const usage = getMetricUsage(metric)

    if (values.length === 0) {
      const rejectionStats = metricAnalysis?.analysis.rejectionStats ?? {}
      if (Object.keys(rejectionStats).length > 0) {
        console.log(
          `[service-account-connectors] metric rejected pointId=${pointId} connectorId=${authorizedConnector.connector.id} type=${metric.type} granularity=${metric.granularity} reasons=${JSON.stringify(rejectionStats)}`
        )
      }
    ...
    if (usage) {
      createdMetricUsages.push(usage)
    }
  }
  ...
  const updatedUsages = appendMissingUsageValues(authorizedConnector.usages, createdMetricUsages)
  const shouldUpdateUsages = updatedUsages.length > authorizedConnector.usages.length
  ...
  await prisma.declarantPointPrelevement.update({
    ...
    data: {
      ...
      ...(shouldUpdateUsages ? {usages: updatedUsages} : {})
    }
  })
