...
import {normalizeSeriesFrequency} from '../util/normalize-series-frequency.js'
...
function getMetricValues(metric) {
  ...
  return metric.values
    .map(value => {
      ...
      if (!date || !Number.isFinite(numberValue)) {
        return null
      }
...
export async function ingestServiceAccountConnectorOutputHandler(req, res) {
  ...
  const minDate = new Date(
    Math.min(...allValues.map(value => value.date.getTime()))
  )

  const maxDate = new Date(
    Math.max(...allValues.map(value => value.date.getTime()))
  )
  ...
  for (const metric of metrics) {
    ...
    await prisma.chunkValue.createMany({
      data: values.map(value => ({
        ...
        unit: metric.unit,
        frequency: normalizeSeriesFrequency(metric.granularity) ?? metric.granularity,
        date: value.date,
        valueKind: 'DECLARED',
        value: value.value
      }))
