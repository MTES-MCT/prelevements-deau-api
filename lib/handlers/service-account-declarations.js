...
const UUID_REGEXP = /^[\da-f]{8}(?:-[\da-f]{4}){3}-[\da-f]{12}$/i

function getObjectMetadata(metadata) {
  return metadata && typeof metadata === 'object' && !Array.isArray(metadata) ? metadata : {}
}

function getDeclarationWhereFromIdentifier(identifier) {
  const trimmedIdentifier = String(identifier || '').trim()

  return UUID_REGEXP.test(trimmedIdentifier)
    ? {id: trimmedIdentifier}
    : {code: trimmedIdentifier.toUpperCase()}
}
...
export function serializeDeclarationProcessingPoint(point) {
  return {
    pointId: point.id,
    name: point.name,
    codeBSS: point.codeBSS || undefined,
...
export async function getDeclarationProcessingContextHandler(req, res) {
  ...
  const declaration = await prisma.declaration.findFirst({
    where: getDeclarationWhereFromIdentifier(declarationId),
    include: {
  ...
  if (declaration.collectionResponse) {
    throw createHttpError(409, 'Cette déclaration appartient à une campagne et ne peut pas être retraitée.')
  }
  ...
  res.status(200).json({
    ...
    data: {
      ...
      files,
      points: points.map(serializeDeclarationProcessingPoint)
    }
  })
}
