...
import {createDeclarationExploitationMatcher} from '../services/declaration-exploitation-matching.js'
import {
  getRootWaterUseCode,
  getWaterUse,
...
function validateUuid(value, label, schema = uuidSchema) {
  const {error, value: uuid} = schema.validate(value)
...
function createDeclarationIndex(chunks, periods, exploitations) {
  const declarationIndex = new Map()
  const matchExploitation = createDeclarationExploitationMatcher(exploitations)
  ...
  for (const chunk of chunks) {
    ...
    const exploitationId = matchExploitation(chunk)
    ...
    for (const period of periods) {
      ...
      const key = `${exploitationId}:${period.key}`
...
function createMatrixCell({declarationIndex, exploitation, period, summary}) {
  const declarationsById = declarationIndex.get(`${exploitation.id}:${period.key}`)
...
export function createMatrixRows({exploitations, chunks, periods}) {
  const declarationIndex = createDeclarationIndex(chunks, periods, exploitations)
  ...
  for (const exploitation of exploitations) {
    ...
    const row = {
      id: exploitation.id,
      exploitationId: exploitation.id,
      countingCode: optionalText(exploitation.countingCode),
  ...
  const groups = [...groupsByDeclarantId.values()]
    .map(group => ({
      ...group,
      rows: group.rows.sort((a, b) => a.pointName.localeCompare(b.pointName, 'fr') || String(a.countingCode ?? '').localeCompare(String(b.countingCode ?? ''), 'fr'))
    }))
...
export function createMissingDeclarationsWorkbook({matrix, periods}) {
  ...
  sheet.columns = [
    ...
    {header: 'Code comptage', key: 'countingCode', width: 24},
  ...
  for (const row of matrix.rows) {
    ...
    for (const cell of missingCells) {
      ...
      sheet.addRow({
        ...
        countingCode: exportValue(row.countingCode),
...
export async function getZonePointPrelevementHandler(req, res) {
  const zoneId = validateUuid(req.params.zoneId, 'Identifiant de zone')
  const pointId = validateUuid(req.params.pointId, 'Identifiant de point', resourceIdSchema.required())
...
export async function getZoneExploitationHandler(req, res) {
  const zoneId = validateUuid(req.params.zoneId, 'Identifiant de zone')
  const exploitationId = validateUuid(req.params.exploitationId, 'Identifiant d’exploitation', resourceIdSchema.required())
