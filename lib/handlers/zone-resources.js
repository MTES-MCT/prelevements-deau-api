...
function getCellStatus({declarations, isExpected}) {
  ...
  return isExpected ? 'MISSING' : 'INACTIVE'
}
...
export async function getZoneDeclarationMonthlyStatusHandler(req, res) {
  ...
  const serializedPeriods = matrixQuery.periods.map(period => ({
    key: period.key,
    label: period.label,
  ...
  res.send({
    ...
    meta: {
      ...
      to: matrixQuery.to,
      periodsCount: matrixQuery.periodsCount,
...
export async function exportZoneDeclarationMissingHandler(req, res) {
  ...
  const query = {
    ...req.query,
    to: req.query.periodKey ?? req.query.to,
    periodCount: req.query.periodKey ? 1 : req.query.periodCount
  }
  ...
  const filename = matrixQuery.from === matrixQuery.to
    ? `non-declarants-${matrixQuery.periodType}-${matrixQuery.to}.xlsx`
    : `non-declarants-${matrixQuery.periodType}-${matrixQuery.from}-${matrixQuery.to}.xlsx`
