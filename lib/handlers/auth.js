...
import {getUserByEmail, getUserById, updateLastLoginAt, updateUserById} from '../models/user.js'
import {createAuthToken, getAuthTokenByToken} from '../models/auth-token.js'
...
async function processAuthTokenVerification(token) {
  ...
  // Créer une session
  const session = await createSessionToken(user.id, user.role)
