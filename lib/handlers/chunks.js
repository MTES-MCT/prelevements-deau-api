...
export async function updateChunkInstructionHandler(req, res, next) {
  try {
    ...
    const accessiblePointIdsSet = isAdmin
      ? null
      : await getAccessiblePointPrelevementIdsSetForInstructor(
        instructorUserId,
        now
      )
    ...
    const transactionResult = await prisma.$transaction(async tx => {
      ...
      if (hasPointPrelevementIdInPayload && finalPointPrelevementId !== null) {
        if (isAdmin) {
          const point = await tx.pointPrelevement.findFirst({
        ...
        } else if (!accessiblePointIdsSet.has(finalPointPrelevementId)) {
    ...
    const refreshedSource = isAdmin
      ? await getSourceForAdmin(transactionResult.sourceId)
      : await getSourceForInstructor(
        instructorUserId,
