...
export async function ingestMeterBatchHandler(req, res) {
  // Avoid retaining thousands of SQL spans for one batch. The HTTP parent,
  // error reporting and audit remain outside this tracing-only boundary.
  res.send(await suppressTracing(() => ingestMeterBatch(req.body, actor(req))))
}
...
export async function getExploitationMeterAllocationsHandler(req, res) {
  ...
  const meterAllocations = allocations.map(allocation => {
    ...
    const manualCollection = stream?.provider === 'manual-collection'
    ...
    return {
      ...
      collectionCampaignId: manualCollection ? stream.scope : null,
      sync: {available: Boolean(stream) && !manualCollection, streamId: stream?.id ?? null, state, lastSuccessAt: stream?.lastSuccessAt ?? null, lastReadingAt: stream?.lastReadingAt ?? null},
      ...
      capabilities: {canReadGlobalReadings: req.user?.role === 'ADMIN', canEditAllocations: req.user?.role === 'ADMIN' && Boolean(stream) && !manualCollection}
    }
  })
