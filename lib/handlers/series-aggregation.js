...
/**
 * Agrège temporellement des valeurs qui ont la même période.
 ...
 */
export function aggregateValuesByPeriodKey(values, operator) {
...
function aggregateValuesByDate(valuesBySeriesId, seriesList, aggregationContext) {
  ...
  for (const series of seriesList) {
    ...
    for (const valueDoc of valueDocs) {
      ...
      const temporallyAggregated = aggregateValuesByPeriodKey(extractedValues, temporalOperator)
  ...
  // Note : cette fonction fait l'agrégation SPATIALE (plusieurs points)
  // L'agrégation TEMPORELLE est faite par aggregateValuesByPeriodKey
 ...
 * Compile les remarques (max 10 uniques par période).
 *
