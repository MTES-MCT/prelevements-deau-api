  ...
  for (const series of seriesList) {
    ...
    for (const valueDoc of valueDocs) {
      ...
      for (const item of extracted) {
        ...
        item.pointId = series.computed?.point || series.pointPrelevement || null
        item.metricTypeCode = aggregationContext.metricTypeCode
...
export async function getAggregatedSeriesHandler(req, res) {
  ...
  if (hasOverlap && spatialOperator !== 'sum' && !canResolveIndexOverlap) {
    throw createHttpError(
      400,
