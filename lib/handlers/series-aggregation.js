...
export function validateQueryParams(query) {
  ...
  return {
    ...value,
    metricTypeCode: normalizeMetricTypeCode(requestedMetricTypeCode),
...
export function buildAggregationMetadata({
  ...
  startDate,
  endDate,
  campaignVolumesEstimated = false
}) {
  const isCumulativeMetric = parametersConfig[metricTypeCode]?.valueType === 'cumulative'
  const metadata = {
    metricTypeCode,
    ...
    ...(campaignVolumesEstimated ? {campaignVolumesEstimated: true} : {}),
...
export async function getExactCampaignPeriods({chunkIds, metricTypeCode, startDate, endDate, client = prisma}) {
  ...
  const values = await client.chunkValue.findMany({
    where: {
      ...
      valueKind: 'COMPUTED',
      chunk: {calculationStrategy: 'CAMPAIGN'},
    ...
    select: {
      ...
      chunk: {select: {pointPrelevementId: true, metadata: true}}
    },
  ...
  return values.map(value => ({
    ...
    campaignId: value.chunk.metadata?.campaignId ?? null,
    submissionId: value.chunk.metadata?.submissionId ?? null,
...
export async function getAggregatedSeriesHandler(req, res) {
  ...
  const exactPeriods = await getExactCampaignPeriods({chunkIds, metricTypeCode, startDate, endDate})
  ...
  const metadata = buildAggregationMetadata({
    ...
    startDate,
    endDate,
    campaignVolumesEstimated: exactPeriods.length > 0
  })
