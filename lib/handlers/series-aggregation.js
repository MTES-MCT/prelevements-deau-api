...
export async function getAggregatedSeriesHandler(req, res) {
  ...
  // Valider l'opérateur spatial uniquement si agrégation spatiale nécessaire
  // (plusieurs séries avec overlap OU paramètre avec opérateurs spatiaux disponibles)
  ...
  // 6. Agréger les valeurs par date ou période (agrégation spatiale si nécessaire)
  // Note : Si séries consécutives (pas d'overlap), il n'y aura qu'une valeur par période,
  // donc l'opérateur d'agrégation retournera juste cette valeur unique
  const aggregatedByPeriod = aggregateValuesByDate(valuesBySeriesId, seriesList, {
    operator: spatialOperator || 'mean', // Fallback pour paramètres sans opérateur spatial
    parameter,
    aggregationFrequency
  })
