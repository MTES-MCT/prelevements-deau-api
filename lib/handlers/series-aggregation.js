...
export {ALL_FREQUENCIES, DAILY_FREQUENCY}
 ...
 */
export async function resolvePointsForAggregation({pointIdsStr, preleveurId, sourceId}) {
  ...
  throw createHttpError(400, 'Vous devez fournir au moins pointIds, preleveurId ou sourceId')
}
...
async function fetchAllSeriesValues(seriesList, startDate, endDate) {
  ...
  // Overlap: key = `${pointId}:${date}` count>1 dans le scope
...
async function resolvePointsAndSeries({sourceId, pointIdsStr, preleveurId, metricTypeCode, startDate, endDate}) {
  const {resolvedPoints, notFound} = await resolvePointsForAggregation({pointIdsStr, preleveurId, sourceId})
