 ...
 */
async function resolvePointById(pointId) {
...
async function resolvePointIds(pointIds) {
  ...
  const resolvePromises = pointIds.map(async pointId => {
    const point = await resolvePointById(pointId)
  ...
  for (const {pointId, point} of results) {
    if (point) {
      found.push({
        seqId: point.id || pointId, // Utiliser id_point si disponible, sinon l'ID original
        objectId: point.id,
        point
      })
    } else {
 ...
 */
async function resolvePreleveurPoints(preleveurId) {
  // Résoudre le préleveur (accepte ID numérique ou ObjectId)
  const preleveur = await resolvePreleveurById(preleveurId)

  if (!preleveur) {
    throw createHttpError(404, `Préleveur non trouvé: ${preleveurId}`)
  }

  const points = await getPointsFromPreleveur(preleveur.id)
 ...
 *
 * @param {Array} seriesList - Liste des séries avec leur id et frequency
  ...
  for (const s of series) {
    if (s.computed?.integratedDays && Array.isArray(s.computed.integratedDays)) {
      for (const date of s.computed.integratedDays) {
        ...
        dateToSeriesMap.get(date).add(s.id.toString())
      }
    }
  }
...
export async function resolvePointsForAggregation({pointIdsStr, preleveurId}) {
  if (preleveurId && pointIdsStr) {
    ...
    const {found: preleveurPoints} = await resolvePreleveurPoints(preleveurId)
  ...
  if (pointIdsStr) {
    ...
    const pointIds = pointIdsStr.split(',')
    const {found, notFound} = await resolvePointIds(pointIds)
  ...
  const {found, notFound} = await resolvePreleveurPoints(preleveurId)
  return {resolvedPoints: found, notFound}
}
 ...
 */
export async function resolveSeriesForAttachment({attachmentId, pointIdsStr, parameter, startDate, endDate}) {
  ...
  const {found: resolvedPoints} = await resolvePointIds(uniquePointIds)
 ...
 */
export async function resolvePreleveurObjectId(preleveurId) {
  if (!preleveurId) {
    return null
  }

  const preleveur = await resolvePreleveurById(preleveurId)
  return preleveur ? preleveur.id : null
}
 ...
 */
async function resolvePointsAndSeries({attachmentId, pointIdsStr, preleveurId, parameter, startDate, endDate}) {
  if (attachmentId) {
  ...
  const preleveurObjectId = await resolvePreleveurObjectId(preleveurId)
