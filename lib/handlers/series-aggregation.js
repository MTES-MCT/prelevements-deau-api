...
export async function scopeResolvedPointsForAggregation({
  ...
  const allowPartialInstructorScope = !pointIdsStr && Boolean(preleveurId || collecteurId)
...
function buildDateFilter({startDate, endDate}, params, {overlap = false} = {}) {
  ...
  if (endDate) {
    params.push(endDate)
    filters.push(overlap
      ? `cv."periodStart" < ($${params.length}::date + interval '1 day')`
  ...
  const sql = `
    ...
    distributed_cumulative_values AS (
      SELECT
        ...
        raw_values.value
          * extract(epoch FROM (
            least(raw_values."periodEnd", buckets.bucket_date + interval '1 day')
            - greatest(raw_values."periodStart", buckets.bucket_date)
          ))
      ...
      CROSS JOIN LATERAL generate_series(
        date_trunc('day', raw_values."periodStart"),
        date_trunc('day', raw_values."periodEnd" - interval '1 millisecond'),
        interval '1 day'
...
export async function getExactMeterPeriods({chunkIds, metricTypeCode, startDate, endDate, client = prisma}) {
  ...
  const values = await client.chunkValue.findMany({
    where: {
      ...
      ...(startDate ? {periodEnd: {gt: new Date(startDate)}} : {}),
      ...(endDate ? {periodStart: {lt: new Date(new Date(endDate).getTime() + 86_400_000)}} : {})
    },
    select: {
