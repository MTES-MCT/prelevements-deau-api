...
export async function assertResolvedPointsAccess(user, resolvedPoints, permittedZoneIds = []) {
  ...
  if (user.role === 'INSTRUCTOR') {
    const rows = await prisma.pointPrelevementZone.findMany({
      where: {
  ...
  if (user.role === 'DECLARANT') {
    const rows = await prisma.pointPrelevement.findMany({
      where: {
        id: {in: pointIds},
        OR: [
 ...
 */
async function resolvePointsAndSeries({sourceId, pointIdsStr, preleveurId, collecteurId, metricTypeCode, pointFlowType, startDate, endDate}) {
...
export async function getAggregatedSeriesHandler(req, res) {
  ...
  await assertResolvedPointsAccess(req.user, resolvedPoints, req.permittedZoneIds)
