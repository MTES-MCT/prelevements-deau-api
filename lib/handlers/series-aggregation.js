...
export function extractValuesFromDocument(valueDoc, context) {
  ...
  if (isSubDaily && !useAggregates) {
    ...
    // On utilise l'agrégat journalier correspondant à l'opérateur temporel
    // (l'opérateur temporel s'applique lors de l'agrégation jour→mois/trimestre/année)
    const {dailyAggregates} = valueDoc
    let value = null

    // Mapper l'opérateur temporel vers le champ dailyAggregates correspondant
    // Note: sum n'existe que pour les paramètres cumulatifs
...
function aggregateValuesByDate(valuesBySeriesId, seriesList, aggregationContext) {
  const {operator, aggregationFrequency, temporalOperator} = aggregationContext
 ...
 */
async function resolvePreleveurObjectId(preleveurId, territoire) {
  if (!preleveurId) {
    return null
  }
...
export async function getAggregatedSeriesHandler(req, res) {
  ...
  const aggregatedByPeriod = aggregateValuesByDate(valuesBySeriesId, seriesList, {
    operator: spatialOperator || 'mean', // Fallback neutre : sur valeur unique, tous les opérateurs retournent la même valeur
    parameter,
    aggregationFrequency,
    temporalOperator
  })
