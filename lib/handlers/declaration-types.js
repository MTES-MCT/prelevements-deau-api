...
import {getDeclarantRight} from '../services/resource-permissions.js'
...
function validateUuid(value, label = 'Identifiant', schema = uuidSchema) {
...
export async function listDeclarantDeclarationTypesHandler(req, res) {
  const declarantUserId = validateUuid(req.params.declarantId, 'Identifiant de déclarant', resourceIdSchema.required())
  const right = await getDeclarantRight(req.user, declarantUserId)

  const payload = await listDeclarantDeclarationTypesPayload(declarantUserId, {
    canManage: right.permissions.includes('declarant.declaration-type.update')
  })
...
export async function addDeclarantDeclarationTypeHandler(req, res) {
  ...
  const link = await prisma.declarantDeclarationType.create({
    data: {
      id: randomUUID(),
      declarantUserId,
      declarationTypeId: declarationType.id,
  ...
  stageAuditMutation(req, {
    operation: 'CREATE',
    entityType: 'DECLARANT_DECLARATION_TYPE',
    entityId: link.id,
    entityLabel: declarationType.name,
    after: link
  })
