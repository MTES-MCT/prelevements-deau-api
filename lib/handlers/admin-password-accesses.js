...
const createSchema = Joi.object({
  userId: resourceIdSchema.required()
})

const userIdSchema = resourceIdSchema.required()
...
export async function handlePasswordActivationIssue(req, res, {
  issue = issuePasswordActivation,
  notify = sendPasswordSecurityNotification,
  readFrontUrl = readPasswordActivationFrontUrl
} = {}) {
  const {userId} = validate(createSchema, req.body, 'userId invalide.')
  ensureDifferentUser(req.user.id, userId)
  const result = await issue(userId, {
...
export function issuePasswordActivationHandler(req, res) {
  return handlePasswordActivationIssue(req, res)
}
