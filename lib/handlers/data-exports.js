...
const createDataExportSchema = Joi.object({
  ...
  includeMeterReadings: Joi.boolean().default(false),
  ...
  sandreZoneIds: Joi.array()
    .items(Joi.string().guid({version: 'uuidv4'}).lowercase())
    .unique()
    .default([]),
...
export function validateCreateDataExportPayload(body) {
