 ...
 */
function groupSeriesByParameter(series, useIntegratedDays = true) {
  ...
  for (const [parameterName, parameterSeries] of parameterMap.entries()) {
    ...
    // Fail-fast : si overlap détecté et paramètre incompatible → erreur 400
    if (hasOverlap && config.spatialOperators.length === 0) {
      const allowedOps = config.temporalOperators.join(', ')
      throw createHttpError(
        400,
        `Le paramètre "${parameterName}" ne peut pas être agrégé spatialement car plusieurs séries `
