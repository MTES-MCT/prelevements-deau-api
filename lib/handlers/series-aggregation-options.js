...
export function buildAggregationOptionsPayload({groupedBySeries, resolvedPoints}) {
  ...
  for (const g of groupedBySeries) {
    ...
    const minD = isCumulative ? g._min.periodStart : g._min.periodEnd
...
export async function getAggregatedSeriesOptionsHandler(req, res) {
  ...
  const {resolvedPoints: allResolvedPoints} = await resolvePointsForAggregation({pointIdsStr, preleveurId, collecteurId, sourceId})
  ...
  /**
   * On veut :
   * - minDate / maxDate par metricTypeCode (+unit), selon toute la période
   * - seriesCount = nb de séries distinctes (ici: nb de chunkId distincts) pour ce metric
   *
   * => 1) groupBy (metricTypeCode, unit, chunkId) + min(periodStart)/max(periodEnd)
