...
export function buildAggregationOptionsPayload({groupedBySeries, resolvedPoints, includeExploitationIndexes = false}) {
...
export function buildAggregationOptionGroupsQuery({pointIds = [], sourceId, user, preleveurId, collecteurId, exploitationId}) {
...
export async function listAggregationOptionGroups({
  ...
  return client.$queryRaw(buildAggregationOptionGroupsQuery({pointIds, sourceId, user, preleveurId, collecteurId, exploitationId}))
}
...
export async function getAggregatedSeriesOptionsHandler(req, res) {
  ...
  const {pointIds: pointIdsStr, preleveurId, collecteurId, sourceId, exploitationId} = validated
  ...
  const groupedBySeries = await withRequestPerformancePhase(
    ...
    () => listAggregationOptionGroups({pointIds, sourceId, user: req.user, preleveurId, collecteurId, exploitationId})
  )
  const payload = withRequestPerformancePhase(
    'aggregation_options_serialize',
    () => buildAggregationOptionsPayload({groupedBySeries, resolvedPoints, includeExploitationIndexes: validated.includeExploitationIndexes})
  )
