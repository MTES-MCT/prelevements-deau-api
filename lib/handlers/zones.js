...
async function countZoneDeclarants(zoneId, declarantRole) {
  return prisma.declarant.count({
    where: {
...
async function decorateZoneRight(right) {
  const [
    ...
    countZoneDeclarants(right.zone.id, 'PRELEVEUR'),
    countZoneDeclarants(right.zone.id, 'COLLECTEUR'),
...
export async function addZoneInstructorHandler(req, res) {
  ...
  await prisma.$transaction(async tx => {
    ...
    const before = existingRight
      ? {
        permissions: existingPermissions,
        startDate: existingRight.startDate,
        endDate: existingRight.endDate
      }
      : null
    ...
    await replaceInstructorZonePermissions({
      ...
      action: existingRight ? 'UPDATED' : 'CREATED',
      before
    })
  })
...
export async function removeZoneInstructorHandler(req, res) {
  ...
  await prisma.$transaction(async tx => {
    ...
    await tx.instructorZonePermissionAudit.create({
      data: {
        instructorZoneId: right.id,
        zoneId,
        instructorUserId,
        actorUserId: req.user.id,
        action: 'REMOVED',
