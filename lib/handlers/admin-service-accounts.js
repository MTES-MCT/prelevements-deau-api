...
export const createDeclarantLinkSchema = Joi.object({
  declarantUserId: resourceIdSchema.required(),
...
function serializeServiceAccount(serviceAccount) {
  ...
  return {
    ...
    scope: 'GLOBAL',
    scopeLabel: 'Tous les déclarants',
    ...status,
    credentials,
    declarants,
    counts: {
...
export async function createServiceAccountCredentialHandler(req, res) {
  ...
  stageAuditMutation(req, {
    ...
    entityLabel: credential.name || credential.keyId,
    after: credential
  })
...
export async function addServiceAccountDeclarantHandler(req, res) {
  ...
  stageAuditMutation(req, {
    operation: 'CREATE',
    entityType: 'SERVICE_ACCOUNT_DECLARANT',
    entityId: link.id,
    entityLabel: payload.declarantUserId,
    after: link
  })
