...
test('getUsageDistribution ne filtre pas un usage principal obligatoire contre null', async t => {
  ...
  t.false(Object.hasOwn(exploitationQuery.where, 'usageId'))
  t.true(exploitationQuery.select.pointPrelevementId)
})

for (const declarantUserIds of [null, ['declarant-1']]) {
  test(`getUsageDistribution conserve le périmètre ${declarantUserIds ? 'du déclarant' : 'territorial'}`, async t => {
    let exploitationQuery
    const client = {
      declarantPointPrelevement: {
        async findMany(options) {
    ...
    await getUsageDistribution(['zone-1', 'zone-2'], {client, declarantUserIds})

    t.deepEqual(exploitationQuery.where, {
      ...(declarantUserIds
        ? {
      ...
      pointPrelevement: {
        deletedAt: null,
        zones: {some: {zoneId: {in: ['zone-1', 'zone-2']}}}
      }
    })
    t.true(exploitationQuery.select.pointPrelevementId)
    t.truthy(exploitationQuery.select.usage.include.parent)
    t.truthy(exploitationQuery.select.secondaryUsageLinks.include.usage.include.parent)
  })
}

test('getUsageDistribution sans zone ne charge aucune exploitation', async t => {
  const distribution = await getUsageDistribution([], {
    client: {
      declarantPointPrelevement: {
        async findMany() {
          throw new Error('Aucune exploitation ne doit être chargée sans zone')
        }
      }
    }
  })
...
test('la distribution compte un PP une fois par usage malgré plusieurs exploitations', t => {
  const irrigation = createUsage('irrigation', '2', 'Irrigation')
  const industry = createUsage('industry', '4', 'Industrie')
...
test('les deux parcours comptent les mêmes PP distincts par usage parent, y compris les exploitations terminées', async t => {
  const irrigation = createUsage('irrigation', '2', 'Irrigation')
  const industry = createUsage('industry', '4', 'Industrie')
  const irrigationChild = {
    ...createUsage('irrigation-child', '2.1', 'Sous-usage irrigation'),
    kind: 'SUB_USAGE',
    parentId: irrigation.id,
    parent: irrigation
  }
  const hidden = {...createUsage('hidden', '9', 'Usage masqué'), dashboardVisible: false}
  const hiddenChild = {
    ...createUsage('hidden-child', '9.1', 'Sous-usage masqué'),
    kind: 'SUB_USAGE',
    parentId: hidden.id,
    parent: hidden
  }
      ...
      declarants: [
        {
          ...
          usage: irrigation,
          secondaryUsageLinks: [
            {usage: irrigation}, {usage: irrigationChild},
            {usage: industry}, {usage: industry}, {usage: hidden}
          ]
        },
        {status: 'TERMINEE', usage: irrigationChild, secondaryUsageLinks: [{usage: industry}]}
      ]
    },
    {
      id: 'point-2',
      declarants: [{status: 'TERMINEE', usage: industry, secondaryUsageLinks: []}]
    },
    {
      id: 'point-3',
      declarants: [
        {status: 'TERMINEE', usage: irrigationChild, secondaryUsageLinks: [{usage: irrigation}]}
      ]
    },
    {id: 'point-4', declarants: [{usage: hiddenChild, secondaryUsageLinks: [{usage: hidden}]}]},
    {id: 'point-5', declarants: []}
  ]
  const expectedDistribution = [{usage: irrigation, count: 2}, {usage: industry, count: 2}]
  const exploitations = points.flatMap(point =>
    point.declarants.map(exploitation => ({...exploitation, pointPrelevementId: point.id})))
  const client = {
    declarantPointPrelevement: {
      async findMany(query) {
        t.false(Object.hasOwn(query.where, 'status'))
        t.true(query.select.pointPrelevementId)
  ...
  const dependencies = {
    ...
    async getDistribution(zoneIds, options) {
      return getUsageDistribution(zoneIds, {...options, client})
    },
    async loadPointCorpus() {
      return {points, coordinatesById: new Map()}
    }
  }
  const withPoints = await getDashboardTerritoryPointData({
    zoneIds: ['zone-1'],
    declarantUserIds: null,
    includePoints: true,
    shouldLoadActivityPoints: false
  }, dependencies)
  const withoutPoints = await getDashboardTerritoryPointData({
    zoneIds: ['zone-1'],
    declarantUserIds: null,
    includePoints: false,
    shouldLoadActivityPoints: false
  ...
  t.deepEqual(buildUsageDistributionFromDashboardPoints(points), expectedDistribution)
  t.deepEqual(withPoints.usageDistribution, expectedDistribution)
  t.deepEqual(withoutPoints.usageDistribution, expectedDistribution)
  t.is(withPoints.totalPoints, 5)
  t.is(withoutPoints.totalPoints, 5)
  t.is(withPoints.points.length, 5)
  t.deepEqual(withoutPoints.points, [])
})

test('le corpus cartographique conserve les filtres de zones et de déclarant sans exclure les exploitations terminées', async t => {
  let pointQuery
  const points = await listDashboardPoints(['zone-1'], {
    ...
    async getCoordinatesByPointIds(pointIds) {
      t.deepEqual(pointIds, [])
      return new Map()
    }
  })
  ...
  t.deepEqual(pointQuery.where, {
    deletedAt: null,
    declarants: {some: {declarantUserId: {in: ['declarant-1']}}},
    zones: {some: {zoneId: {in: ['zone-1']}}}
  })
  t.deepEqual(pointQuery.select.declarants.where, {
    declarantUserId: {in: ['declarant-1']},
    declarant: {user: {deletedAt: null}}
  })
  t.true(pointQuery.select.id)
})
