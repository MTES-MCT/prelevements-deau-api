...
test('parseDeclarantsSearchQuery active uniquement le format compact explicite', t => {
  t.is(parseDeclarantsSearchQuery({format: ' COMPACT '}).format, 'compact')
  t.false(Object.hasOwn(parseDeclarantsSearchQuery({format: 'complet'}), 'format'))
})

test('toCompactDeclarantSearchItem conserve seulement les champs utiles à la liste', t => {
  const item = toCompactDeclarantSearchItem({
    id: 'declarant-1',
    email: 'contact@example.test',
    firstName: 'Camille',
    lastName: 'Martin',
    accountCreationMailSentAt: new Date('2026-01-01T00:00:00.000Z'),
    declarant: {
      civility: 'MRS',
      ...
      _count: {pointPrelevements: 3, collecteurExploitations: 1},
      user: {emailAliases: [{email: 'alias@example.test'}]}
    },
    searchSummary: {
  ...
  t.deepEqual(item, {
    id: 'declarant-1',
    email: 'contact@example.test',
    loginEmail: 'contact@example.test',
    contactEmails: [],
    civility: 'MRS',
    firstName: 'Camille',
    lastName: 'Martin',
    ...
    canDisplayActivity: true
  })
})

test('le format compact ne divulgue ni points ni activité à un instructeur sans droits', t => {
  const item = toCompactDeclarantSearchItem({
    id: 'declarant-1',
    declarant: {
      declarantRole: 'PRELEVEUR',
      lastDeclarationAt: new Date('2026-07-01T00:00:00.000Z'),
      _count: {pointPrelevements: 42, collecteurExploitations: 0}
    },
    searchSummary: {usages: [{code: '2', label: 'Irrigation'}]},
    right: {
      canRead: true,
      isAdmin: false,
      permissions: ['declarant.detail.read']
    }
  })

  t.true(item.canReadDetail)
  t.false(item.canDisplayPoints)
  t.false(item.canDisplayActivity)
  t.is(item.pointCount, null)
  t.is(item.usages, null)
  t.is(item.lastDeclarationAt, null)
})

test('toCompactDeclarantSearchItem expose les points du périmètre collecteur explicite', t => {
  const item = toCompactDeclarantSearchItem({
    id: 'preleveur-1',
    declarant: {
      declarantRole: 'PRELEVEUR',
      lastDeclarationAt: new Date('2026-07-01T00:00:00.000Z'),
      _count: {pointPrelevements: 3, collecteurExploitations: 0}
    },
    searchSummary: {usages: [{code: '2', label: 'Irrigation'}]},
    right: {
      canRead: true,
  ...
  t.true(item.canReadDetail)
  t.true(item.canDisplayPoints)
  t.false(item.canDisplayActivity)
  t.is(item.pointCount, 3)
  t.deepEqual(item.usages, [{code: '2', label: 'Irrigation'}])
  t.is(item.lastDeclarationAt, null)
})
