...
test('les index de compteurs sont une option explicite compatible avec les anciens exports', t => {
  t.false(validateCreateDataExportPayload(validPayload()).includeMeterReadings)
  t.true(validateCreateDataExportPayload({...validPayload(), includeMeterReadings: true}).includeMeterReadings)
  t.throws(() => validateCreateDataExportPayload({...validPayload(), includeMeterReadings: 'all'}))
})
