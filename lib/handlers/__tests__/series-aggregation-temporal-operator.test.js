...
import {
  ...
  aggregateSubDailyValuesByPeriod
} from '../series-aggregation.js'

// Tests pour aggregateSubDailyValuesByPeriod

test('aggregateSubDailyValuesByPeriod - agrège plusieurs valeurs de la même période avec sum', t => {
  const values = [
  ...
  const result = aggregateSubDailyValuesByPeriod(values, 'sum')
...
test('aggregateSubDailyValuesByPeriod - agrège plusieurs valeurs de la même période avec max', t => {
  const values = [
    {period: '2024-01-15 10:00', value: 10},
    {period: '2024-01-15 10:00', value: 30},
    {period: '2024-01-15 10:00', value: 20}
  ...
  const result = aggregateSubDailyValuesByPeriod(values, 'max')
...
test('aggregateSubDailyValuesByPeriod - agrège plusieurs valeurs de la même période avec min', t => {
  const values = [
    {period: '2024-01-15 10:00', value: 10},
    {period: '2024-01-15 10:00', value: 30},
    {period: '2024-01-15 10:00', value: 20}
  ...
  const result = aggregateSubDailyValuesByPeriod(values, 'min')
...
test('aggregateSubDailyValuesByPeriod - agrège plusieurs valeurs de la même période avec mean', t => {
  const values = [
    {period: '2024-01-15 10:00', value: 10},
    {period: '2024-01-15 10:00', value: 20},
    {period: '2024-01-15 10:00', value: 30}
  ...
  const result = aggregateSubDailyValuesByPeriod(values, 'mean')
...
test('aggregateSubDailyValuesByPeriod - garde les périodes séparées distinctes', t => {
  const values = [
    {period: '2024-01-15 10:00', value: 10},
    {period: '2024-01-15 10:00', value: 20},
...
test('aggregateSubDailyValuesByPeriod - valeur unique reste inchangée', t => {
  const values = [
...
test('aggregateSubDailyValuesByPeriod - tableau vide retourne tableau vide', t => {
  const result = aggregateSubDailyValuesByPeriod([], 'sum')
  t.deepEqual(result, [])
})

test('aggregateSubDailyValuesByPeriod - null retourne tableau vide', t => {
  const result = aggregateSubDailyValuesByPeriod(null, 'sum')
  t.deepEqual(result, [])
})

test('aggregateSubDailyValuesByPeriod - préserve les remarques', t => {
  const values = [
...
test('aggregateSubDailyValuesByPeriod - déduplique les remarques identiques', t => {
  const values = [
    {period: '2024-01-15 10:00', value: 10, remark: 'Estimation'},
...
// Tests d'intégration : extractValuesFromDocument + aggregateSubDailyValuesByPeriod
...
test('intégration - valeurs infra-journalières agrégées temporellement avec sum', t => {
  ...
  const aggregated = aggregateSubDailyValuesByPeriod(extracted, 'sum')
...
test('intégration - valeurs infra-journalières agrégées temporellement avec max', t => {
  ...
  const extracted = extractValuesFromDocument(valueDoc, context)
  const aggregated = aggregateSubDailyValuesByPeriod(extracted, 'max')
...
test('intégration - valeurs infra-journalières agrégées temporellement avec min', t => {
  ...
  const extracted = extractValuesFromDocument(valueDoc, context)
  const aggregated = aggregateSubDailyValuesByPeriod(extracted, 'min')
...
test('intégration - valeurs infra-journalières agrégées temporellement avec mean', t => {
  ...
  const extracted = extractValuesFromDocument(valueDoc, context)
  const aggregated = aggregateSubDailyValuesByPeriod(extracted, 'mean')
...
test('intégration - opérateurs différents donnent résultats différents', t => {
  ...
  const aggregatedSum = aggregateSubDailyValuesByPeriod(extracted, 'sum')
  const aggregatedMean = aggregateSubDailyValuesByPeriod(extracted, 'mean')
  const aggregatedMin = aggregateSubDailyValuesByPeriod(extracted, 'min')
  const aggregatedMax = aggregateSubDailyValuesByPeriod(extracted, 'max')
