...
test('périodes de campagne exactes accompagnent la projection avec un avertissement explicite', async t => {
  let query
  const exact = await getExactCampaignPeriods({
    ...
    client: {chunkValue: {async findMany(options) {
      query = options
      return [{id: 'value', periodStart: new Date('2025-11-01'), periodEnd: new Date('2026-06-01'), value: 100, unit: 'm³', chunk: {pointPrelevementId: POINT_ID_1, metadata: {campaignId: 'campaign'}}}]
    }}}
  })
  t.deepEqual(query.where.chunkId.in, ['authorized-chunk'])
  t.is(query.where.chunk.calculationStrategy, 'CAMPAIGN')
  t.is(exact[0].value, '100')
  t.is(exact[0].periodStart.toISOString(), '2025-11-01T00:00:00.000Z')
  t.is(exact[0].periodEnd.toISOString(), '2026-06-01T00:00:00.000Z')
  const metadata = buildAggregationMetadata({metricTypeCode: 'volume', aggregationFrequency: '1 day', resolvedPoints: [], campaignVolumesEstimated: true})
  t.true(metadata.campaignVolumesEstimated)
...
for (const {name, startDate, endDate, expectedStart, expectedEnd} of [
  {
    ...
    endDate: '2026-09-07',
    expectedStart: '2023-01-01T00:00:00.000Z',
    expectedEnd: '2026-09-08T00:00:00.000Z'
  },
  {
    name: 'passage à l’heure d’été',
    startDate: '2026-03-29',
    endDate: '2026-03-29',
    expectedStart: '2026-03-29T00:00:00.000Z',
    expectedEnd: '2026-03-30T00:00:00.000Z'
  },
  {
    name: 'passage à l’heure d’hiver',
    startDate: '2026-10-25',
    endDate: '2026-10-25',
    expectedStart: '2026-10-25T00:00:00.000Z',
    expectedEnd: '2026-10-26T00:00:00.000Z'
  },
  {
    name: 'jour bissextile',
    startDate: '2024-02-29',
    endDate: '2024-02-29',
    expectedStart: '2024-02-29T00:00:00.000Z',
    expectedEnd: '2024-03-01T00:00:00.000Z'
  },
  {
    name: 'objets Date existants sans mutation',
    startDate: new Date('2026-01-01T00:00:00.000Z'),
    endDate: new Date('2026-01-31T00:00:00.000Z'),
    expectedStart: '2026-01-01T00:00:00.000Z',
    expectedEnd: '2026-02-01T00:00:00.000Z'
  }
]) {
  test(`périodes de campagne exactes : bornes Prisma UTC pour ${name}`, async t => {
    let query
    ...
    const periods = await getExactCampaignPeriods({
...
test('périodes de campagne exactes : les bornes absentes ne créent pas de filtre', async t => {
  let query
  await getExactCampaignPeriods({
    chunkIds: ['authorized-chunk'],
    metricTypeCode: 'volume',
    client: {chunkValue: {async findMany(options) {
      query = options
