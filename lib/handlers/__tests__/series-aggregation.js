...
test('aggregateValuesByPeriod - 1 quarter avec sum', t => {
  ...
  const result = aggregateValuesByPeriod(dailyValues, '1 quarter', 'sum')
  t.deepEqual(result, [
...
test('aggregateValuesByPeriod - 1 quarter avec mean', t => {
  ...
  const result = aggregateValuesByPeriod(dailyValues, '1 quarter', 'mean')
  t.is(result.length, 2)
...
test('aggregateValuesByPeriod - 1 quarter avec min/max', t => {
  ...
  const minResult = aggregateValuesByPeriod(dailyValues, '1 quarter', 'min')
  const maxResult = aggregateValuesByPeriod(dailyValues, '1 quarter', 'max')
...
test('extractValuesFromDocument - série infra-journalière avec dailyAggregates sum', t => {
  const valueDoc = {
    date: '2024-01-15',
    dailyAggregates: {
      sum: 100,
