...
import {unavailablePublicVisitors} from '../../services/public-visitors.js'

const NOW = new Date('2026-09-09T12:00:00Z')

function createApp(loadStats, loadVisitors = async () => unavailablePublicVisitors(NOW)) {
  const app = express()
  const router = createRoutes({
    publicStatsHandler: createPublicStatsHandler({loadStats, loadVisitors, now: () => NOW})
  })
...
test('les statistiques publiques sont accessibles sans session et avec un ancien token', async t => {
  ...
  t.deepEqual(responses[0].body, {...snapshot, publicVisitors: unavailablePublicVisitors(NOW)})
...
test('Matomo reste indépendant du mois métier et une panne ne masque pas les autres statistiques', async t => {
  const visitorQueries = []
  const app = createApp(async ({month}) => ({month, totals: {preleveursCount: 4}}), async query => {
    visitorQueries.push(query)
    throw new Error('Private upstream response must not be exposed')
  })
  const response = await request(app).get('/stats/public?month=2026-05')
  t.is(response.status, 200)
  t.is(response.body.month, '2026-05')
  t.is(response.body.totals.preleveursCount, 4)
  t.deepEqual(response.body.publicVisitors, unavailablePublicVisitors(NOW))
  t.deepEqual(visitorQueries, [{now: NOW}])
  t.false(response.text.includes('Private upstream'))
})
