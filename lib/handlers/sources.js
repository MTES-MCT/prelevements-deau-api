...
export async function listMySourcesHandler(req, res, next) {
  try {
    ...
    const data = req.user.role === 'ADMIN'
      ...
      : await listSourcesForInstructor({
        ...query,
        zoneIds: req.permittedZoneIds
      })
...
export async function getMySourceHandler(req, res, next) {
  try {
    ...
    if (req.user.role === 'ADMIN') {
      item = await getSourceForAdmin(sourceId)
    } else {
      const [readZoneIds, instructZoneIds, reconcileZoneIds] = await Promise.all([
      ...
      const canDownloadFiles = await hasZonePermission(
        req.user,
        'declaration.file.download',
        req.permittedZoneIds
      )

      item = await getSourceForInstructor(sourceId, {
        readZoneIds,
        instructZoneIds,
        reconcileZoneIds,
        canInstructUnmatched: await hasZonePermission(
          req.user,
          'declaration.instruct',
          req.permittedZoneIds
        ),
        canReconcileUnmatched: await hasZonePermission(
          req.user,
