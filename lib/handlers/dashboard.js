...
export async function resolveDashboardZoneSelection(user, requestedZoneCodesValue) {
  const declarantUserIds = await getDashboardDeclarantUserIds(user)
  const zoneRights = await getAccessibleZoneRights(user, declarantUserIds)
...
function getDashboardPointWhere(zoneIds, declarantUserIds) {
  return {
    deletedAt: null,
    ...getDeclarantPointWhere(declarantUserIds),
    ...(zoneIds.length > 0
...
export async function buildDashboardMapPayload({
  ...
  getDeclarantUserIds = getDashboardDeclarantUserIds,
  listPoints = listDashboardPoints,
  ...
  if (mapScope === 'activity') {
    ...
    const declarantUserIds = await getDeclarantUserIds(user)
  ...
  const selection = await resolveZoneSelection(user, requestedZoneCodes)
  const zoneIds = selection.selectedZones.map(zone => zone.id)
  const points = await listPoints(zoneIds, {
    declarantUserIds: selection.declarantUserIds
  })
