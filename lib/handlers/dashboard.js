...
async function listDashboardPoints(zoneIds, {declarantUserIds = null} = {}) {
  if (zoneIds.length === 0) {
    return []
  }
...
export async function getDashboardTerritoryHandler(req, res) {
  ...
  const [points, usageDistribution, periodOptions, volumeYearOptions] = await Promise.all([
    ...
    perf.time('declaration-period-options', () =>
      getDeclarationPeriodOptions(zoneIds, selectedPeriodType, {declarantUserIds})),
