...
const EXACT_METER_INTERVAL_SQL = Prisma.sql`(c."calculationStrategy" = 'METER' AND v.frequency = 'irregular')`
const parisCalendarDate = new Intl.DateTimeFormat('en-CA', {
  timeZone: 'Europe/Paris', year: 'numeric', month: '2-digit', day: '2-digit'
})
const parisCalendarHour = new Intl.DateTimeFormat('en-GB', {
  timeZone: 'Europe/Paris', hour: '2-digit', hourCycle: 'h23'
})

function parisPeriodStart(date) {
  // Paris changes offset after midnight; UTC midnight is still on the same
  // Paris calendar day and gives the offset of that day's local midnight.
  return new Date(date.getTime() - Number(parisCalendarHour.format(date)) * 3600000)
}

// ChunkValue timestamps contain UTC instants. Meter intervals are half-open and
// apportioned by elapsed seconds over Paris calendar months, including DST.
// Ordinary sources keep their historical attribution to periodStart's month.
export function buildDashboardVolumeMonthProjection(selectedYear) {
  return Prisma.sql`
    CROSS JOIN LATERAL (
      SELECT EXTRACT(MONTH FROM v."periodStart")::int AS month, v.value AS volume
      WHERE NOT ${EXACT_METER_INTERVAL_SQL}
        AND v."periodStart" >= ${`${selectedYear}-01-01`}::date
        AND v."periodStart" < ${`${selectedYear + 1}-01-01`}::date
      UNION ALL
      SELECT EXTRACT(MONTH FROM calendar.month)::int AS month,
        v.value * EXTRACT(EPOCH FROM (
          LEAST(v."periodEnd", bounds."end") - GREATEST(v."periodStart", bounds.start)
        )) / NULLIF(EXTRACT(EPOCH FROM (v."periodEnd" - v."periodStart")), 0) AS volume
      FROM generate_series(${`${selectedYear}-01-01`}::timestamp, ${`${selectedYear}-12-01`}::timestamp, INTERVAL '1 month') calendar(month)
      CROSS JOIN LATERAL (
        SELECT (calendar.month AT TIME ZONE 'Europe/Paris') AT TIME ZONE 'UTC' AS start,
          ((calendar.month + INTERVAL '1 month') AT TIME ZONE 'Europe/Paris') AT TIME ZONE 'UTC' AS "end"
      ) bounds
      WHERE ${EXACT_METER_INTERVAL_SQL}
        AND v."periodEnd" > v."periodStart"
        AND v."periodStart" < bounds."end" AND v."periodEnd" > bounds.start
    ) volume_period
  `
}
...
function buildUsageDistributionFromExploitations(exploitations) {
  ...
  for (const exploitation of exploitations) {
    for (const usage of getExploitationWaterUses(exploitation)) {
      addPointToUsage(pointIdsByUsageId, serializedUsagesById, usage, exploitation.pointPrelevementId)
    }
  }
...
export function buildUsageDistributionFromDashboardPoints(points) {
  return buildUsageDistributionFromExploitations(
    points.flatMap(point => (point.declarants ?? []).map(exploitation => ({
      ...exploitation,
...
export async function getDeclarationPeriodOptions(
  ...
  const chunks = await client.chunk.findMany({
    ...
    select: {
      ...
      chunkValues: {
        where: {frequency: 'irregular', chunk: {calculationStrategy: 'METER'}},
  ...
  for (const chunk of chunks) {
    ...
    let minDate = chunk.minDate
    let maxDate = chunk.maxDate
    if (chunk.calculationStrategy === 'METER') {
      const intervals = (chunk.chunkValues ?? []).filter(value => value.periodEnd > value.periodStart)
      if (!intervals.length) continue
      minDate = new Date(parisCalendarDate.format(new Date(Math.min(...intervals.map(value => value.periodStart.getTime())))))
      maxDate = new Date(parisCalendarDate.format(new Date(Math.max(...intervals.map(value => value.periodEnd.getTime())) - 1)))
    }
    for (const periodKey of getDeclarationPeriodKeysBetween(periodType, minDate, maxDate)) {
...
export async function getRegisteredPrelevementsByUsage(
  ...
  const chunks = await client.chunk.findMany({
    where: {
      ...
      AND: [{OR: [
        {calculationStrategy: {not: 'METER'}, minDate: {lt: nextPeriodStart}, maxDate: {gte: periodStart}},
        {calculationStrategy: 'METER', chunkValues: {some: {
          frequency: 'irregular', metricTypeCode: {in: VOLUME_METRIC_TYPE_CODES},
          periodStart: {lt: parisPeriodStart(nextPeriodStart)}, periodEnd: {gt: parisPeriodStart(periodStart)}
        }}}
      ]}],
      instructionStatus: {
        not: 'REJECTED'
...
export async function getVolumeYearOptions(zoneIds, waterBodyTypes, {client = prisma} = {}) {
  ...
  const rows = await client.$queryRaw(Prisma.sql`
    SELECT DISTINCT available_year.year
    FROM "ChunkValue" v
    ...
    CROSS JOIN LATERAL generate_series(
      EXTRACT(YEAR FROM CASE WHEN ${EXACT_METER_INTERVAL_SQL}
        THEN v."periodStart" AT TIME ZONE 'UTC' AT TIME ZONE 'Europe/Paris' ELSE v."periodStart" END)::int,
      EXTRACT(YEAR FROM CASE WHEN ${EXACT_METER_INTERVAL_SQL}
        THEN GREATEST(v."periodStart", v."periodEnd" - INTERVAL '1 microsecond') AT TIME ZONE 'UTC' AT TIME ZONE 'Europe/Paris'
        ELSE v."periodStart" END)::int
    ) available_year(year)
    WHERE v."metricTypeCode" IN (${Prisma.join(VOLUME_METRIC_TYPE_CODES)})
      AND v.value <> 0
      AND available_year.year <= ${currentYear}
      AND (NOT ${EXACT_METER_INTERVAL_SQL} OR v."periodEnd" > v."periodStart")
  ...
  const [rows, expectedUsages] = await Promise.all([
    client.$queryRaw(Prisma.sql`
      SELECT
        ...
        SUM(volume_period.volume)::float8 AS volume
      FROM "ChunkValue" v
      JOIN "Chunk" c ON c.id = v."chunkId"
      JOIN "Source" s ON s.id = c."sourceId"
      JOIN "PointPrelevement" p ON p.id = c."pointPrelevementId"
      JOIN "SandreWaterUse" usage ON usage.id = c."usageId"
      LEFT JOIN "SandreWaterUse" parent_usage ON parent_usage.id = usage."parentId"
      ${buildDashboardVolumeMonthProjection(selectedYear)}
      WHERE v."metricTypeCode" IN (${Prisma.join(VOLUME_METRIC_TYPE_CODES)})
        AND (
          (NOT ${EXACT_METER_INTERVAL_SQL}
            AND v."periodStart" >= ${`${selectedYear}-01-01`}::date
            AND v."periodStart" < ${`${selectedYear + 1}-01-01`}::date)
          OR (${EXACT_METER_INTERVAL_SQL}
            AND v."periodStart" < ((${`${selectedYear + 1}-01-01`}::timestamp AT TIME ZONE 'Europe/Paris') AT TIME ZONE 'UTC')
            AND v."periodEnd" > ((${`${selectedYear}-01-01`}::timestamp AT TIME ZONE 'Europe/Paris') AT TIME ZONE 'UTC'))
        )
        AND c."pointPrelevementId" IS NOT NULL
        AND c."instructionStatus" <> 'REJECTED'
        ${COMPLETED_DASHBOARD_SOURCE_SQL}
