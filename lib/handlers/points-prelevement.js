...
export async function listPointMapSummaries(req, res) {
  ...
  switch (req.userRole) {
    case 'ADMIN': {
      points = await getPointMapSummaries()
      break
    }

    case 'INSTRUCTOR': {
      const [mapPoints, detailZoneIds, declarantZoneIds, exploitationZoneIds] = await Promise.all([
    ...
    case 'DECLARANT': {
      points = await getPointMapSummariesByDeclarant(req.user.id)
      break
    }
    // No default
  }

  res.send(serializePointMapSummaries(points, {
