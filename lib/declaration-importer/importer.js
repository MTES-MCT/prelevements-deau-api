...
import {extractAquasys, extractGidaf, extractTemplateFile} from '@fabnum/prelevements-deau-timeseries-parsers'
...
export async function processDeclaration(declarationId, logger = createLogger()) {
  ...
  const {errors, data} = await extractSeriesFromDeclarationFiles(declaration, logger)
  ...
  await createChunksFromData(data, newSource.id, declarantUserId, declaration.autoValidationEnabled, logger)
}
...
async function extractSeriesFromDeclarationFiles(declaration, logger) {
  switch (declaration.type) {
    case 'template-file': {
      const templateFile = declaration.files.find(f => f.type === 'template-file')
      if (!templateFile) {
        logger.error('Aucun fichier template-file à traiter')
        Sentry.captureException(new Error(`Déclaration ${declaration.id} : aucun fichier template-file trouvé`))
        return {errors: [], data: null}
      }

      const buffer = await downloadFileFromStorage(templateFile.storageKey, logger)
      return extractSeriesFromTemplateFile(buffer, logger)
    }

    case 'aquasys': {
      const aquasysFile = declaration.files.find(f => f.type === 'aquasys')
      if (!aquasysFile) {
        logger.error('Aucun fichier aquasys à traiter')
        Sentry.captureException(new Error(`Déclaration ${declaration.id} : aucun fichier aquasys trouvé`))
        return {errors: [], data: null}
      }

      const buffer = await downloadFileFromStorage(aquasysFile.storageKey, logger)
      return extractSeriesFromAquasysFile(buffer, logger)
    }

    case 'gidaf': {
      const cadresFile
        = declaration.files.find(f => f.type === 'gidaf-cadres')
          ?? declaration.files.find(f => /cadres/i.test(f.filename))
          ?? declaration.files.find(f => /cadre/i.test(f.filename))
          ?? null

      const prelevementsFile
        = declaration.files.find(f => f.type === 'gidaf-prelevements')
          ?? declaration.files.find(f => /prelevements/i.test(f.filename))
          ?? declaration.files.find(f => /consommations/i.test(f.filename))
          ?? null

      if (!cadresFile) {
        logger.error('Impossible de déterminer le fichier gidaf Cadres')
        Sentry.captureException(new Error(`Déclaration ${declaration.id} : fichier gidaf Cadres introuvable`))
        return {errors: [], data: null}
      }

      if (!prelevementsFile) {
        logger.error('Impossible de déterminer le fichier gidaf Prelevements')
        Sentry.captureException(new Error(`Déclaration ${declaration.id} : fichier gidaf Prelevements introuvable`))
        return {errors: [], data: null}
      }

      const cadresBuffer = await downloadFileFromStorage(cadresFile.storageKey, logger)
      const prelevementsBuffer = await downloadFileFromStorage(prelevementsFile.storageKey, logger)

      return extractSeriesFromGidafFiles(cadresBuffer, prelevementsBuffer, logger)
    }
...
async function extractSeriesFromAquasysFile(buffer, logger) {
  logger.log('Extraction aquasys en cours...')
  try {
    const result = await extractAquasys(buffer)
    logger.log(
      `Extraction terminée: ${result.errors?.length ?? 0} erreur(s), données: ${result.data ? 'oui' : 'non'}`
    )
    return {errors: result.errors ?? [], data: result.data}
  } catch (error) {
    logger.error(`Erreur extractAquasys: ${error.message}`)
    console.error('[process-declaration] extractAquasys:', error)
    Sentry.captureException(error)
    throw error
  }
}

async function extractSeriesFromGidafFiles(cadresBuffer, prelevementsBuffer, logger) {
  logger.log('Extraction gidaf en cours...')
  try {
    const result = await extractGidaf(cadresBuffer, prelevementsBuffer)
    logger.log(
      `Extraction terminée: ${result.errors?.length ?? 0} erreur(s), données: ${result.data ? 'oui' : 'non'}`
    )
    return {errors: result.errors ?? [], data: result.data}
  } catch (error) {
    logger.error(`Erreur extractGidaf: ${error.message}`)
    console.error('[process-declaration] extractGidaf:', error)
    Sentry.captureException(error)
    throw error
  }
}
