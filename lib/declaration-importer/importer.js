...
async function createChunksFromData(data, sourceId, declarantUserId, autoValidationEnabled, logger) {
  ...
  for (const chunk of data.series) {
    ...
    logger.log([
      `Chunk: sourcePointId="${sourcePointId ?? 'n/a'}"`,
    ...
    const pointPrelevement = await resolvePointPrelevement(sourcePointId, pointPrelevementName)
    ...
    async tx => tx.source.create({
      data: {
        ...
        metadata: {
          ...
          sourcePointId: apiDeclaration.sourcePointId ?? apiDeclaration.pointId ?? apiDeclaration.metadata?.point_id ?? null,
...
function normalizeApiDeclaration(apiDeclaration) {
  ...
  const series = (apiDeclaration.data?.metrics ?? []).map(metric => ({
    ...
    sourcePointId: pointPrelevementId,
...
async function resolvePointPrelevement(sourcePointId, pointPrelevementName) {
  if (sourcePointId) {
