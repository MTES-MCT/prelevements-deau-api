...
const DECLARATION_INGEST_TRANSACTION_OPTIONS = {
  maxWait: 10_000,
  timeout: 60_000
}
...
async function getCollecteurPointAccess({
  ...
  const linkOnWindow = await client.declarantCollecteurExploitation.findFirst({
    where: {
      collecteurUserId,
      exploitation: {
        pointPrelevementId: pointId,
...
function computeTotalWaterVolumeForSeries(series, parameterName) {
  return series.reduce((sum, item) => sum + computeTotalWaterVolume(item, parameterName), 0)
}
...
export async function ingestDeclarationSeries({
  ...
  const source = await prisma.$transaction(async tx => {
    ...
    return source
  }, DECLARATION_INGEST_TRANSACTION_OPTIONS)
