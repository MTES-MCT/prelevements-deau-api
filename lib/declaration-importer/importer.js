...
import {resolveTemporalPeriod} from '../util/temporal-discretization.js'
import {
...
import {
  ...
  normalizeCountingCode,
  selectUniqueExploitation
} from '../services/exploitation-periods.js'

const PLACEHOLDER_WATER_USE_CODES = new Set(['0', '1'])
...
function withClientTransaction(client, callback) {
...
function chooseExploitationCandidate(exploitations, externalDeclarant = null, scope = {}) {
  if (exploitations.length === 0) {
    return null
  }

  if (!externalDeclarant) {
    return selectUniqueExploitation(exploitations, scope)
  }
  ...
  if (matchingExploitations.length > 0) {
    return selectUniqueExploitation(matchingExploitations, scope)
  }
...
export async function findAccessibleExploitationForPointName({
  client = prisma,
  declarantUserId,
  declarantRole,
  ...
  const exploitations = await client.declarantPointPrelevement.findMany({
    where: {
      ...accessWhere,
      ...(exploitationId ? {id: exploitationId} : {}),
      ...(countingCode ? {countingCode} : {}),
      ...exploitationOverlapsPeriodWhere(chunkStart, chunkEnd),
      pointPrelevement: {
        deletedAt: null
      }
    },
  ...
  // Un nom exact ou un alias explicite doit primer sur une correspondance
  // partielle ; cette dernière n'autorise jamais à choisir le premier résultat.
  const exactMatches = matches.filter(match => match.matchStrategy !== 'PointPrelevement.name contains + DeclarantPointPrelevement')
  const candidates = exactMatches.length > 0 ? exactMatches : matches

  const selected = chooseExploitationCandidate(
    candidates.map(match => match.exploitation),
    externalDeclarant,
    {declarantUserId}
  )
  if (!selected) {
...
export async function getDirectPointAccess({
  client = prisma,
  declarantUserId,
  pointId,
  exploitationId,
  ...
  const linkCount = await client.declarantPointPrelevement.count({
    where: {
      declarantUserId,
  ...
  const linkOnWindow = await findUniqueExploitationForPeriod({
    client,
    declarantUserId,
    pointPrelevementId: pointId,
    exploitationId,
    ...
    select: {id: true, countingCode: true, declarantUserId: true, usageId: true, usage: true}
  })
...
export async function getCollecteurPointAccess({
  client = prisma,
  collecteurUserId,
  pointId,
  externalDeclarant,
  ...
  const linkCount = await client.declarantCollecteurExploitation.count({
    where: {
      collecteurUserId,
      exploitation: {
        pointPrelevementId: pointId
  ...
  return {
    ...
    linkOnWindow: selectedLinkOnWindow?.exploitation
      ? {
        ...
        countingCode: selectedLinkOnWindow.exploitation.countingCode,
...
export async function findUniquePointByDeclaredName({client = prisma, pointPrelevementName}) {
  if (!normalizeDeclaredPointName(pointPrelevementName)) return null
  const exact = await client.pointPrelevement.findMany({
    where: {name: pointPrelevementName, deletedAt: null},
    select: {id: true, flowType: true},
    take: 2
  })
  const matches = exact.length > 0 ? exact : await client.pointPrelevement.findMany({
    where: {name: {contains: pointPrelevementName, mode: 'insensitive'}, deletedAt: null},
    select: {id: true, flowType: true},
    take: 2
  })
  if (matches.length > 1) {
    throw createHttpError(409, 'Plusieurs points correspondent au nom déclaré.')
  }
  return matches[0] ?? null
}

export async function resolveDeclarationChunkPoint({
  client = prisma, declarantUserId, declarantRole, pointPrelevementName,
  externalDeclarant, exploitationId, countingCode, chunkStart, chunkEnd
}) {
  const unresolved = {
    pointPrelevementId: null, matchedExploitation: null,
    matchedDeclarantUserId: null, candidatePointFlowType: null
  }
  const scope = {client, exploitationId, countingCode, chunkStart, chunkEnd}
  const diagnostics = {
    pointPrelevementName,
    exploitationId: exploitationId ?? null,
    countingCode: countingCode ?? null,
    window: {from: chunkStart.toISOString(), to: chunkEnd.toISOString()}
  }

  try {
    const accessible = await findAccessibleExploitationForPointName({
      ...scope, declarantUserId, declarantRole, pointPrelevementName, externalDeclarant
    })
    if (accessible) {
      return {
        pointPrelevementId: accessible.pointPrelevementId,
        matchedExploitation: accessible.exploitation,
        matchedDeclarantUserId: accessible.exploitation.declarantUserId,
        candidatePointFlowType: accessible.exploitation.pointPrelevement.flowType,
        parsingInfo: {
          ...diagnostics, case: 1, reason: 'POINT_FOUND_AND_LINK_ACTIVE_ON_WINDOW',
          pointPrelevementId: accessible.pointPrelevementId,
          exploitationId: accessible.exploitation.id, matchStrategy: accessible.matchStrategy
        }
      }
    }

    const point = await findUniquePointByDeclaredName({client, pointPrelevementName})
    if (!point) {
      return {...unresolved, parsingInfo: {...diagnostics, case: 4, reason: 'POINT_NOT_FOUND'}}
    }

    const direct = await getDirectPointAccess({...scope, declarantUserId, pointId: point.id})
    let access = direct
    if (!direct.linkOnWindow && declarantRole === 'COLLECTEUR') {
      const delegated = await getCollecteurPointAccess({
        ...scope, collecteurUserId: declarantUserId, pointId: point.id, externalDeclarant
      })
      if (delegated.linkOnWindow || (!direct.hasAnyLink && delegated.hasAnyLink)) access = delegated
    }

    if (access.linkOnWindow) {
      return {
        pointPrelevementId: point.id,
        matchedExploitation: access.linkOnWindow,
        matchedDeclarantUserId: access.linkOnWindow.declarantUserId,
        candidatePointFlowType: point.flowType,
        parsingInfo: {
          ...diagnostics, case: 1, reason: 'POINT_FOUND_AND_LINK_ACTIVE_ON_WINDOW',
          pointPrelevementId: point.id, exploitationId: access.linkOnWindow.id,
          matchStrategy: access.matchStrategy
        }
      }
    }
    ...
    return {
      ...
      candidatePointFlowType: point.flowType,
      parsingInfo: {
        ...diagnostics, pointPrelevementId: point.id, declarantUserId,
        case: access.hasAnyLink ? 2 : 3,
        reason: access.hasAnyLink
          ? (countingCode || exploitationId ? 'EXPLOITATION_IDENTITY_NOT_FOUND_ON_WINDOW' : 'POINT_FOUND_BUT_LINK_OUTSIDE_WINDOW')
          : 'POINT_FOUND_BUT_NO_LINK_FOR_DECLARANT',
        matchStrategy: access.matchStrategy
      }
    }
  } catch (error) {
    if (error?.statusCode !== 409) throw error
    // Une ambiguïté conserve les observations à rapprocher. Les autres
    // lignes du fichier restent importables ; aucune identité n'est devinée.
    return {
      ...unresolved,
      parsingInfo: {
        ...diagnostics, case: 6, reason: 'EXPLOITATION_IDENTITY_UNRESOLVED', message: error.message
      }
    }
  }
}
...
function getSeriesGroupKey(series) {
  ...
  return JSON.stringify({
    ...
    exploitationId: series.exploitationId ?? null,
    countingCode: series.countingCode ?? null,
    externalDeclarant: getExternalDeclarantKey(externalDeclarant),
    flowType: getSeriesFlowType(series),
    usageId: series.usageId ?? null,
    usage: series.usage ?? null
  })
}

function getSeriesFlowType(series) {
  const explicitFlowType = normalizePointFlowType(
    series.flowType
    ?? series.flow_type
    ?? getObjectMetadata(series.metadata).flowType
    ?? getObjectMetadata(series.metadata).flow_type
  )
  const legacyFlowType = inferFlowTypeFromLegacyMetricTypeCode(series.parameter)

  if (explicitFlowType && legacyFlowType && explicitFlowType !== legacyFlowType) {
    throw new Error(
      `Le type de point ${explicitFlowType} contredit le paramètre historique ${series.parameter}.`
    )
  }

  return explicitFlowType ?? legacyFlowType
}

function normalizeIncomingSeries(series) {
  if (series.exploitationId !== undefined && series.exploitationId !== null
    && (typeof series.exploitationId !== 'string' || !/^[\da-f]{8}(?:-[\da-f]{4}){3}-[\da-f]{12}$/i.test(series.exploitationId))) {
    throw createHttpError(400, 'L’identifiant de l’exploitation doit être un UUID valide.')
  }
  return {
    ...series,
    exploitationId: series.exploitationId ?? undefined,
    countingCode: normalizeCountingCode(series.countingCode) ?? undefined,
    metadata: {
      ...getObjectMetadata(series.metadata),
      ...(series.countingCode !== undefined && {rawCountingCode: series.countingCode})
    },
    parameter: normalizeMetricTypeCode(series.parameter),
    flowType: getSeriesFlowType(series)
  }
}

export function groupSeriesIntoChunks(series) {
  const groupsByKey = new Map()

  for (const rawItem of series) {
    const item = normalizeIncomingSeries(rawItem)
    ...
    if (!existingGroup) {
      groupsByKey.set(key, {
        ...
        countingCode: item.countingCode,
...
export function shouldUpdateExploitationWaterUse(waterUse, matchedExploitation) {
  if (!waterUse || !matchedExploitation?.id || PLACEHOLDER_WATER_USE_CODES.has(waterUse.code)) {
    return false
  }

  const rootUsageId = getWaterUseRootId(waterUse)

  return Boolean(rootUsageId && matchedExploitation.usageId !== rootUsageId)
}
...
async function createChunksFromData(
  ...
  client = prisma,
  {
    deferredExploitationUsageIds = null,
  ...
  for (const chunk of chunkCandidates) {
    ...
    let {
      pointPrelevementId,
      parsingInfo,
      matchedDeclarantUserId,
      matchedExploitation,
      candidatePointFlowType
    } = await resolveDeclarationChunkPoint({
      client,
      declarantUserId,
      declarantRole,
      pointPrelevementName,
      ...
      countingCode: chunk.countingCode,
      chunkStart,
      chunkEnd
    })
    if (pointPrelevementId) matchedPoints++
    else unmatchedPoints++

    const sourceFlowType = chunk.flowType
    const matchedPointFlowType = candidatePointFlowType

    if (
      pointPrelevementId
      && sourceFlowType
      && matchedPointFlowType
      && sourceFlowType !== matchedPointFlowType
    ) {
      const conflictingPointPrelevementId = pointPrelevementId
      pointPrelevementId = null
      matchedExploitation = null
      matchedDeclarantUserId = null
      matchedPoints--
      unmatchedPoints++
      parsingInfo = {
        case: 5,
        ...
        pointPrelevementName,
        pointPrelevementId: conflictingPointPrelevementId,
        sourceFlowType,
        pointFlowType: matchedPointFlowType
      }
      logger.warn(
        `Type de point incompatible: point="${pointPrelevementName}", source=${sourceFlowType}, point=${matchedPointFlowType}`
      )
    }

    const flowType = sourceFlowType ?? matchedPointFlowType
    const totalWaterVolume = computeTotalWaterVolumeForSeries(chunk.series, METRIC_TYPE_CODES.VOLUME)
    const totalWaterVolumeWithdrawn = flowType === POINT_FLOW_TYPES.PRELEVEMENT ? totalWaterVolume : 0
    const totalWaterVolumeDischarged = flowType === POINT_FLOW_TYPES.REJET ? totalWaterVolume : 0
    ...
    if (shouldUpdateExploitationWaterUse(waterUse, matchedExploitation)) {
      const rootUsageId = getWaterUseRootId(waterUse)
      const usageIds = deferredExploitationUsageIds?.get(matchedExploitation.id) ?? new Set()

      usageIds.add(rootUsageId)

      if (deferredExploitationUsageIds) {
        deferredExploitationUsageIds.set(matchedExploitation.id, usageIds)
      } else {
        await addExploitationSecondaryUsage(matchedExploitation.id, rootUsageId, {client})
      }
    }
    ...
    const newChunk = await client.chunk.create({
      data: {
        ...
        pointPrelevementName,
        pointPrelevementId,
        exploitationId: pointPrelevementId ? matchedExploitation?.id ?? null : null,
        flowType,
        ...actorData,
        ...
        metadata: {
          ...
          declaredExploitationId: chunk.exploitationId ?? null,
          countingCode: chunk.countingCode ?? null,
          externalDeclarant,
          sourceFlowType,
          flowType,
          totalWaterVolume,
    ...
    const valueRows = rawValues
      .map(({datum, parameter, unit, frequency}) => {
        ...
        const {periodEnd, periodStart, error: periodError} = resolveTemporalPeriod(datum, frequency)
    ...
    if (hasFilteredValues) {
      const filteredTotalWaterVolume = computeTotalWaterVolumeForValueRows(
        valueRowsToInsert,
        METRIC_TYPE_CODES.VOLUME
      )
      const filteredTotalWaterVolumeWithdrawn = flowType === POINT_FLOW_TYPES.PRELEVEMENT
        ? filteredTotalWaterVolume
        : 0
      const filteredTotalWaterVolumeDischarged = flowType === POINT_FLOW_TYPES.REJET
        ? filteredTotalWaterVolume
        : 0
      ...
      await client.chunk.update({
        ...
        data: {
          ...
          parsingInfo: {
            ...getObjectMetadata(newChunk.parsingInfo),
          ...
          metadata: {
            ...
            totalWaterVolume: filteredTotalWaterVolume,
    ...
    const insertedTotalWaterVolume = computeTotalWaterVolumeForValueRows(
      valueRowsToInsert,
      METRIC_TYPE_CODES.VOLUME
    )
    const insertedTotalWaterVolumeWithdrawn = flowType === POINT_FLOW_TYPES.PRELEVEMENT
      ? insertedTotalWaterVolume
      : 0
    const insertedTotalWaterVolumeDischarged = flowType === POINT_FLOW_TYPES.REJET
      ? insertedTotalWaterVolume
      : 0
...
export async function completeDeclarationSourceIngestion({
  sourceId,
  ...
  return withClientTransaction(client, async tx => {
    ...
    await refreshSourceDeclarantsLastDeclarationAt(sourceId, {client: tx})
...
export async function ingestDeclarationSeries({
  ...
  const deferredExploitationUsageIds = new Map()
  ...
  try {
    await createChunksFromData(
      ...
      prisma,
      {
        deferredExploitationUsageIds,
        deferredReplacedSourceIds
      }
    )

    await prisma.$transaction(async tx => {
      for (const [exploitationId, usageIds] of [...deferredExploitationUsageIds.entries()]
