...
function chooseExploitationCandidate(exploitations, externalDeclarant = null, {allowAmbiguousDefault = true} = {}) {
  if (exploitations.length === 0) {
    return null
  }

  if (!externalDeclarant) {
    return allowAmbiguousDefault || exploitations.length === 1 ? exploitations[0] : null
  }
...
async function findAccessibleExploitationForPointName({
  ...
  const selected = chooseExploitationCandidate(matches.map(match => match.exploitation), externalDeclarant, {
    allowAmbiguousDefault: declarantRole !== 'COLLECTEUR'
  })
  if (!selected) {
    return null
  }
