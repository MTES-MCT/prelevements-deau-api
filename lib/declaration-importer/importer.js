...
import {ChunkGranularity} from '@prisma/client'
import * as Sentry from '@sentry/node'
import moment from 'moment'
import {randomUUID} from 'node:crypto'
import {computeGlobalInstructionStatus} from '../handlers/chunks.js'

const CHUNK_GRANULARITY_FROM_PAYLOAD = new Map([
  ['15_minutes', ChunkGranularity.FIFTEEN_MINUTES],
  ['1 hour', ChunkGranularity.ONE_HOUR],
  ['1 day', ChunkGranularity.ONE_DAY],
  ['1 week', ChunkGranularity.ONE_WEEK],
  ['1 month', ChunkGranularity.ONE_MONTH],
  ['1 year', ChunkGranularity.ONE_YEAR]
])

function toChunkGranularity(raw, logger) {
  if (raw === null || raw === undefined || raw === '') {
    return undefined
  }

  const value = CHUNK_GRANULARITY_FROM_PAYLOAD.get(String(raw))
  if (!value) {
    logger.warn(`Granularité inconnue, ignorée côté enum Chunk: "${raw}"`)
    return undefined
  }
...
async function createChunksFromData(data, sourceId, declarantUserId, autoValidationEnabled, logger) {
  ...
  for (const chunk of data.series) {
    ...
    const {
      ...
      minDate,
      maxDate,
      pointId: chunkPointId
    } = chunk

    logger.log([
      `Chunk: pointId="${chunkPointId ?? 'n/a'}"`,
    ...
    const pointPrelevement = await resolvePointPrelevement(chunkPointId, pointPrelevementName)

    let pointPrelevementId = null
    let parsingInfo = null

    if (pointPrelevement?.id) {
      const linkedPointId = pointPrelevement.id

      const anyLink = await prisma.declarantPointPrelevement.findFirst({
        where: {
          declarantUserId,
          pointPrelevementId: linkedPointId
        },
        select: {id: true}
      })
      ...
      if (linkOnWindow?.id) {
        ...
        logger.warn(
          `Point trouvé mais lien déclarant hors période chunk: name="${pointPrelevementName}" -> id=${linkedPointId}`
        )
      } else {
        unmatchedPoints++
        parsingInfo = {
        ...
        logger.warn(
          `Point trouvé mais aucun lien déclarant: name="${pointPrelevementName}" -> id=${linkedPointId}, declarantUserId=${declarantUserId}`
        )
      }
    } else {
      unmatchedPoints++
    ...
    const chunkGranularity = toChunkGranularity(granularity, logger)
    ...
    await prisma.chunkValue.createMany({
      data: chunk.data.map(d => ({
        ...
        frequency: granularity ?? '15_minutes',
...
export async function processAPIDeclaration(apiDeclarationId, logger = createLogger()) {
  ...
  const apiImport = await getApiDeclaration(apiDeclarationId)
  if (!apiImport) {
  ...
  const apiDeclaration = apiImport.payload ?? apiImport
  ...
  const newSource = await prisma.$transaction(async tx => {
    if (apiImport.id) {
      await tx.source.deleteMany({where: {apiImportId: apiImport.id}})
    }
...
async function getApiDeclaration(apiDeclarationId) {
  ...
  return apiDeclarations.find(d => String(d.id) === String(apiDeclarationId)) ?? apiDeclarations[0]
}
