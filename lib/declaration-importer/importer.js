...
export async function findAccessibleExploitationForPointName({
  ...
  const selected = chooseExploitationCandidate(
    matches.map(match => match.exploitation),
    externalDeclarant,
    {declarantUserId}
  )
  if (!selected) {
...
async function createChunksFromData(
  ...
  for (const chunk of chunkCandidates) {
    ...
    let candidatePointFlowType = null
    ...
    if (accessibleExploitationMatch) {
      ...
      candidatePointFlowType = accessibleExploitationMatch.exploitation.pointPrelevement.flowType
      ...
      if (pointPrelevement?.id) {
        ...
        candidatePointFlowType = pointPrelevement.flowType
        ...
        if (!directAccess.linkOnWindow && declarantRole === 'COLLECTEUR') {
          ...
          if (collecteurAccess.linkOnWindow || (!directAccess.hasAnyLink && collecteurAccess.hasAnyLink)) {
        ...
        } else if (pointAccess.hasAnyLink) {
          unmatchedPoints++
          parsingInfo = {
            case: 2,
