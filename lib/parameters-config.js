import {METRIC_TYPE_CODES, normalizeMetricTypeCode} from './constants/metric-type-codes.js'
...
export const ALL_FREQUENCIES = [...SUB_DAILY_FREQUENCIES, DAILY_FREQUENCY, '1 week', ...SUPER_DAILY_FREQUENCIES]

/**
 * Fréquences disponibles pour les paramètres cumulatifs (inclut l’infra-journalier
 * lorsque les connecteurs stockent du 15 min / 1 h, etc.)
 */
const CUMULATIVE_FREQUENCIES = [...SUB_DAILY_FREQUENCIES, DAILY_FREQUENCY, ...SUPER_DAILY_FREQUENCIES]
...
export function isParameterSupported(parameter) {
  return normalizeMetricTypeCode(parameter) in parametersConfig
}
...
export function getAvailableOperators(parameter, context = 'spatial') {
  const config = parametersConfig[normalizeMetricTypeCode(parameter)]
  if (!config) {
    return null
  }
...
export function getParameterValueType(parameter) {
  return parametersConfig[normalizeMetricTypeCode(parameter)]?.valueType || null
}
