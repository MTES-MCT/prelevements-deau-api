...
export async function sendAccountCreationNotification(person, {role} = {}) {
  ...
  if (!user?.email) {
    throw createHttpError(400, 'Impossible d’envoyer la notification : ce compte n’a pas d’adresse email.')
  }

  const html = await renderAccountCreationEmail(user, FRONT_URL)
...
export async function sendZoneAttachmentNotification({instructor, zone}) {
  ...
  const html = await renderZoneAttachmentEmail(user, zone, FRONT_URL)
