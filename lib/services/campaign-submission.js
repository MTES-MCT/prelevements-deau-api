...
import {campaignMeterReadings, campaignMeterFingerprint, campaignPublicationIssue, CAMPAIGN_MANUAL_PROVIDER} from './campaign-readings.js'
...
async function validateCampaignUsages(tx, data) {
  ...
  const usages = new Map((await tx.sandreWaterUse.findMany({where: {id: {in: [...new Set(usageFields.map(([, id]) => id))]}}})).map(usage => [usage.id, usage]))
...
export async function submitCampaignResponse({user, campaignId, responseId, body}, {client = prisma, now = new Date()} = {}) {
  ...
  await runCollectionTransaction(client, async tx => {
    ...
    for (const [index, meter] of resolved.meters.entries()) Object.assign(conflictingFields, await conflictingReadingFields(tx, response, meter, index))
