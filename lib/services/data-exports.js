...
const EXPORT_HEADERS = [
  {key: 'pointPrelevementId', label: 'ID du point de prélèvement'},
  {key: 'pointPrelevementNom', label: 'Nom du point de prélèvement'},
  ...
  {key: 'typeDonnee', label: 'Type de donnée'},
  {key: 'unite', label: 'Unité'},
  {key: 'date', label: 'Date'},
  {key: 'heure', label: 'Heure'},
  {key: 'dateHeure', label: 'DateHeure'},
...
function getValueType(metricTypeCode) {
  return parametersConfig[metricTypeCode]?.valueType || ''
}

function getMetricTypeLabel(metricTypeCode) {
  return parametersConfig[metricTypeCode]?.label || metricTypeCode || ''
}
...
function rowToExportObject(row) {
  const dateParts = toUtcDateTimeParts(row.periodStart)
  ...
  return {
    ...
    unite: row.unit || '',
    date: dateParts.date,
    heure: dateParts.time,
    dateHeure: dateParts.dateTime,
    valeur: decimalToExcelValue(row.value),
    remarque: '',
    frequence: row.frequency || '',
    typeValeur: getValueType(row.metricTypeCode),
...
async function resolveExportScope({user, filters}) {
  ...
  const selectedZoneIds = filters.zoneIds.filter(zoneId => allowedZoneIdSet.has(zoneId))

  return {
    allowedZones,
    allowedZoneIds,
    selectedZoneIds,
    rejectedZoneIds: filters.zoneIds.filter(zoneId => !allowedZoneIdSet.has(zoneId))
  }
}
...
export async function processDataExport(dataExportId, logger = console) {
  ...
  try {
    ...
    const requestedBy = {
      ...dataExport.requestedBy,
      role: dataExport.requestedByRole
    }
