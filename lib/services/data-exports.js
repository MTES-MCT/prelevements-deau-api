...
function getSandreZoneIdsSql(sandreZoneIds) {
  return Prisma.join(sandreZoneIds.map(sandreZoneId => Prisma.sql`${sandreZoneId}::uuid`))
}
...
export function getSandreZoneFilterSql(sandreZoneIds) {
  ...
  return Prisma.sql`
    AND EXISTS (
      ...
      WHERE export_sandre_zone.id IN (${getSandreZoneIdsSql(sandreZoneIds)})
  ...
  return Prisma.sql`
    AND EXISTS (
      ...
      WHERE accessible_ppz."pointPrelevementId" = accessible_point.id
        AND accessible_ppz."zoneId" IN (${getZoneIdsSql(allowedZoneIds)})
    )
  `
}
...
export async function getAccessibleSandreZones({
  ...
  const idFilter = Array.isArray(sandreZoneIds)
    ? Prisma.sql`AND sandre_zone.id IN (${getSandreZoneIdsSql(sandreZoneIds)})`
    : Prisma.empty
