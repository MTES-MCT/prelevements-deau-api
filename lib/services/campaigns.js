...
import {
  ...
  campaignResponseRequestSchema, campaignSubmitSchema, campaignReopenSchema, campaignVersionSchema, campaignMeterSchema, campaignManagersSchema
...
function publicTarget(target) {
  ...
  return {
    ...
    pointPrelevement: {id: pointPrelevement.id, name: pointPrelevement.name, usageName: pointPrelevement.usageName, collectionMode: pointPrelevement.collectionMode},
    ...
    meters: meters.map(meter => ({id: meter.id, associationId: meter.associationId, compteurId: meter.compteurId, compteur: {id: meter.compteur.id, serialNumber: meter.compteur.serialNumber, identifier: meter.compteur.identifier}, startDate: day(meter.startDate), endDate: day(meter.endDate)}))
  }
}
...
export async function changeCampaignStatus(user, campaignId, status, input, {client = prisma} = {}) {
  ...
  await transaction(client, async tx => {
    ...
    await bumpCampaign(tx, campaign, expectedVersion, {status, ...(status === 'OPEN' ? {openedAt: new Date()} : {closedAt: new Date()})})
    if (status === 'OPEN') {
      await enqueueCampaignNotifications(tx, {campaign, kind: 'OPENING', preleveurUserIds: [...new Set(access.targets.map(target => target.preleveurUserId))]})
    }
  })
  return getCampaignDetail(user, campaignId, {client})
}
...
export async function listCampaigns(user, {client = prisma} = {}) {
  ...
  for (const row of rows) {
    items.push(await getCampaignDetail(user, row.id, {client}))
  }

  return {items, permissions: {canCreate: user.role === 'ADMIN' || declarant?.declarantRole === 'COLLECTEUR' || managingZoneIds.length > 0}}
}
...
export async function getCampaignContext(user, campaignId, requestedPreleveurUserId, {client = prisma} = {}) {
  ...
  const context = await calculateContext(access.campaign, scope.targets, responses.INDEX?.draft, client)
  return {campaign: serializeCampaign({...access.campaign, targets: scope.targets}, scope.permissions), permissions: {...scope.permissions, canEdit: responses.INDEX.permissions.canEdit || responses.NEEDS.permissions.canEdit, canSubmit: responses.INDEX.permissions.canSubmit || responses.NEEDS.permissions.canSubmit}, editableTargetIds: scope.editableTargets.map(target => target.id), targets: scope.targets.map(target => publicTarget(target)), preleveurUserId, availablePreleveurs, responses, ...context}
}
...
export async function saveCampaignResponse(user, campaignId, kind, input, {client = prisma} = {}) {
  ...
  await transaction(client, async tx => {
    ...
    const incoming = validateCampaignDraft(kind, request.data, {campaign: access.campaign, targets: scope.editableTargets})
  ...
  return {response: context.responses[kind], calculation: context.calculation}
}
...
export async function submitCampaignResponse(user, campaignId, kind, input, {client = prisma} = {}) {
  ...
  const committedSubmission = await transaction(client, async tx => {
    ...
    assertResponseEditable(current, request.expectedVersion)
    const snapshot = validateCampaignDraft(kind, current.draft, {campaign: access.campaign, targets: scope.editableTargets, complete: true})
    ...
    let draft = snapshot
    if (kind === 'NEEDS') {
      await tx.campaignNeedLine.createMany({data: snapshot.needs.map(need => ({...need, submissionId: submission.id}))})
    } else {
      const publication = await publishCampaignIndexSubmission({submission, campaign: access.campaign, targets: scope.editableTargets, actorUserId: user.id, client: tx})
  ...
  const context = await getCampaignContext(user, campaignId, request.preleveurUserId, {client})
  return {response: context.responses[kind], calculation: context.calculation, submission: committedSubmission}
}

export async function reopenCampaignResponse(user, campaignId, kind, input, {client = prisma} = {}) {
  const request = validateCampaignValue(campaignReopenSchema, input)
  const reopenUntil = request.reopenUntil ?? new Date(Date.now() + (7 * 86_400_000))
  if (reopenUntil <= new Date() || reopenUntil > new Date(Date.now() + (30 * 86_400_000))) {
    throw createError(400, 'La réouverture doit avoir une échéance future de trente jours au maximum.')
  }
  ...
  await transaction(client, async tx => {
    const access = await getCampaignAccess(user, campaignId, {client: tx})
    const scope = getCampaignResponseScope(access, user, request.preleveurUserId)
    assertCampaignCapability(scope, 'canReopen')
    const current = await tx.campaignResponse.findUnique({where: {campaignId_preleveurUserId_kind: {campaignId, preleveurUserId: request.preleveurUserId, kind}}})
    const data = {status: 'DRAFT', reopenedAt: new Date(), reopenUntil, reopenedByUserId: user.id, reopenReason: request.reason}
    if (!current && request.expectedVersion === 0) {
      await tx.campaignResponse.create({data: {campaignId, preleveurUserId: request.preleveurUserId, kind, version: 1, ...data}})
    } else {
      const changed = await tx.campaignResponse.updateMany({where: {campaignId, preleveurUserId: request.preleveurUserId, kind, version: request.expectedVersion}, data: {...data, version: {increment: 1}}})
      if (changed.count !== 1) {
        throw conflict()
      }
    }
...
export async function getCampaignOptions(user, {zoneId, ownerCollecteurUserId, usageId, q = '', cursor, limit = 500, client = prisma} = {}) {
  const zoneIds = await getPermissionZoneIdsForUser(user, 'campaign.manage', {client})
  ...
  const zones = await client.zone.findMany({where: isAdmin ? {} : (isCollecteur ? {pointPrelevementZones: {some: {pointPrelevement: {declarants: {some: {collecteurs: {some: {collecteurUserId: user.id}}}}}}}} : {id: {in: zoneIds}}), select: {id: true, name: true, type: true, code: true}, orderBy: {name: 'asc'}})
  ...
  const collecteurs = await client.declarant.findMany({where: {declarantRole: 'COLLECTEUR', user: {deletedAt: null}, ...(isCollecteur ? {userId: user.id} : (effectiveZoneId ? {collecteurExploitations: {some: {exploitation: {pointPrelevement: {zones: {some: {zoneId: effectiveZoneId}}}}}}} : {}))}, select: {userId: true, socialReason: true, user: {select: {firstName: true, lastName: true}}}, take: 200})
  const selectedOwner = ownerCollecteurUserId || (isCollecteur ? user.id : collecteurs[0]?.userId)
  ...
  const scope = effectiveZoneId && selectedOwner
    ? {
      ...
      declarant: {declarantRole: 'PRELEVEUR', user: {deletedAt: null}}
    }
    : null
  ...
  const managers = await client.declarant.findMany({where: {declarantRole: 'COLLECTEUR', user: {deletedAt: null}, ...(effectiveZoneId ? {collecteurExploitations: {some: {exploitation: {pointPrelevement: {zones: {some: {zoneId: effectiveZoneId}}}}}}} : {userId: selectedOwner || user.id})}, select: {userId: true, socialReason: true, user: {select: {firstName: true, lastName: true}}}, take: 200})
  return {
