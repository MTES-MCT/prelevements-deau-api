...
import {loadCampaignList} from './campaign-list.js'
import {loadCampaignExistingReadings, prepareCampaignIndexPublication, publishCampaignIndexSubmission} from './campaign-publication.js'
...
export async function changeCampaignStatus(user, campaignId, status, input, {client = prisma, now = new Date()} = {}) {
  ...
  await transaction(client, async tx => {
    ...
    if (status === 'OPEN') {
      ...
      const bindings = await loadCampaignMeterHistory(await tx.compteurPointPrelevement.findMany({where: {compteurId: {in: pointBindings.map(binding => binding.compteurId)}}, include: {compteur: true}}), tx)
      ...
      for (const target of access.targets) {
        ...
        await tx.campaignTargetMeter.createMany({data: bindings.filter(binding => binding.pointPrelevementId === target.pointPrelevementId && !binding.compteur.deletedAt && (!binding.startDate || day(binding.startDate) <= [...campaign.indexDates].sort().at(-1)) && (!binding.endDate || day(binding.endDate) >= [...campaign.indexDates].sort()[0])).map(binding => ({targetId: target.id, associationId: binding.id, compteurId: binding.compteurId, startDate: binding.startDate, endDate: binding.endDate}))})
      }
    }
...
export async function listCampaigns(user, {client = prisma} = {}) {
  return loadCampaignList(user, {client})
}
...
export async function submitCampaignResponse(user, campaignId, kind, input, {client = prisma} = {}) {
  ...
  const committedSubmission = await transaction(client, async tx => {
    ...
    const preparation = kind === 'INDEX'
      ? await prepareCampaignIndexPublication({campaign: access.campaign, targets: publicationContext.targets, snapshot, client: tx})
      : null
    // Le snapshot immuable et ses exports excluent les saisies hors phase dès
    // leur création. Le brouillon reste complet pour annuler sans perte.
    const submission = await tx.campaignSubmission.create({data: {
      id: randomUUID(), responseId: current.id, version: current.version + 1, idempotencyKey: request.idempotencyKey,
      createdByUserId: user.id, snapshot: preparation?.officialSnapshot ?? snapshot
    }})
    let draft = snapshot
    if (kind === 'NEEDS') {
      ...
      const publication = await publishCampaignIndexSubmission({submission, campaign: access.campaign, targets: publicationContext.targets, actorUserId: user.id, client: tx, preparation})
