...
export async function getPointsFromPreleveur(preleveurId, includeDeleted = false) {
  const exploitations = await getDeclarantExploitations(preleveurId, {point: 1})
...
function getStatus(exploitations) {
  if (exploitations.some(e => e.status === 'En activité')) {
    return 'En activité'
  }

  if (exploitations.some(e => e.status === 'Non renseigné')) {
    return 'Non renseigné'
  }

  if (exploitations.some(e => e.status === 'Abandonnée')) {
    return 'Abandonnée'
  }

  if (exploitations.some(e => e.status === 'Terminée')) {
    return 'Terminée'
  }
}
