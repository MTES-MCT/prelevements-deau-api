...
export async function decoratePointPrelevement(point) {
  ...
  const exploitationsWithDate = exploitations.filter(e => e.date_debut)
  const oldestExploitation = exploitationsWithDate.length > 0 ? minBy(exploitationsWithDate, e => e.date_debut) : null

  return {
    ...point,
    preleveurs,
    exploitationsStatus: hasExploitations ? getStatut(exploitations) : null,
    exploitationsStartDate: oldestExploitation?.date_debut ?? null,
