...
import {getExploitationWaterUses} from './exploitation-usages.js'
...
function aggregateWaterUses(exploitations = []) {
  ...
  for (const exploitation of exploitations) {
    for (const usage of getExploitationWaterUses(exploitation)) {
      if (usage.id) {
        byId.set(usage.id, serializeWaterUse(usage))
      }
    }
  }
...
export async function updatePointPrelevement(pointId, payload, {user} = {}) {
  ...
  const updatesWaterBodyConnections = Object.hasOwn(changes, 'nature')
    || WATER_BODY_FIELDS.some(field => Object.hasOwn(changes, field))
...
export async function decoratePointsPrelevement(points, {
  ...
  const decoratedPoints = points.map(point => {
    ...
    const pointDeclarants = [...new Set(
      (point.declarants ?? []).map(d => d.declarantUserId)
    )]
      .map(declarantUserId => declarantsById.get(declarantUserId))
      .filter(Boolean)
