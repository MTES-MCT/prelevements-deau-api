...
function getPointSearchIdentifiers(point) {
...
export function serializePointMapSummary(point, {
  ...
  const preleveurExploitations = canReadDeclarants
    ? declarants.filter(exploitation => exploitation.declarant?.declarantRole === 'PRELEVEUR')
    : []
  ...
  return {
    ...
    searchIdentifiers: getPointSearchIdentifiers(point),
