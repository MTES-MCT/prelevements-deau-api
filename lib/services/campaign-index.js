...
import {campaignMeterEventDateIssues} from '../validation/campaign-meter-events.js'
...
function meterPhase(meter, targetId, meterEvents) {
  const startDate = meter.startDate ? campaignDate(meter.startDate) : null
  const endDate = meter.endDate ? campaignDate(meter.endDate) : null
  const replacements = meterEvents.filter(event => event.targetId === targetId && event.type === 'REPLACEMENT'
    && (!startDate || event.at >= startDate) && (!endDate || event.at <= endDate))
  const starts = [startDate, ...replacements.filter(event => event.nextCompteurId === meter.compteurId).map(event => campaignDate(event.at))].filter(Boolean).sort()
  const ends = [endDate, ...replacements.filter(event => event.previousCompteurId === meter.compteurId).map(event => campaignDate(event.at))].filter(Boolean).sort()
  return {...meter, startDate: starts.at(-1) ?? null, endDate: ends[0] ?? null}
}

const containsDate = (meter, date) => (!meter.startDate || date >= meter.startDate) && (!meter.endDate || date <= meter.endDate)
...
function checkExistingReading(reading, target, source) {
  ...
  if (!source.chunk.compteurId && reading.compteurId !== null && reading.meterConfirmed !== true) {
...
function prepareReadings({targets, readings, existingReadings, campaign, periods, meterEvents}) {
  const targetMap = new Map(targets.map(target => [target.id, target]))
  const meterlessPeriods = new Map(targets.map(target => [target.id, campaignMeterlessPeriod(target, {campaign, periods, meterEvents})]))
  ...
  for (const rawReading of readings) {
    ...
    const allowedMeter = target && (reading.compteurId === null
      ? isCampaignMeterlessDate(meterlessPeriods.get(target.id), reading.readingDate)
    ...
    // Une saisie devenue extérieure à la phase du compteur reste dans le
    // brouillon, mais n'est ni reprise comme historique ni publiée. Elle peut
    // redevenir utile si le déclarant annule ou déplace son remplacement.
    try {
      if (!phasesByTarget.has(target.id)) {
        const meterless = meterlessPeriods.get(target.id)
        const meters = [...(target.meters ?? []), ...(meterless ? [{compteurId: null, ...meterless}] : [])]
        phasesByTarget.set(target.id, Map.groupBy(meters.map(meter => meterPhase(meter, target.id, meterEvents)), meter => meter.compteurId))
      }

      const phases = phasesByTarget.get(target.id).get(reading.compteurId) ?? []
      if (!phases.some(meter => containsDate(meter, date))) {
        ignoredReadings.push({...reading, readingDate: date, code: 'READING_OUTSIDE_METER_PERIOD'})
        continue
      }
    } catch {
      issues.push(issue('INVALID_PERIOD_OR_METER_DATE', context))
      continue
    }
  ...
  return {prepared, issues, ignoredReadings}
}

function prepareEvents(targets, events, {campaign, periods}) {
  const targetMap = new Map(targets.map(target => [target.id, target]))
  const meterlessPeriods = new Map(targets.map(target => [target.id, campaignMeterlessPeriod(target, {campaign, periods, meterEvents: events})]))
  const prepared = []
  const issues = []
  const seen = new Set()
  for (const event of events) {
    ...
    const previousAllowed = event.previousCompteurId === null
      ? target && isCampaignMeterlessDate(meterlessPeriods.get(target.id), at)
      : meterIds.has(event.previousCompteurId)
    const nextAllowed = event.type === 'RESET' ? previousAllowed : meterIds.has(nextCompteurId)
    ...
    if (!['RESET', 'REPLACEMENT'].includes(event.type)
      || !previousAllowed || !nextAllowed
      || (event.type === 'REPLACEMENT' && nextCompteurId === event.previousCompteurId)
      || (event.type === 'RESET' && event.nextCompteurId !== undefined && event.nextCompteurId !== event.previousCompteurId)
...
function calculateMeter({target, meter, start, end, events, readings, context, missing, conflicts}) {
  ...
  const meterStart = meter.startDate ?? start
  const meterEnd = meter.endDate ?? end
...
function metersForPeriod({meters, start, end, context, conflicts}) {
  const active = meters.filter(meter => (!meter.startDate || meter.startDate < end) && (!meter.endDate || meter.endDate > start))
  const result = []
  for (const [compteurId, bindings] of Map.groupBy(active, meter => meter.compteurId)) {
    const ordered = bindings.sort((a, b) => (a.startDate ?? '').localeCompare(b.startDate ?? ''))
    let previous
    for (const meter of ordered) {
      if (previous && (!previous.endDate || !meter.startDate || meter.startDate < previous.endDate)) {
        conflicts.push(issue('AMBIGUOUS_METER_BINDING', {...context, compteurId}))
        continue
      }

      if (previous && previous.endDate === meter.startDate) {
        // Deux affectations qui se touchent ne doublent pas la durée ni le
        // relevé partagé du même compteur physique.
        previous.endDate = meter.endDate
      } else {
        previous = {...meter}
...
/** Calcul exact par compteur, ou par point pour la phase sans inventaire. */
export function calculateCampaignIndexTotals({campaign, periods = campaign?.periods ?? [], targets, readings = [], meterEvents = [], existingReadings = []}) {
  const preparedReadings = prepareReadings({targets, readings, existingReadings, campaign, periods, meterEvents})
  const preparedEvents = prepareEvents(targets, meterEvents, {campaign, periods})
  const issues = [...preparedReadings.issues,
    ...preparedEvents.issues,
    ...campaignMeterEventDateIssues(preparedEvents.prepared, {campaign, periods, targets})]
  ...
  for (const reading of [...preparedReadings.prepared.values(), ...preparedReadings.ignoredReadings]) {
  ...
  for (const target of targets) {
    const meterless = campaignMeterlessPeriod(target, {campaign, periods, meterEvents: preparedEvents.prepared})
    for (const period of indexPeriods) {
      ...
      try {
        ...
        if (start >= end) {
          ...
          const meters = [...(target.meters ?? []), ...(meterless ? [{compteurId: null, endDate: meterless.endDate}] : [])]
            .map(meter => meterPhase(meter, target.id, preparedEvents.prepared))
          for (const meter of metersForPeriod({meters, start, end, context, conflicts})) {
  ...
  return {
    ...
    ignoredReadings: preparedReadings.ignoredReadings,
