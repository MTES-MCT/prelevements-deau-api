...
/** Préparation et publication doivent partager la transaction de transmission. */
export async function prepareCampaignIndexPublication({campaign, targets, snapshot, client, submissionId}) {
  ...
  if (submissionId) {
    const previous = await client.campaignSubmission.findUnique({where: {id: submissionId}, select: {publication: true}})
    if (previous?.publication) {
      return {existingPublication: previous.publication}
    }
  }

  const existingReadings = await loadCampaignExistingReadings({campaign, targets, client})
  const calculation = calculateCampaignIndexTotals({campaign, targets, ...snapshot, existingReadings})
  ...
  const ignored = new Set(calculation.ignoredReadings.map(reading => `${reading.targetId}:${reading.compteurId}:${reading.readingDate}`))
  const officialSnapshot = {...snapshot, readings: snapshot.readings.filter(reading => !ignored.has(`${reading.targetId}:${reading.compteurId}:${reading.readingDate}`))}
  return {calculation, existingReadings, officialSnapshot}
}

/** Appelé uniquement dans la transaction de transmission, après contrôle des droits. */
export async function publishCampaignIndexSubmission({submission, campaign, targets, actorUserId, client, preparation}) {
  ...
  const prepared = preparation ?? await prepareCampaignIndexPublication({campaign, targets, snapshot: submission.snapshot, client, submissionId: submission.id})
  if (prepared.existingPublication) {
    return prepared.existingPublication
  }

  const {calculation, existingReadings} = prepared
