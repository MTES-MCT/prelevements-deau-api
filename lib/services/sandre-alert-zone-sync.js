...
import {getSandreGeometryRepairApproval} from './sandre-alert-zone-geometry-approvals.js'
...
export async function normalizeSandreZoneGeometries(features, {database = prisma} = {}) {
  ...
  for (const [index, feature] of geometricFeatures.entries()) {
    ...
    const repairApproval = getSandreGeometryRepairApproval(feature)
    const areaDeltaApproved = relativeAreaDelta <= MAX_RELATIVE_AREA_DELTA
      || (
        row.raw_valid === false
        && repairApproval
        && relativeAreaDelta <= repairApproval.maxRelativeAreaDelta
      )
    if (
