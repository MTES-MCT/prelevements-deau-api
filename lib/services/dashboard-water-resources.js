...
import {getMonitoringStationSyncStatus} from './monitoring-stations.js'

const PIEZOMETRY_PERIODS = new Set(['week', 'month', 'year', 'twenty-years'])
...
async function listStationGroups(type, zoneIds) {
  if (zoneIds.length === 0) {
    return []
  }
...
export async function getDashboardPiezometry({zoneIds, period = 'week', now = new Date()}) {
  ...
  const stationGroups = await listStationGroups('PIEZOMETER', zoneIds)
  ...
  const rows = stationIds.length === 0
    ? []
    : await prisma.groundwaterObservation.findMany({
      where: {
  ...
  const stations = stationGroups.map(group => {
    ...
    const values = normalizedPeriod === 'twenty-years'
      ? aggregateGroundwaterValuesByUTCWeek(rawValues)
      : rawValues
  ...
  return {
    ...
    aggregation: normalizedPeriod === 'twenty-years'
