...
import {buildPiezometryIps, PIEZOMETRY_IPS} from './piezometry-index.js'
...
export async function getDashboardPiezometry({
  ...
  const [rows, ipsRows] = stationIds.length === 0
    ? [[], []]
    : await Promise.all([
      prisma.groundwaterObservation.findMany({
        where: {
      ...
      includeIps
        ? prisma.groundwaterObservation.findMany({
          where: {
            ...
            kind: 'CHRONICLE',
            measuredAt: {
              gte: subtractYears(now, PIEZOMETRY_IPS.referenceHistoryYears),
              lt: now
            }
          },
          orderBy: {measuredAt: 'asc'},
  ...
  const stations = stationGroups.map(group => {
    ...
    return {
      ...
      values,
      ...(includeIps && {
        ips: buildPiezometryIps(ipsRowsByStation.get(station.id) ?? [], {
