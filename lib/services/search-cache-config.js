...
import {readRedisUrl} from '../config/redis-url.js'
...
function readSearchCacheUrl(environment = process.env) {
  ...
  const bullMqEndpoints = new Set([
    redisEndpointIdentity(bullMqUrl),
    redisEndpointIdentity(readRedisUrl(environment))
  ].filter(Boolean))

  return searchEndpoint && !bullMqEndpoints.has(searchEndpoint)
    ? searchCacheUrl
    : null
}
...
export function closeDedicatedSearchCacheRedis() {
  dedicatedRedis?.disconnect(false)
  dedicatedRedis = undefined
  dedicatedRedisSignature = undefined
}
