...
import {createDeclarationExploitationMatcher} from './declaration-exploitation-matching.js'
import {
...
function recipientTemplateParams({
  ...
  return {
    ...
    POINTS: points.map(point => point.countingCode ? `${point.name} (comptage ${point.countingCode})` : point.name).join(', ')
  }
}
...
async function getDeclaredExploitationIds({exploitations, pointIds, periodStart, periodEnd, client = prisma}) {
  ...
  const chunks = await client.chunk.findMany({
    ...
    select: {
      ...
      minDate: true,
      maxDate: true,
      source: {select: {declaration: {select: {declarantUserId: true}}}}
    }
  })

  const matchExploitation = createDeclarationExploitationMatcher(exploitations)
  return new Set(chunks.map(matchExploitation).filter(Boolean))
}
...
function addRecipient(recipientsByEmail, recipient, context) {
  ...
  for (const email of contactEmails.emails) {
    ...
    for (const point of context.points) {
      current.points.set(point.exploitationId ?? point.id, point)
    }
...
export async function computeDeclarationNotificationRecipients({
  ...
  const declaredExploitationIds = await getDeclaredExploitationIds({
    exploitations,
    pointIds,
    periodStart,
    periodEnd,
  ...
  for (const exploitation of exploitations) {
    ...
    const hasDeclared = declaredExploitationIds.has(exploitation.id)
  ...
  for (const row of expectedRows) {
    const hasRecipient = recipients.some(recipient =>
      recipient.points.some(point => point.exploitationId === row.exploitationId)
    )
...
async function sendBrevoTemplateEmail({email, name, templateId, params}) {
  ...
  try {
    ...
    // Brevo can return a non-JSON error page; retain its HTTP status below.
  }
...
export async function buildMissingDeclarationsWorkbook({
  ...
  sheet.columns = [
    ...
    {header: 'Codes comptage', key: 'countingCodes', width: 28},
  ...
  for (const recipient of preview.recipients) {
    sheet.addRow({
      ...
      countingCodes: recipient.points.map(point => point.countingCode ? `${point.name} : ${point.countingCode}` : `${point.name} : ${fallback}`).join(', '),
