...
function getUserEmails(user) {
  ...
  return [...new Set(emails.map(email => normalizeEmail(email, {required: false})).filter(Boolean))]
}
...
function serializePoint(exploitation) {
  ...
  return {
    ...
    usage: exploitation.usage
      ? {
        id: exploitation.usage.id,
        code: exploitation.usage.code,
        name: exploitation.usage.name
      }
      : null
  }
}
...
function recipientTemplateParams({
  ...
  return {
    ...
    RAISON_SOCIALE: recipient.socialReason || '',
    PERIODE: periodLabel,
...
function addRecipient(recipientsByEmail, recipient, context) {
  for (const email of getUserEmails(recipient.user)) {
...
function getExclusionReasonContent(reason, details = {}) {
  ...
  const content = {
    ...
    TELEMETRY: {
      reasonLabel: 'Point en télérelève',
      reasonDescription: 'Les points suivis automatiquement par télérelève ne reçoivent pas de rappel ou de relance.'
    },
    NO_ZONE: {
...
function addExclusion(exclusions, exploitation, reason, details = {}) {
  exclusions.push({
    reason,
    ...getExclusionReasonContent(reason, details),
    declarantUserId: exploitation.declarantUserId,
    declarantLabel: buildDisplayName({
      firstName: exploitation.declarant?.user?.firstName,
      lastName: exploitation.declarant?.user?.lastName,
      email: exploitation.declarant?.user?.email,
      socialReason: exploitation.declarant?.socialReason
    }),
...
export async function computeDeclarationNotificationRecipients({
  ...
  for (const exploitation of exploitations) {
    ...
    if (exploitation.connectors?.length > 0) {
      addExclusion(exclusions, exploitation, 'TELEMETRY')
      continue
    }
...
export async function listUpcomingDeclarationNotificationPreviews({from = new Date(), client = prisma} = {}) {
  const definitions = [
