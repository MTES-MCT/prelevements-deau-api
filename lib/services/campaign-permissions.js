...
// Les lectures détaillées et la liste utilisent les mêmes capacités. La liste
// fournit seulement les cibles déjà filtrées et leur nombre total en base.
export function getCampaignScopePermissions(user, campaign, targets, {zonePermissions, totalTargetCount = campaign.targets.length}) {
  ...
  const scopeComplete = targets.length === totalTargetCount
  return {
    permissions: {
      canRead, canManage, canManageSharing: canManage && scopeComplete, canExport, exportTargetIds,
      canRemind: canManage, canFollowup: canManage || canExport || user.role === 'INSTRUCTOR'
    },
    scopeComplete
  }
}
...
export async function getCampaignAccess(user, campaignId, {client = prisma} = {}) {
  ...
  if (campaign.status === 'DRAFT') {
    const bindings = await loadCampaignMeterHistory(await client.compteurPointPrelevement.findMany({
      where: {pointPrelevementId: {in: campaign.targets.map(target => target.pointPrelevementId)}}, include: {compteur: true}
    }), client)
  ...
  const {permissions, scopeComplete} = getCampaignScopePermissions(user, campaign, targets, {zonePermissions})
  if (!permissions.canRead || (campaign.status === 'DRAFT' && !permissions.canManage)) {
