...
function exploitationDateBoundary(value, followingDay = false) {
...
export function planMeterInterval(start, end, stream, allocations) {
  ...
  if (!stream.enabled || !stream.activatedAt || start.observedAt < new Date(stream.activatedAt)) return {reason: 'NOT_ACTIVATED'}
  ...
  for (const share of shares.filter(entry => entry.inScope)) {
    ...
    const exploitationStart = exploitationDateBoundary(exploitation?.startDate)
    const exploitationEnd = exploitationDateBoundary(exploitation?.endDate, true)
