...
export function planMeterInterval(start, end, stream, allocations) {
  ...
  for (const share of shares.filter(entry => entry.inScope)) {
    ...
    const usageId = version.usageId ?? exploitation.usageId
    const previousGroup = groups.get(exploitation.id)
    if (previousGroup && previousGroup.exploitation.usageId !== usageId) return {reason: 'ALLOCATION_USAGE_MISMATCH'}
    const group = previousGroup ?? {exploitation: {...exploitation, usageId}, shares: [], volume: 0n, additive: true}
