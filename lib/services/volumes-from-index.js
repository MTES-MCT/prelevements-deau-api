...
export async function reconstructVolumesForChunks(chunks, client) {
  ...
  for (const chunk of chunks) {
    ...
    const reason = chunk.calculationStrategy === 'CAMPAIGN'
      ? 'CAMPAIGN_EXACT_CALCULATION'
  ...
  const computedRows = await client.$queryRaw`
    WITH eligible_chunks AS (
      SELECT c.id, c."preleveurUserId", c."compteurId"
      FROM "Chunk" c
    ...
    dedup AS (
      SELECT DISTINCT ON ("preleveurUserId", "compteurId", date)
        "preleveurUserId",
        "compteurId",
        date,
        value,
        ...
        frequency
      FROM raw_idx
      ORDER BY "preleveurUserId", "compteurId", date, "createdAt" DESC, id DESC
    ),
    calc AS (
      SELECT
        "chunkId",
        "preleveurUserId",
        LAG(date) OVER (PARTITION BY "preleveurUserId", "compteurId" ORDER BY date) AS "periodStart",
        date AS "periodEnd",
        CASE
          WHEN LAG(value) OVER (PARTITION BY "preleveurUserId", "compteurId" ORDER BY date) IS NULL THEN NULL
          WHEN value - LAG(value) OVER (PARTITION BY "preleveurUserId", "compteurId" ORDER BY date) >= 0
            THEN value - LAG(value) OVER (PARTITION BY "preleveurUserId", "compteurId" ORDER BY date)
          ELSE value
        END AS volume,
        unit,
        frequency
    ...
    WHERE "periodStart" IS NOT NULL
      AND NOT EXISTS (
        ...
        WHERE coverage.active = true
          AND coverage."pointPrelevementId" = ${pointId}::uuid
          AND (coverage."preleveurUserId" = calc."preleveurUserId" OR calc."preleveurUserId" IS NULL)
          AND coverage."periodStart" < calc."periodEnd"
          AND coverage."periodEnd" > calc."periodStart"
      )
  `
