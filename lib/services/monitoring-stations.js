...
function metadataObject(station) {
  return station.metadata && typeof station.metadata === 'object' && !Array.isArray(station.metadata)
    ? station.metadata
    : {}
}

export function getMonitoringStationDetails(station) {
  const metadata = metadataObject(station)

  if (station.type === 'PIEZOMETER') {
    return {
      commune: textValue(metadata.nom_commune),
      department: textValue(metadata.nom_departement),
      altitudeMeters: finiteNumber(metadata.altitude_station),
      investigationDepthMeters: finiteNumber(metadata.profondeur_investigation),
      watercourse: null,
      openedAt: textValue(metadata.date_debut_mesure),
      closedAt: textValue(metadata.date_fin_mesure),
      inService: null
    }
  }

  return {
    commune: textValue(metadata.libelle_commune),
    department: textValue(metadata.libelle_departement),
    altitudeMeters: finiteNumber(metadata.altitude_ref_alti_station),
    investigationDepthMeters: null,
    watercourse: textValue(metadata.libelle_cours_eau),
    openedAt: textValue(metadata.date_ouverture_station),
    closedAt: textValue(metadata.date_fermeture_station),
    inService: typeof metadata.en_service === 'boolean' ? metadata.en_service : null
  }
}
