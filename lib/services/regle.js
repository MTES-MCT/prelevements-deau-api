...
export async function createRegle(payload, preleveurId, codeTerritoire) {
  ...
  if (payload.document) {
    const documentId = mongo.parseObjectId(payload.document)
  ...
  if (payload.exploitations && payload.exploitations.length > 0) {
    const exploitationIds = payload.exploitations.map(id => mongo.parseObjectId(id))
  ...
  return RegleModel.insertRegle(regle, codeTerritoire)
}
...
export async function decorateRegle(regle) {
  ...
  if (regle.document) {
    ...
    decorated.document = document || null
  }
