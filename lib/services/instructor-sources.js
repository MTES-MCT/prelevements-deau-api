...
import {getMeterChunkAccessWhere, getVisibleTelemetryChunksWhere, hydrateMeterSourcePeriods, scopeMeterSource} from './telemetry-source-access.js'
...
function getSourceListInclude(chunkWhere) {
  return {
    declaration: {
      include: {
        files: {
...
function chunkPeriodWhere({startDate, endDate, meterChunkWhere = {}} = {}) {
  if (!startDate && !endDate) return {}
  return {OR: [
    {calculationStrategy: {not: 'METER'},
      ...(startDate ? {maxDate: {gte: startDate}} : {}),
      ...(endDate ? {minDate: {lte: endDate}} : {})},
    {calculationStrategy: 'METER', ...meterChunkWhere, chunkValues: {some: {
...
function sourceChunkWhere({pointIds, startDate, endDate, meterChunkWhere} = {}) {
  const chunkWhere = {
    ...(pointIds ? {pointPrelevementId: {in: pointIds}} : {}),
    ...chunkPeriodWhere({startDate, endDate, meterChunkWhere})
  }
...
function declarationSearchWhere({declarant, dossierNumber, meterChunkWhere = {}} = {}) {
  const where = {}
  ...
  if (declarant && !dossierNumber) {
    return {OR: [
      {declaration: {is: where}},
      {chunks: {some: {calculationStrategy: 'METER', ...meterChunkWhere, preleveur: where.declarant}}}
    ]}
  }

  return Object.keys(where).length > 0 ? {declaration: {is: where}} : {}
}
...
async function decorateSourcesWithDeclarationTypes(sources, {client = prisma} = {}) {
  sources = await hydrateMeterSourcePeriods(sources, {client})
...
export function getSourceChunkMutationCapabilities(chunk, {canInstruct = false, canReconcile = false} = {}) {
  if (chunk.metadata?.collectionResponseId || (chunk.calculationStrategy && chunk.calculationStrategy !== 'GENERIC')) {
    return {canInstruct: false, canReconcile: false}
  }

  return {canInstruct, canReconcile: canChangeChunkPointAssociation(chunk) && canReconcile}
}
...
async function decorateSourceDetail(source, {
  ...
  source = scopeMeterSource(source)

  const storage = canDownloadFiles && source.declaration?.files?.length > 0
  ...
  return {
    ...
    chunks: source.chunks.map(chunk => ({
      ...
      ...getSourceChunkMutationCapabilities(chunk, {canInstruct: canInstructChunk(chunk), canReconcile: canReconcileChunk(chunk)}),
...
export async function listSourcesForInstructor({
  ...
  const where = andWhere([
    sourceStatusWhere(statuses), sourceTypeWhere(types),
    declarationSearchWhere({declarant, dossierNumber,
      meterChunkWhere: getMeterChunkAccessWhere({user: {role: 'INSTRUCTOR'}, pointIds})}),
    instructorSourceScopeWhere({declarantUserIds, pointIds}), visibleSourceWhere(),
    sourceChunkWhere({startDate, endDate,
      meterChunkWhere: getMeterChunkAccessWhere({user: {role: 'INSTRUCTOR'}, pointIds})}), pointsToAssociateWhere(pointsToAssociate)
  ])
  ...
  const [total, sources] = await Promise.all([
    ...
    client.source.findMany({
      ...
      include: getSourceListInclude(getVisibleTelemetryChunksWhere({user: {role: 'INSTRUCTOR'}, pointIds}))
    })
  ])

  return paginatedSourcesPayload({
    items: await decorateSourcesWithDeclarationTypes(sources, {client}),
    page,
    pageSize,
    total
  })
...
export async function listSourcesForAdmin({
  ...
  const where = andWhere([
    sourceStatusWhere(statuses), sourceTypeWhere(types),
    declarationSearchWhere({declarant, dossierNumber}), visibleSourceWhere(),
    sourceChunkWhere({startDate, endDate}), pointsToAssociateWhere(pointsToAssociate)
  ])
...
export async function getValidatedChunkConflictsForChunks(chunks, db = prisma) {
  const candidateChunks = chunks.filter(
    chunk =>
      chunk.calculationStrategy !== 'METER'
  ...
  for (const chunk of candidateChunks) {
    ...
    const conflicts = validatedChunks
      .filter(validatedChunk => {
        ...
        if (chunk.exploitationId && validatedChunk.exploitationId
          && chunk.exploitationId !== validatedChunk.exploitationId) {
          return false
        }
      ...
      .map(conflict => ({
        ...
        exploitationId: conflict.exploitationId ?? null,
...
export async function getSourceForInstructor(sourceId, {
  ...
  const source = await client.source.findFirst({
    ...
    include: {
      ...
      chunks: {
        where: getVisibleTelemetryChunksWhere({user: {role: 'INSTRUCTOR'}, pointIds: readPointIds}),
