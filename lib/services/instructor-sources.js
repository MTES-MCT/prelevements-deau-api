...
export function instructorSourceScopeWhere({pointIds = [], zoneIds = []} = {}) {
  const scopes = []
  ...
  if (zoneIds.length > 0) {
    const declarantZoneWhere = {zones: {some: {zoneId: {in: zoneIds}}}}
    scopes.push({
      declaration: {
        is: {
          OR: [
            {declarant: declarantZoneWhere},
            {createdByDeclarant: declarantZoneWhere}
          ]
        }
      }
    })
...
async function decorateSourceDetail(source, {
  ...
  return {
    ...
    chunks: source.chunks.map(chunk => ({
      ...
      canReconcile: canReconcileChunk(chunk),
...
export async function getAccessiblePointPrelevementIdsSetForInstructor(
  ...
  const pointIds = await getAccessiblePointPrelevementIdsForInstructor(zoneIds)
...
export async function listSourcesForInstructor({
  ...
  const where = {
    ...
    ...instructorSourceScopeWhere({pointIds, zoneIds}),
    ...visibleSourceWhere(),
    ...andWhere([
...
export async function getSourceForInstructor(sourceId, {
  ...
  const pointIdsSet = await getAccessiblePointPrelevementIdsSetForInstructor(readZoneIds)
  const instructPointIdsSet = await getAccessiblePointPrelevementIdsSetForInstructor(instructZoneIds)
  const reconcilePointIdsSet = await getAccessiblePointPrelevementIdsSetForInstructor(reconcileZoneIds)
