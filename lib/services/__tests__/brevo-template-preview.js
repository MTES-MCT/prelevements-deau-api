...
test('les longs espaces et accolades incomplètes restent traités sans backtracking coûteux', t => {
  const spaces = ' '.repeat(50_000)
  const preview = renderBrevoTemplatePreview({subject: `{{${spaces}INCONNU${spaces}}}`, htmlContent: `{{${spaces}`}, {})
  t.deepEqual(preview.unresolvedPlaceholders, ['INCONNU'])
  t.is(preview.htmlContent.length, spaces.length + 2)
})
