...
async function fixture({shares = [70, 30], external = 0, enabled = true, targetExploitation,
  splitExploitations = false, samePair = false,
  ...
  const exploitation = await prisma.declarantPointPrelevement.create({data: {
    countingCode: samePair ? '001' : null,
    pointPrelevementId: point.id, declarantUserId: user.id, usageId: usage.id, status: 'EN_ACTIVITE'
  }})
  const otherUser = splitExploitations && !samePair
    ? await prisma.user.create({data: {role: 'DECLARANT', declarant: {create: {preleveurType: 'IRRIGANT'}}}})
    : user
  const otherPoint = splitExploitations && !samePair
    ? await prisma.pointPrelevement.create({data: {name: `Other meter point ${key}`, waterBodyType: 'SUPERFICIELLE', flowType: 'PRELEVEMENT'}})
    : point
  const otherExploitation = splitExploitations ? await prisma.declarantPointPrelevement.create({data: {
    pointPrelevementId: otherPoint.id, declarantUserId: otherUser.id, usageId: usage.id, status: 'EN_ACTIVITE', countingCode: samePair ? '002' : null
  }}) : null
  ...
  await Promise.all(shares.map((percentage, index) => prisma.meterAllocation.create({data: {
    sourceId: `${key}:${index}`, provider, scope, metadata: {contractId: `${index}`, lieuId: key}, compteurId: meter.id,
    exploitationId: targetExploitation?.id ?? (index > 0 && otherExploitation ? otherExploitation.id : exploitation.id),
  ...
  return {key, provider, scope, user, admin, usage, point, exploitation, otherExploitation, otherUser, otherPoint, meter, account, stream,
...
integration('scoped archive replay changes only selected streams and leaves other checkpoints and readings untouched', async t => {
  const selected = await fixture()
  const untouched = await fixture()
  await prisma.meterStream.update({where: {id: untouched.stream.id}, data: {
    serviceAccountId: selected.account.id, checkpoint: new Date('2026-07-05Z'), lastSuccessAt: new Date('2026-07-05Z'),
    blockedWindows: [{windowStart: '2026-07-01T00:00:00Z', windowEnd: '2026-07-16T00:00:00Z', fetchedAt: '2026-07-10T00:00:00Z', mode: 'LIVE'}]
  }})
  const before = await prisma.meterStream.findUnique({where: {id: untouched.stream.id}})
  const payload = {...batch(selected), streamIds: [selected.stream.id], preserveOrdinary: true}
  const result = await ingestMeterBatch(payload, selected.actor)
  t.deepEqual(result.streamIds, [selected.stream.id])
  t.is(result.counts.accepted, 2)
  t.deepEqual(await ingestMeterBatch(payload, selected.actor), result)
  t.deepEqual(await prisma.meterStream.findUnique({where: {id: untouched.stream.id}}), before)
  t.is(await prisma.meterReading.count({where: {compteurId: untouched.meter.id}}), 0)
  t.is(await prisma.meterPublication.count({where: {compteurId: untouched.meter.id}}), 0)
  const other = await fixture()
  const forbidden = {...payload, batchId: randomUUID(), streamIds: [selected.stream.id, other.stream.id]}
  t.is((await t.throwsAsync(ingestMeterBatch(forbidden, selected.actor))).status, 403)
  t.is(await prisma.meterIngestion.count({where: {batchId: forbidden.batchId}}), 0)
  const outside = {...payload, batchId: randomUUID(), readings: batch(untouched).readings}
  t.is((await t.throwsAsync(ingestMeterBatch(outside, selected.actor))).status, 400)
  t.is(await prisma.meterIngestion.count({where: {batchId: outside.batchId}}), 0)
})

integration('scoped batch identity sorts stream IDs and cannot be replayed with another selection', async t => {
  const first = await fixture()
  const second = await fixture()
  await prisma.meterStream.update({where: {id: second.stream.id}, data: {serviceAccountId: first.account.id}})
  const payload = {...batch(first), streamIds: [first.stream.id, second.stream.id]}
  const result = await ingestMeterBatch(payload, first.actor)
  t.deepEqual(await ingestMeterBatch({...payload, streamIds: [...payload.streamIds].reverse()}, first.actor), result)
  const {streamIds: _selection, ...unscoped} = payload
  t.is((await t.throwsAsync(ingestMeterBatch(unscoped, first.actor))).status, 409)
  t.is((await t.throwsAsync(ingestMeterBatch({...payload, streamIds: [first.stream.id]}, first.actor))).status, 409)
  t.is((await t.throwsAsync(ingestMeterBatch({...payload, batchId: randomUUID(), streamIds: []}, first.actor))).status, 400)
})

for (const samePair of [false, true]) {
  integration(`one physical meter distributes once to two exploitations (${samePair ? 'same owner and point, distinct codes' : 'distinct owners and points'})`, async t => {
    const f = await fixture({shares: [60, 40], splitExploitations: true, samePair})
    const payload = batch(f)
    const result = await ingestMeterBatch(payload, f.actor)
    t.is(result.counts.published, 1)
    const values = await prisma.chunkValue.findMany({where: {chunk: {compteurId: f.meter.id, instructionStatus: {not: 'REJECTED'}}}, include: {chunk: true}})
    t.is(values.length, 2)
    const byExploitation = new Map(values.map(value => [value.chunk.exploitationId, value]))
    t.is(byExploitation.get(f.exploitation.id).value.toString(), '6')
    t.is(byExploitation.get(f.otherExploitation.id).value.toString(), '4')
    t.is(byExploitation.get(f.otherExploitation.id).chunk.pointPrelevementId, f.otherPoint.id)
    t.deepEqual(await ingestMeterBatch(payload, f.actor), result)
    t.is(await prisma.meterReading.count({where: {compteurId: f.meter.id}}), 2)
...
integration('protected concurrent ingestions preserve fully covered ordinary volumes and bind protection to batch identity', async t => {
  const f = await fixture({shares: [100], supersedeSameMeter: true})
  const ordinary = await ordinaryVolume(f, f.meter.id)
  const payload = {...batch(f), preserveOrdinary: true}
  const [first, concurrent] = await Promise.all([ingestMeterBatch(payload, f.actor), ingestMeterBatch(payload, f.actor)])
  t.deepEqual(concurrent, first)
  t.true(first.preserveOrdinary)
  t.is(first.counts.accepted, 2)
  t.is(first.counts.published, 0)
  t.is(first.counts.conflicts, 1)
  t.deepEqual(await prisma.chunkValue.findUnique({where: {id: ordinary.row.id}}), ordinary.row)
  t.deepEqual(await prisma.chunk.findUnique({where: {id: ordinary.chunk.id}}), ordinary.chunk)
  t.deepEqual(await prisma.source.findUnique({where: {id: ordinary.source.id}}), ordinary.source)
  t.is(await prisma.chunkValueReplacement.count({where: {replacedChunkValueId: ordinary.row.id}}), 0)
  t.is((await activeValues(f)).length, 0)
  t.is(await prisma.meterIngestion.count({where: {serviceAccountId: f.account.id}}), 1)
  const durable = await prisma.meterIngestion.findUniqueOrThrow({where: {id: first.ingestionId}})
  t.true(durable.result.preserveOrdinary)
  t.deepEqual(durable.rawPayload, payload.readings)
  t.is((await prisma.meterStream.findUniqueOrThrow({where: {id: f.stream.id}})).lastIssue, 'ORDINARY_VOLUME_PRESERVED')
  t.deepEqual(await ingestMeterBatch(payload, f.actor), first)
  t.is((await t.throwsAsync(ingestMeterBatch({...payload, preserveOrdinary: false}, f.actor))).status, 409)
  const {preserveOrdinary: _preserveOrdinary, ...withoutProtection} = payload
  t.is((await t.throwsAsync(ingestMeterBatch(withoutProtection, f.actor))).status, 409)
  t.is((await t.throwsAsync(ingestMeterBatch({...payload, batchId: randomUUID(), preserveOrdinary: 'invalid'}, f.actor))).status, 400)
  // Protection is local to this batch, not an implicit stream/configuration edit.
  t.true((await prisma.meterStream.findUniqueOrThrow({where: {id: f.stream.id}})).supersedeSameMeter)
})

integration('unprotected ingestions retain the old hash, replacement behavior and false-or-omitted retry compatibility', async t => {
  const f = await fixture({shares: [100], supersedeSameMeter: true})
  const ordinary = await ordinaryVolume(f, f.meter.id)
  const payload = batch(f)
  const legacyHash = meterHash({provider: payload.provider, scope: payload.scope, mode: payload.mode,
    fetchedAt: payload.fetchedAt, windowStart: payload.windowStart, windowEnd: payload.windowEnd, readings: payload.readings})
  const result = await ingestMeterBatch(payload, f.actor)
  const durable = await prisma.meterIngestion.findUniqueOrThrow({where: {id: result.ingestionId}})
  t.is(durable.payloadHash, legacyHash)
  t.false(Object.hasOwn(durable.result, 'preserveOrdinary'))
  t.is(result.counts.published, 1)
  t.is(await prisma.chunkValue.count({where: {id: ordinary.row.id}}), 0)
  t.is(await prisma.chunkValueReplacement.count({where: {replacedChunkValueId: ordinary.row.id}}), 1)
  t.deepEqual(await ingestMeterBatch({...payload, preserveOrdinary: false}, f.actor), result)
  t.deepEqual(await ingestMeterBatch(payload, f.actor), result)
  t.is((await t.throwsAsync(ingestMeterBatch({...payload, preserveOrdinary: true}, f.actor))).status, 409)
  t.is(await prisma.meterIngestion.count({where: {serviceAccountId: f.account.id}}), 1)
})
