...
function routeApp({user, serviceAccount, impersonation} = {}) {
  ...
  app.use((req, res, next) => {
    if (user || serviceAccount) req.auth = {type: serviceAccount ? 'SERVICE_ACCOUNT_ACCESS' : 'USER_SESSION', impersonation}
    req.user = user
...
integration('dedicated HTTP routes enforce service-account, admin and exploitation permissions', async t => {
  ...
  t.is((await request(routeApp({user: f.admin})).post(adminPath).send(batch(f, {mode: 'OFFLINE'}))).status, 200)
