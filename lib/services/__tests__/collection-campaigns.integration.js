...
integration('HTTP campaign guards require human sessions, preserve the API alias and reject wrong owners or impersonation', async t => {
  const f = await fixture()
  const app = (actor, auth = actor ? {type: 'USER_SESSION', role: actor.role, user: actor} : undefined) => {
    const server = express()
    server.use(express.json())
    server.use((req, _res, next) => { req.auth = auth; req.user = actor; req.userRole = actor?.role; next() })
    const routes = createRoutes()
    server.use('/', routes)
    server.use('/api', routes)
    server.use((error, _req, res, _next) => res.status(error.status || 500).json({message: error.message}))
    return server
  }
  const summary = await request(app(f.farmer)).get('/api/campaigns/summary')
  t.is(summary.status, 200)
  t.true(summary.body.data.items.some(row => row.id === f.campaign.id))
  t.is((await request(app()).get('/campaigns/summary')).status, 401)
  t.is((await request(app({id: f.admin.id, role: 'INSTRUCTOR'})).get('/campaigns/summary')).status, 403)
  t.is((await request(app(f.admin, {type: 'SERVICE_ACCOUNT_ACCESS', role: 'ADMIN'})).get('/campaigns/summary')).status, 403)
  t.is((await request(app(f.other)).get(`/campaigns/${f.campaign.id}/responses/${f.response.id}`)).status, 404)
  t.is((await request(app(f.farmer, {type: 'USER_SESSION', impersonation: {actor: f.admin}})).put(`/campaigns/${f.campaign.id}/responses/${f.response.id}`).send({revision: 0, data: {}})).status, 403)
})
