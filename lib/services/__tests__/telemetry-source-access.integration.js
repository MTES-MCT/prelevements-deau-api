...
import {getSourceForInstructor, listSourcesForAdmin, listSourcesForInstructor} from '../instructor-sources.js'
...
integration('une publication partagée reste visible sans divulguer les autres bénéficiaires ni leurs totaux', async t => {
  await rollbackFixtures(async tx => {
    ...
    for (const [user, ids, expectedTotal, expectedChunks] of [
      ...
      t.like(visible.metadata, {periodStart: '2026-08-31T21:59:30.000Z', periodEnd: '2026-08-31T22:00:30.000Z'})
      ...
      t.like(feed.data[0].source.metadata, {periodStart: '2026-08-31T21:59:30.000Z', periodEnd: '2026-08-31T22:00:30.000Z'})
      t.true(feed.data[0].source.chunks.every(chunk => chunk.metadata.periodStart === '2026-08-31T21:59:30.000Z'
        && chunk.metadata.periodEnd === '2026-08-31T22:00:30.000Z'))
    ...
    t.like(instructor.metadata, {periodStart: '2026-08-31T21:59:30.000Z', periodEnd: '2026-08-31T22:00:30.000Z'})
    ...
    t.like(list.items[0].metadata, {periodStart: '2026-08-31T21:59:30.000Z', periodEnd: '2026-08-31T22:00:30.000Z'})
    // Both date columns are Aug 31 UTC, but the exact interval crosses Paris
    // midnight and must be found on Sep 1 too, without any value copy/backfill.
    const septemberList = await listSourcesForInstructor({zoneIds: [f.zones[0]], startDate: new Date('2026-09-01'), endDate: new Date('2026-09-01')}, {client: tx})
    t.true(septemberList.items.some(source => source.id === f.publication.sourceId))
    const outsideList = await listSourcesForInstructor({zoneIds: [f.zones[0]], startDate: new Date('2026-08-30'), endDate: new Date('2026-08-30')}, {client: tx})
    t.false(outsideList.items.some(source => source.id === f.publication.sourceId))
...
integration('la période et les métadonnées exactes ne peuvent provenir du chunk hors périmètre de la même source', async t => {
  await rollbackFixtures(async tx => {
    const f = await makeFixture(tx)
    const source = await tx.source.create({data: {type: 'API', status: 'COMPLETED', metadata: {
      calculationStrategy: 'METER', periodStart: '1900-01-01T00:00:00Z', periodEnd: '2100-01-01T00:00:00Z'
    }}})
    for (const [point, start, end] of [
      [f.points[0], '2026-09-02T20:00:15Z', '2026-09-02T22:00:00Z'],
      [f.points[1], '2026-09-02T23:00:15Z', '2026-09-03T01:00:25Z']
    ]) {
      await tx.chunk.create({data: {sourceId: source.id, calculationStrategy: 'METER', pointPrelevementId: point.id,
        preleveurUserId: f.owner.id, usageId: f.usage.id, minDate: new Date(start), maxDate: new Date(end),
        instructionStatus: 'AUTOMATICALLY_VALIDATED', metadata: {totalWaterVolumeWithdrawn: 1},
        chunkValues: {create: {metricTypeCode: 'volume', unit: 'm³', frequency: 'irregular', valueKind: 'COMPUTED',
          value: '1.0000', periodStart: new Date(start), periodEnd: new Date(end)}}}})
    }
    const filter = {startDate: new Date('2026-09-03'), endDate: new Date('2026-09-03')}
    const unauthorizedMatch = await listSourcesForInstructor({...filter, zoneIds: [f.zones[0]]}, {client: tx})
    t.false(unauthorizedMatch.items.some(item => item.id === source.id))
    const administrator = await listSourcesForAdmin(filter, {client: tx})
    t.true(administrator.items.some(item => item.id === source.id))
    const authorized = await listSourcesForInstructor({zoneIds: [f.zones[0]], startDate: new Date('2026-09-02'), endDate: new Date('2026-09-02')}, {client: tx})
    const visible = authorized.items.find(item => item.id === source.id)
    t.is(visible.chunks.length, 1)
    t.like(visible.metadata, {periodStart: '2026-09-02T20:00:15.000Z', periodEnd: '2026-09-02T22:00:00.000Z'})
    const detail = await getSourceForInstructor(source.id, {readZoneIds: [f.zones[0]]}, {client: tx})
    t.like(detail.metadata, {periodStart: '2026-09-02T20:00:15.000Z', periodEnd: '2026-09-02T22:00:00.000Z'})
    t.like(detail.chunks[0].metadata, {periodStart: '2026-09-02T20:00:15.000Z', periodEnd: '2026-09-02T22:00:00.000Z'})
  })
})
