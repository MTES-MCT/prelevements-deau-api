...
function listClient(user, {partial = false, status = 'OPEN'} = {}) {
  ...
  Object.assign(client, {
    ...
    instructorZone: {async findMany(query) {
      return query.where.permissions.some.permission === 'campaign.read' ? [{zoneId: id(50)}] : []
    }},
...
test('listCampaigns ajoute progress sans doubler les lectures de détail ni modifier le contrat existant', async t => {
  ...
  t.deepEqual(detailReads, [id(10), id(11)])
  t.is(client.calls.length, 1)
  t.true(result.items.every(item => item.campaign && item.permissions && item.targets && item.managerOptions))
