...
import {
  aggregateGroundwaterValuesForPeriod,
  aggregateGroundwaterValuesByUTCWeek,
  buildPiezometryIpsMonthlyQuery,
...
test('les périodes du tableau de bord sont glissantes', t => {
  ...
  t.is(periodBounds('five-years', now).start.toISOString(), '2021-07-10T14:30:00.000Z')
  t.is(periodBounds('ten-years', now).start.toISOString(), '2016-07-10T14:30:00.000Z')
...
test('les vues historiques utilisent toutes la moyenne hebdomadaire', t => {
  const values = [
    {at: new Date('2026-07-07T08:00:00.000Z'), levelNgf: 120, depth: 7},
    {at: new Date('2026-07-08T08:00:00.000Z'), levelNgf: 122, depth: 9}
  ]

  for (const period of ['five-years', 'ten-years', 'twenty-years']) {
    const aggregated = aggregateGroundwaterValuesForPeriod(values, period)
    t.is(aggregated.length, 1)
    t.is(aggregated[0].aggregation, 'WEEKLY_MEAN')
    t.is(aggregated[0].levelNgf, 121)
    t.is(aggregated[0].depth, 8)
  }

  t.is(aggregateGroundwaterValuesForPeriod(values, 'year'), values)
})

test('la requête IPS agrège d’abord par jour puis par mois', async t => {
  const start = new Date('1996-07-10T00:00:00.000Z')
  const end = new Date('2026-07-10T00:00:00.000Z')
  const query = buildPiezometryIpsMonthlyQuery({
    end,
    start,
    stationIds: [STATION_ID]
  })
  let receivedQuery
  const rows = [{monitoringStationId: STATION_ID, levelNgf: 12}]
  const result = await listPiezometryIpsMonthlyValues({
    client: {
  ...
  t.is(result, rows)
  t.regex(query.sql, /WITH daily_values AS/)
  t.regex(query.sql, /GROUP BY observation\."monitoringStationId", observation\."measurementDate"/)
  t.regex(query.sql, /date_trunc\('month', daily_values\.day\)/)
  t.regex(query.sql, /count\(daily_values\."levelNgf"\)::int/)
  t.deepEqual(receivedQuery.values, query.values)
  t.true(query.values.includes(STATION_ID))
  t.true(query.values.some(value => value instanceof Date && value.getTime() === start.getTime()))
  t.true(query.values.some(value => value instanceof Date && value.getTime() === end.getTime()))
})
