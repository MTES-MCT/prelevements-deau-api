...
integration('campaign refuses future index submissions and incomplete or decreasing readings without any writes', async t => {
  ...
  const error = await t.throwsAsync(submit(f, 0, {data: decreasing}))
...
integration('a conflicting historical index identifies its exact field and never replaces existing data', async t => {
  ...
  const source = await prisma.source.create({data: {type: 'BATCH', status: 'COMPLETED', globalInstructionStatus: 'VALIDATED', chunks: {create: {
    ...
    minDate: new Date('2026-06-01Z'), maxDate: new Date('2026-06-01Z'), chunkValues: {create: {
      metricTypeCode: 'index', value: '225', unit: 'm³', frequency: 'instant',
      periodStart: new Date('2026-06-01Z'), periodEnd: new Date('2026-06-01T00:15:00Z'), valueKind: 'DECLARED'
    }}
