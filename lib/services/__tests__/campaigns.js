...
import {assertCampaignOpeningTargets, changeCampaignStatus, mergeCampaignDraft, normalizePublishedCampaignDraft, serializeCampaign, getCampaignResponseHistory} from '../campaigns.js'
...
function statusFixture(overrides = {}) {
  const {campaign: base, targets} = openingFixture()
  const writes = []
  const campaign = {
    ...base, id: 'campaign', version: 0, status: 'DRAFT', periods: [], managers: [],
    targets: targets.map(target => ({...target, preleveurUserId: 'farmer', preleveur: {userId: 'farmer', user: {}}, meters: []})),
    ...overrides
  }
  const client = {
    ...
    campaign: {
      ...
      async updateMany(query) {
        writes.push({type: 'status', ...query})
        Object.assign(campaign, query.data, {version: campaign.version + 1})
        return {count: 1}
      }
    },
    ...
    campaignTargetMeter: {
      async deleteMany(query) {
        writes.push({type: 'meter-delete', ...query})
      },
      async createMany(query) {
        writes.push({type: 'meter-create', ...query})
      }
    },
    campaignNotification: {async createMany(query) {
      writes.push({type: 'notification', ...query})
    }},
    declarant: {async findMany() {
      return []
    }}
  }
  return {client, campaign, writes}
}

for (const overrides of [
  {opensAt: '2026-09-10T09:00:00.001Z'},
  {closesAt: '2026-09-10T09:00:00.000Z'},
  {closesAt: '2026-09-10T08:59:59.999Z'}
]) {
  test(`la campagne ne s’ouvre pas hors de sa fenêtre de réponse : ${JSON.stringify(overrides)}`, async t => {
    const {client, writes} = statusFixture(overrides)
    const error = await t.throwsAsync(() => changeCampaignStatus({id: 'admin', role: 'ADMIN'}, 'campaign', 'OPEN', {expectedVersion: 0}, {client, now: new Date('2026-09-10T09:00:00Z')}))
    t.is(error.statusCode, 409)
    t.deepEqual(writes, [])
  })
}

test('l’ouverture à la borne incluse utilise le même instant et crée une invitation après la transition', async t => {
  const now = new Date('2026-09-10T09:00:00Z')
  const {client, writes} = statusFixture({opensAt: now, closesAt: new Date(now.getTime() + 1)})
  const result = await changeCampaignStatus({id: 'admin', role: 'ADMIN'}, 'campaign', 'OPEN', {expectedVersion: 0}, {client, now})
  t.is(result.campaign.status, 'OPEN')
  t.is(result.campaign.openedAt, now)
  const status = writes.findIndex(write => write.type === 'status')
  const notification = writes.findIndex(write => write.type === 'notification')
  t.true(notification > status)
  t.is(writes[notification].data.length, 1)
  t.is(writes[notification].data[0].kind, 'OPENING')
})

test('la clôture volontaire garde l’instant injecté sans déclencher une nouvelle invitation', async t => {
  const now = new Date('2026-09-10T09:00:00Z')
  const {client, writes} = statusFixture({status: 'OPEN'})
  const result = await changeCampaignStatus({id: 'admin', role: 'ADMIN'}, 'campaign', 'CLOSED', {expectedVersion: 0}, {client, now})
  t.is(result.campaign.status, 'CLOSED')
  t.is(result.campaign.closedAt, now)
  t.false(writes.some(write => write.type === 'notification'))
})
