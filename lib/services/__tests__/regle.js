...
test.beforeEach(async () => {
  await mongo.db.collection('regles').deleteMany({})
  ...
  await mongo.db.collection('sequences').deleteMany({})
})

test.serial('createRegle / crée une règle valide', async t => {
  // Créer un préleveur
  const preleveur = await PreleveurModel.insertPreleveur('TEST-001', {
    nom: 'Préleveur Test',
    statut: 'En activité'
  })
  ...
  t.is(regle.parametre, 'Volume journalier')
