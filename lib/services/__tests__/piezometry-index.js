...
import {
  aggregatePiezometryMonthlyValues,
  buildPiezometryIps,
...
test('les agrégats mensuels conservent la moyenne des moyennes journalières', t => {
  const rows = [
    groundwaterRow('2026-01-01', 10, 30),
    groundwaterRow('2026-01-01', 14, null),
    groundwaterRow('2026-01-02', 20, 20)
  ]
  const monthlyValues = aggregatePiezometryMonthlyValues(rows)

  t.deepEqual(monthlyValues, [{
    at: new Date('2026-01-01T00:00:00.000Z'),
    depth: 25,
    levelNgf: 16,
    depthObservationDays: 2,
    levelNgfObservationDays: 2
  }])
})

test('l’IPS produit le même contrat depuis les lignes brutes ou mensuelles', t => {
  const rows = []

  for (let year = 2006; year <= 2026; year += 1) {
    rows.push(
      groundwaterRow(`${year}-01-01`, 100 + (year - 2006)),
      groundwaterRow(`${year}-01-02`, 102 + (year - 2006))
    )
  }
  ...
  t.deepEqual(
    buildPiezometryIpsFromMonthlyValues(
      aggregatePiezometryMonthlyValues(rows),
      options
    ),
    buildPiezometryIps(rows, options)
  )
})
