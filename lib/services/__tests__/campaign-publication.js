...
import {publishCampaignIndexSubmission, planCampaignVolumePublication, prepareCampaignIndexPublication} from '../campaign-publication.js'
...
function fakeClient({conflicts = [], existingReadings = []} = {}) {
  const state = {conflicts, existingReadings, sources: [], chunks: [], coverages: [], audits: [], deleted: [], publication: null, locks: 0, readingQueries: 0}
  const client = {
    state,
    async $executeRaw() {
      state.locks++
...
test('publication crée index canoniques et volumes COMPUTED exacts, rejouable sans doublon', async t => {
  ...
  client.chunkValue.findMany = async () => {
    throw new Error('Un rejeu déjà publié ne doit pas recalculer des sources historiques.')
  }
...
test('le remplacement conserve la saisie hors phase sans la publier ni corriger sa source historique', async t => {
  const event = {targetId: target.id, type: 'REPLACEMENT', at: '2026-01-01', previousCompteurId: 'meter', nextCompteurId: 'new-meter', previousIndex: '50', nextIndex: '0', reason: 'Compteur remplacé'}
  const targets = [{...target, meters: [...target.meters, {compteurId: 'new-meter', startDate: event.at}]}]
  const inactive = {...readings[2], value: '9999', correctionOfChunkValueId: 'old-index', sourceValueUpdatedAt: '2020-01-01', correctionReason: 'Ancienne saisie corrigée avant le remplacement'}
  const snapshot = {readings: [readings[0], ...readings.slice(1).map(reading => ({...reading, compteurId: 'new-meter'})), inactive], meterEvents: [event]}
  const original = structuredClone(snapshot)
  const client = fakeClient()
  const preparation = await prepareCampaignIndexPublication({campaign, targets, snapshot, client})
  const publication = await publishCampaignIndexSubmission({submission: {...submission, snapshot: preparation.officialSnapshot}, campaign, targets, actorUserId: 'actor', client, preparation})
  t.deepEqual(snapshot, original)
  t.deepEqual(publication.totals.map(row => row.value), ['150', '100'])
  t.deepEqual(preparation.calculation.ignoredReadings, [{...inactive, code: 'READING_OUTSIDE_METER_PERIOD'}])
  t.false(preparation.officialSnapshot.readings.some(reading => reading.value === '9999'))
  t.false(Object.hasOwn(publication, 'ignoredReadings'))
  t.false(publication.readingReferences.some(reading => reading.compteurId === 'meter' && reading.readingDate === readings[2].readingDate))
  t.false(client.state.chunks.flatMap(chunk => chunk.chunkValues.create).some(value => value.value === '9999'))
  t.deepEqual(client.state.audits, [])
  t.is(client.state.readingQueries, 1)
  t.is(client.state.locks, 1)
  const draft = normalizePublishedCampaignDraft(snapshot, publication)
  t.deepEqual(draft.readings.at(-1), inactive)
  ...
  client.state.existingReadings = client.state.chunks.filter(chunk => !chunk.metadata.campaignMeterEvent).flatMap(chunk => chunk.chunkValues.create
    .filter(value => value.metricTypeCode === 'index').map(value => ({...value, chunk: {
      pointPrelevementId: target.pointPrelevementId, preleveurUserId: target.preleveurUserId, compteurId: chunk.compteurId,
      instructionStatus: 'VALIDATED', source: {status: 'COMPLETED'}
    }})))
  const second = await publishCampaignIndexSubmission({submission: {...submission, id: 'second-submission', snapshot: {...draft, comment: 'Précision'}}, campaign, targets, actorUserId: 'actor', client})
  t.deepEqual(second.totals.map(row => row.value), ['150', '100'])
  t.deepEqual(second.readingReferences, publication.readingReferences)
  t.false(client.state.chunks.flatMap(chunk => chunk.chunkValues.create).some(value => value.value === '9999'))
  t.deepEqual(client.state.audits, [])
})
...
test('sans inventaire, publication et révision conservent les index par point sans créer de compteur', async t => {
  ...
  t.true(draft.readings.every(reading => !Object.hasOwn(reading, 'meterConfirmed') && reading.sourceChunkValueId))
...
test('une source historique sans identité ne peut être attribuée à un vrai compteur sans confirmation explicite', async t => {
  const source = {
    id: 'old-index', valueKind: 'DECLARED', metricTypeCode: 'index', value: '0', updatedAt: new Date('2026-01-01'), periodStart: new Date('2025-10-31'),
    chunk: {pointPrelevementId: 'point', preleveurUserId: 'user', compteurId: null, instructionStatus: 'VALIDATED', source: {status: 'COMPLETED'}}
  }
  const client = fakeClient({existingReadings: [source]})
  const snapshot = {readings: [{...readings[0], sourceChunkValueId: source.id, sourceValueUpdatedAt: source.updatedAt}, ...readings.slice(1)]}
  const error = await t.throwsAsync(() => publishCampaignIndexSubmission({submission: {...submission, snapshot}, campaign, targets: [target], actorUserId: 'actor', client}))
  t.is(error.statusCode, 409)
  t.true(error.issues.some(issue => issue.code === 'AMBIGUOUS_HISTORICAL_METER'))
  t.deepEqual(client.state.sources, [])
  t.deepEqual(client.state.chunks, [])
  t.deepEqual(client.state.coverages, [])
  const confirmed = {...snapshot, readings: snapshot.readings.map((reading, index) => index ? reading : {...reading, meterConfirmed: true})}
  const publication = await publishCampaignIndexSubmission({submission: {...submission, snapshot: confirmed}, campaign, targets: [target], actorUserId: 'actor', client})
  t.true(publication.readingReferences.some(reference => reference.chunkValueId === source.id))
})

test('le remplacement sans ancien inventaire publie deux identités distinctes et réutilise les transitions au rejeu', async t => {
  const client = fakeClient()
  const replacementTarget = {...target, meters: [{compteurId: 'new-meter', startDate: '2026-01-01'}]}
  const snapshot = {readings: readings.map((reading, index) => ({...reading, compteurId: index ? 'new-meter' : null})), meterEvents: [{
    targetId: target.id, type: 'REPLACEMENT', at: '2026-01-01', previousCompteurId: null, nextCompteurId: 'new-meter',
    previousIndex: '50', nextIndex: '0', reason: 'Premier compteur référencé après remplacement'
  }]}
  const publication = await publishCampaignIndexSubmission({submission: {...submission, snapshot}, campaign, targets: [replacementTarget], actorUserId: 'actor', client})
  t.deepEqual(publication.totals.map(row => row.value), ['150', '100'])
  t.is(publication.meterEventReferences[0].previous.compteurId, null)
  t.is(publication.meterEventReferences[0].next.compteurId, 'new-meter')
  const eventChunks = client.state.chunks.filter(chunk => chunk.metadata.campaignMeterEvent)
  t.deepEqual(eventChunks.map(chunk => chunk.compteurId), [null, 'new-meter'])
  t.deepEqual(eventChunks.map(chunk => chunk.metadata.campaignMeterEvent.phase), ['previous', 'next'])
  client.state.existingReadings = client.state.chunks.filter(chunk => !chunk.metadata.campaignMeterEvent).flatMap(chunk => chunk.chunkValues.create
    .filter(value => value.metricTypeCode === 'index')
    .map(value => ({...value, chunkId: chunk.id, chunk: {...chunk, source: {status: 'COMPLETED'}}})))
  const draft = normalizePublishedCampaignDraft(snapshot, publication)
  t.is(draft.readings[0].compteurId, null)
  t.truthy(draft.readings[0].sourceChunkValueId)
  const revision = await publishCampaignIndexSubmission({
    submission: {...submission, id: 'revision', snapshot: {...draft, comment: 'Précision'}}, campaign, targets: [replacementTarget], actorUserId: 'actor', client
  })
  t.deepEqual(revision.totals.map(row => row.value), ['150', '100'])
  t.is(client.state.chunks.flatMap(chunk => chunk.chunkValues.create).filter(value => value.metricTypeCode === 'index').length, 5)
})

test('une remise à zéro non référencée conserve les deux index de transition null sans compter leur différence', async t => {
  const client = fakeClient()
  const snapshot = {readings: readings.map(reading => ({...reading, compteurId: null})), meterEvents: [{
    targetId: target.id, type: 'RESET', at: '2026-01-01', previousCompteurId: null, nextCompteurId: null,
    previousIndex: '50', nextIndex: '0', reason: 'Remise à zéro'
  }]}
  const publication = await publishCampaignIndexSubmission({submission: {...submission, snapshot}, campaign, targets: [{...target, meters: []}], actorUserId: 'actor', client})
  t.deepEqual(publication.totals.map(row => row.value), ['150', '100'])
  t.is(publication.meterEventReferences[0].previous.compteurId, null)
  t.is(publication.meterEventReferences[0].next.compteurId, null)
  const eventChunks = client.state.chunks.filter(chunk => chunk.metadata.campaignMeterEvent)
  t.true(eventChunks.every(chunk => chunk.compteurId === null))
  t.deepEqual(eventChunks.flatMap(chunk => chunk.chunkValues.create.map(value => value.value)), ['50', '0'])
})
