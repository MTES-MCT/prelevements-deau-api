...
test('le détail du brouillon projette les changements déjà transmis sans modifier les affectations historiques', async t => {
  const campaign = campaignFixture()
  campaign.status = 'DRAFT'
  campaign.targets = [campaign.targets[0]]
  campaign.targets[0].pointPrelevementId = 'point'
  const bindings = [
    {id: 'old-binding', pointPrelevementId: 'point', compteurId: 'old', compteur: {}, startDate: null, endDate: null},
    {id: 'new-binding', pointPrelevementId: 'point', compteurId: 'new', compteur: {}, startDate: new Date('2026-06-01'), endDate: null}
  ]
  const database = client(campaign)
  database.compteurPointPrelevement = {async findMany() {
    return bindings
  }}
  const queries = []
  database.campaignResponse = {async findMany(query) {
    queries.push(query)
    return [{
      campaign: {targets: [{id: 'earlier-target', pointPrelevementId: 'point'}]},
      latestSubmission: {snapshot: {meterEvents: [{targetId: 'earlier-target', type: 'REPLACEMENT', at: '2026-06-01', previousCompteurId: 'old', nextCompteurId: 'new'}]}}
    }]
  }}
  const access = await getCampaignAccess({id: 'admin', role: 'ADMIN'}, campaign.id, {client: database})
  t.is(access.targets[0].meters[0].endDate.toISOString(), '2026-06-01T00:00:00.000Z')
  t.is(bindings[0].endDate, null)
  t.is(queries.length, 1)
  t.deepEqual(queries[0].where.latestSubmissionId, {not: null})
  t.false(Object.hasOwn(queries[0].select, 'draft'))
})
