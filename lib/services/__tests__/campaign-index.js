...
test('sans inventaire, le calcul par point exige la continuité explicitement confirmée', t => {
  ...
  t.false(unconfirmed.canSubmit)
  t.true(unconfirmed.issues.some(issue => issue.code === 'METER_CONTINUITY_CONFIRMATION_REQUIRED'))
  t.true(unconfirmed.totals.every(total => total.value === null))
...
test('historique sans compteur : aucune reprise automatique même si valeur et confirmation coïncident', t => {
  const targets = [{...target, meters: []}]
  const meterless = readings.map(reading => ({...reading, compteurId: null, meterConfirmed: true}))
  ...
  t.false(calculate({targets, readings: [{...referenced[0], meterConfirmed: false}, ...referenced.slice(1)], existingReadings: [old]}).canSubmit)
