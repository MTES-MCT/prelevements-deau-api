...
import {
  ...
  buildMissingDeclarationsWorkbook,
  ...
  const client = {
    ...
    chunk: {
      findMany: async () => [
        {
          pointPrelevementId: 'point-1',
          preleveurUserId: 'preleveur-1',
          source: {declaration: {declarantUserId: 'preleveur-1', createdByDeclarantUserId: 'collecteur-1'}}
        }
      ]
    }
  }
...
test('une relance distingue deux codes comptage du même préleveur sur le même point', async t => {
  let chunkQuery
  const first = buildExploitation({countingCode: '001'})
  const second = buildExploitation({id: 'exploitation-2', countingCode: '002'})
  const client = {
    declarantPointPrelevement: {findMany: async () => [first, second]},
    chunk: {
      async findMany(query) {
        chunkQuery = query
        return [{pointPrelevementId: 'point-1', preleveurUserId: 'preleveur-1', exploitationId: first.id}]
      }
    }
  }
  const preview = await computeDeclarationNotificationRecipients({
    notificationType: 'followup', periodType: 'month', periodKey: '2026-06', client
  })

  t.is(chunkQuery.where.source.type, 'DECLARATION')
  t.is(preview.recipients.length, 2)
  for (const recipient of preview.recipients) {
    t.deepEqual(recipient.points.map(point => point.exploitationId), [second.id])
    t.is(recipient.points[0].countingCode, '002')
    t.true(recipient.templateParams.POINTS.includes('comptage 002'))
  }
  t.deepEqual(preview.exclusions.filter(exclusion => exclusion.reason === 'ALREADY_DECLARED').map(exclusion => exclusion.exploitationId), [first.id])
})

test('un ancien chunk ambigu ne dispense aucun des deux comptages et le rappel ne les fusionne pas', async t => {
  const client = {
    declarantPointPrelevement: {findMany: async () => [
      buildExploitation({countingCode: '001'}),
      buildExploitation({id: 'exploitation-2', countingCode: '002'})
    ]},
    chunk: {findMany: async () => [{pointPrelevementId: 'point-1', preleveurUserId: 'preleveur-1', exploitationId: null}]}
  }
  const preview = await computeDeclarationNotificationRecipients({
    notificationType: 'followup', periodType: 'month', periodKey: '2026-06', client
  })

  t.false(preview.exclusions.some(exclusion => exclusion.reason === 'ALREADY_DECLARED'))
  t.is(preview.recipients.length, 2)
  for (const recipient of preview.recipients) {
    t.deepEqual(recipient.points.map(point => point.countingCode), ['001', '002'])
  }
})

test('l’export des relances conserve les deux codes comptage du destinataire', async t => {
  const client = {
    declarantPointPrelevement: {findMany: async () => [
      buildExploitation({countingCode: '001'}),
      buildExploitation({id: 'exploitation-2', countingCode: '002'})
    ]},
    chunk: {findMany: async () => []}
  }
  const {workbook} = await buildMissingDeclarationsWorkbook({periodType: 'month', periodKey: '2026-06', client})
  const sheet = workbook.getWorksheet('Non déclarants')
  t.is(sheet.getCell('H1').value, 'Codes comptage')
  t.is(sheet.getCell('H2').value, 'Point test : 001, Point test : 002')
})
...
test('computeDeclarationNotificationRecipients utilise les contacts sans envoyer au login ni aux alias', async t => {
  const exploitation = buildExploitation()
  exploitation.declarant.contactEmails = [
    {email: 'contact-principal@example.fr', isPrimary: true},
    {email: 'contact-secondaire@example.fr', isPrimary: false}
  ]
  exploitation.declarant.user.emailAliases = [{email: 'alias-login@example.fr'}]
  exploitation.collecteurs = []
  const client = {
    declarantPointPrelevement: {
      findMany: async () => [exploitation]
    },
  ...
  t.deepEqual(preview.recipients.map(recipient => recipient.email), [
    'contact-principal@example.fr',
    'contact-secondaire@example.fr'
  ])
  t.false(preview.recipients.some(recipient => recipient.email === 'preleveur@example.fr'))
  t.false(preview.recipients.some(recipient => recipient.email === 'alias-login@example.fr'))
})

test('computeDeclarationNotificationRecipients exclut les comptes supprimés de la requête', async t => {
  let query
  const client = {
    declarantPointPrelevement: {
  ...
  await computeDeclarationNotificationRecipients({
    notificationType: 'followup',
    periodType: 'month',
    periodKey: '2026-06',
    client
  ...
  t.deepEqual(query.where.declarant, {user: {deletedAt: null}})
  t.deepEqual(query.include.collecteurs.where, {
    collecteur: {user: {deletedAt: null}}
  })
})

test('computeDeclarationNotificationRecipients expose les usages secondaires sans dupliquer le point', async t => {
  const client = {
    declarantPointPrelevement: {
      findMany: async () => [buildExploitation({
        usage: {id: 'usage-5', code: '5', label: 'Irrigation'},
        secondaryUsageLinks: [{
          usage: {id: 'usage-2', code: '2', label: 'Industrie'}
        }]
      })]
    },
    chunk: {
  ...
  t.true(preview.recipients.length > 0)
  t.is(preview.recipients[0].points.length, 1)
  t.is(preview.recipients[0].points[0].usage.name, 'Irrigation')
  t.deepEqual(preview.recipients[0].points[0].secondaryUsages, [{
    id: 'usage-2',
    code: '2',
    name: 'Industrie'
  }])
