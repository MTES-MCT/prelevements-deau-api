...
test('getPointInfo accepte un pointId string', async t => {
  const p1 = new ObjectId()
  await seedPoints([
    {_id: p1, id_point: 55, nom: 'Point String'}
  ])

  const info = await getPointInfo(p1.toString())

  t.is(info._id.toString(), p1.toString())
  t.is(info.id_point, 55)
  t.is(info.nom, 'Point String')
})
