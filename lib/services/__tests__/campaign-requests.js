...
test('le résumé du tableau de bord sélectionne les demandes actionnables avant de limiter la page', async t => {
  const campaign = campaignFixture()
  const deadline = aroundNow(2)
  const {client, calls} = fixture({campaign, page: [{campaignId: campaign.id, preleveurUserId: farmer.id, total: 9, deadlineAt: deadline}]})
  const result = await listCampaignRequests(farmer, {actionableOnly: true, limit: 3, client})
  t.is(result.total, 9)
  t.is(result.items.length, 1)
  t.is(result.items[0].deadlineAt, deadline.toISOString())
  t.deepEqual(result.pagination, {limit: 3, hasMore: true, nextCursor: null})
  const {sql, values} = calls.find(call => call.method === 'page')
  t.true(sql.includes('response.status IS DISTINCT FROM \'SUBMITTED\''))
  t.true(sql.includes('OR response."reopenUntil" >'))
  t.true(sql.includes('campaign."opensAt" <= '))
  t.true(sql.includes('campaign."closesAt" > '))
  t.true(sql.includes('COUNT(*) OVER()::integer AS total'))
  t.true(sql.includes('ORDER BY "deadlineAt" ASC NULLS LAST'))
  t.true(sql.indexOf('response.status IS DISTINCT') < sql.indexOf('LIMIT'))
  t.is(values.at(-1), 3)
  t.is(calls.length, 5)
})

test('la sélection prioritaire reproduit les critères des points inscriptibles et les seuls mandats', async t => {
  const {client, calls} = fixture({actorRole: 'COLLECTEUR'})
  const {items: [item]} = await listCampaignRequests(collector, {actionableOnly: true, limit: 3, client})
  t.is(item.pointCount, 1)
  t.true(item.responses.INDEX.canEdit)
  t.false(item.responses.INDEX.canSubmit)
  const {sql, values} = calls.find(call => call.method === 'page')
  for (const clause of [
    ...
    'preleveur."deletedAt" IS NULL',
    'point."collectionMode" IS DISTINCT FROM \'EXTERNAL\'',
    'exploitation.status NOT IN (\'ABANDONNEE\', \'TERMINEE\')',
    'exploitation."startDate"::text <= dates.first',
    'exploitation."endDate"::text >= dates.last',
    'jsonb_array_elements_text(campaign."indexDates")',
    'DeclarantCollecteurExploitation'
  ]) {
    t.true(sql.includes(clause), `Clause attendue : ${clause}`)
  }

  t.true(values.includes(collector.id))
  t.false(sql.includes(collector.id))
  t.false(sql.includes('CampaignManager'))
  t.false(sql.includes('ownerCollecteurUserId'))
  const selected = calls.find(call => call.method === 'campaigns').query.select.targets.select.pointPrelevement.select
  t.deepEqual(selected, {collectionMode: true, deletedAt: true})
})

test('une demande entièrement transmise est retirée même si les droits de saisie existent encore', async t => {
  const {client} = fixture({responses: ['INDEX', 'NEEDS'].map(kind => ({kind, status: 'SUBMITTED'}))})
  const result = await listCampaignRequests(farmer, {actionableOnly: true, client})
  t.deepEqual(result.items, [])
})

test('le résumé ne conserve que le volet réellement réouvert', async t => {
  const campaign = campaignFixture()
  campaign.status = 'CLOSED'
  campaign.closesAt = aroundNow(-1)
  const {client} = fixture({campaign, responses: [{kind: 'NEEDS', status: 'DRAFT', reopenUntil: aroundNow(1)}]})
  const {items: [item]} = await listCampaignRequests(farmer, {actionableOnly: true, client})
  t.true(item.responses.NEEDS.canEdit)
  t.false(item.responses.INDEX.canEdit)
})

test('le résumé revérifie les dates et les droits après sa sélection SQL', async t => {
  for (const update of [
    campaign => {
      campaign.opensAt = aroundNow(1)
    },
    ...
    campaign => {
      for (const target of campaign.targets) {
        target.exploitation.status = 'TERMINEE'
      }
    },
    campaign => {
      for (const target of campaign.targets) {
        target.exploitation.startDate = new Date('2026-06-01')
      }
    },
    campaign => {
      for (const target of campaign.targets) {
        target.pointPrelevement.collectionMode = 'EXTERNAL'
      }
    },
    campaign => {
      for (const target of campaign.targets) {
...
test('un résumé vide reste léger et le mode refuse les curseurs de la liste historique', async t => {
  const {client, calls} = fixture({page: []})
  const result = await listCampaignRequests(farmer, {actionableOnly: true, limit: 3, client})
  t.deepEqual(result, {items: [], total: 0, pagination: {limit: 3, hasMore: false, nextCursor: null}})
  t.is(calls.length, 2)
  const error = await t.throwsAsync(() => listCampaignRequests(farmer, {actionableOnly: true, cursor: 'historical-cursor', client}))
  t.is(error.statusCode, 400)
  const invalid = await t.throwsAsync(() => listCampaignRequests(farmer, {actionableOnly: 'invalid', client}))
