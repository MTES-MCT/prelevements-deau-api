...
test('le cache reste dédié après changement de l’hôte BullMQ et ne reçoit pas son override', t => {
  const environment = {
    APP_ENV: 'testing',
    REDIS_URL: 'rediss://user:secret@bullmq-public.test:6379/1',
    REDIS_HOST_OVERRIDE: '172.16.16.4',
    REDIS_TLS_IDENTITY: '51.15.243.75'
  }
  t.false(isSearchCorpusCacheEnabled(environment))
  t.false(isSearchCorpusCacheEnabled({...environment, SEARCH_CACHE_REDIS_URL: 'rediss://172.16.16.4:6379/9'}))
  t.false(isSearchCorpusCacheEnabled({...environment, SEARCH_CACHE_REDIS_URL: 'rediss://bullmq-public.test:6379/9'}))
  t.true(isSearchCorpusCacheEnabled({...environment, SEARCH_CACHE_REDIS_URL: 'rediss://dedicated-cache.test:6379'}))

  const client = getDedicatedSearchCacheRedis({
    ...environment,
    SEARCH_CACHE_REDIS_URL: 'rediss://cache-user:cache-password@dedicated-cache.test:6380/9'
  })
  t.teardown(() => client.disconnect(false))
  t.is(client.options.host, 'dedicated-cache.test')
  t.is(client.options.port, 6380)
  t.is(client.options.db, 9)
  t.is(client.options.username, 'cache-user')
  t.is(client.options.password, 'cache-password')
  t.is(client.options.tls?.checkServerIdentity, undefined)
})
...
test('le signal d’activité ne vide pas le cache de recherche, contrairement aux mutations métier', async t => {
  let invalidations = 0
  const middleware = createSearchCacheInvalidationMiddleware({
    cache: {
      isEnabled: () => true,
  ...
  const app = express()
  app.use(middleware)
  app.post(['/users/me/activity', '/api/users/me/activity'], (_req, res) => res.json({month: '2026-09'}))
  app.patch('/users/me/profile', (_req, res) => res.json({ok: true}))

  await request(app).post('/users/me/activity').expect(200)
  await request(app).post('/api/users/me/activity').expect(200)
  await request(app).post('/users/me/activity/').expect(200)
  t.is(invalidations, 0)

  await request(app).patch('/users/me/profile').expect(200)
