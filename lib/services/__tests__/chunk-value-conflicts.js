...
for (const requestedPolicy of ['REPLACE_EXISTING', 'SKIP_NEW_CHUNK', 'SKIP_CONFLICTING_VALUES', 'REPLACE_EXISTING_EXCEPT_WILLIE']) {
  test(`${requestedPolicy} refuse de modifier une publication METER`, async t => {
    const may = monthPeriod(2025, 4)
    let locked = false
    const error = await t.throwsAsync(applyConflictPolicyForIncomingChunkValues({
      pointPrelevementId: 'point', requestedPolicy,
      valueRows: [{metricTypeCode: 'volume', periodStart: may.start, periodEnd: may.end}],
      client: {async $executeRaw() {
        locked = true
      }},
      findConflictingChunkValues: async () => [{calculationStrategy: 'METER'}]
    }))

    t.true(locked)
    t.is(error.statusCode, 409)
    t.regex(error.message, /compteur protégée/)
  })
}

test('les valeurs ponctuelles détectent aussi le chevauchement avec un intervalle METER exact', async t => {
  let query
  await findConflictingChunkValuesForIncomingChunkValues({
    pointPrelevementId: 'point',
    valueRows: [{metricTypeCode: 'volume', periodStart: new Date('2026-01-01'), periodEnd: new Date('2026-01-01T00:15:00Z')}],
    client: {async $queryRaw(strings, ...values) {
      query = strings.join('') + values.map(value => value?.sql ?? '').join('')
      return []
    }}
  })
  t.true(query.includes('c."calculationStrategy" = \'METER\''))
  t.true(query.includes('cv."periodStart" < inc."periodEnd"'))
})
