...
test('compareSeries: handles all unchanged series', t => {
  const existingSeries = [
    {_id: new ObjectId(), hash: 'abc123'},
    {_id: new ObjectId(), hash: 'def456'}
  ]
  const newSeries = [
    {hash: 'abc123', data: []},
    {hash: 'def456', data: []}
...
test('compareSeries: handles duplicate hashes in new series', t => {
  const existingSeries = [
    {_id: new ObjectId(), hash: 'abc123'}
  ]
  const newSeries = [
...
test('compareSeries: handles series replacement (different hash, same position)', t => {
  const existingSeries = [
    {_id: new ObjectId(), hash: 'abc123'}
  ]
  const newSeries = [
...
test('compareSeries: handles empty inputs', t => {
  const result = SerieService.compareSeries([], [])
