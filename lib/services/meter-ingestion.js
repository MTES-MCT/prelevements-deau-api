...
export function authorizedMeterStreamsWhere({serviceAccountId, user, mode = 'LIVE', provider, scope}) {
  ...
  if (user?.role === 'ADMIN') return {provider, scope, ...(mode === 'LIVE' ? {enabled: true} : {})}
...
export async function ingestMeterBatch(payload, actor, {client = prisma} = {}) {
  ...
  return client.$transaction(async tx => {
    ...
    if (previous) {
      if (previous.payloadHash !== payloadHash || (actor.user?.role !== 'ADMIN' && previous.serviceAccountId !== actor.serviceAccountId)) {
    ...
    if (!streams.length && actor.user?.role !== 'ADMIN') throw createHttpError(403, 'Aucun flux compteur autorisé pour ce compte de service.')
    ...
    const ingestion = await tx.meterIngestion.create({data: {
      ...
      serviceAccountId: actor.serviceAccountId ?? null, actorUserId: actor.user?.id ?? null,
