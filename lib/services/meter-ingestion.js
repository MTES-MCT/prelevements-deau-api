...
export async function ingestMeterBatch(payload, actor, {client = prisma} = {}) {
  ...
  const payloadHash = meterHash({provider: value.provider, scope: value.scope, mode: value.mode, fetchedAt: value.fetchedAt,
    windowStart: value.windowStart, windowEnd: value.windowEnd, readings: value.readings, ...protection})
  return client.$transaction(async tx => {
    ...
    const result = {persisted: true, ingestionId: ingestion.id, counts, checkpoint: value.windowEnd, ...protection}
