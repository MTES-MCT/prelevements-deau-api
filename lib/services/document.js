...
async function assertExploitationBelongsToDeclarant(exploitationId, declarantUserId) {
  if (!exploitationId) {
    return
  }
  ...
  if (!exploitation) {
    throw createHttpError(400, 'Cette exploitation est introuvable.')
  }

  if (exploitation.declarantUserId !== declarantUserId) {
    throw createHttpError(400, 'Cette exploitation n’est pas rattachée à ce déclarant.')
  }
}
...
export async function createDocument({payload, file, declarantUserId, declarantPointPrelevementId = null, s3 = createStorageClient}) {
  ...
  await assertExploitationBelongsToDeclarant(linkedExploitationId, declarantUserId)
...
export async function updateDocument(documentId, payload, declarantUserId) {
  ...
  if (Object.hasOwn(changes, 'declarantPointPrelevementId')) {
    await assertExploitationBelongsToDeclarant(
      changes.declarantPointPrelevementId,
      declarantUserId
    )
  }
