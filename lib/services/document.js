 ...
 */
export async function uploadDocumentToS3({buffer, filename, preleveurSeqId, s3 = s3Default}) {
  // Mode idempotent : hash du fichier
  const hash = createHash('sha256').update(buffer).digest('hex').slice(0, 8)
  const objectKey = `${preleveurSeqId}/${hash}/${filename}`
 ...
 */
export async function createDocument({payload, file, preleveurSeqId, preleveurObjectId, s3 = s3Default}) {
  ...
  try {
    return await DocumentModel.insertDocument(document)
  } catch (error) {
 ...
 * @param {Function} options.s3 - Instance S3 (optionnel, pour injection dans tests)
 */
export async function decorateDocument(document, {includeRelations = false, s3 = s3Default} = {}) {
