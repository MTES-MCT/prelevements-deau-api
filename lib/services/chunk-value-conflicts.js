...
async function getConflictingChunksForBatch({pointPrelevementId, valueRows, exactMatchOnly}) {
...
async function getConflictingChunks({pointPrelevementId, valueRows, exactMatchOnly}) {
  ...
  const conflictsByChunkId = new Map()
  ...
  for (let index = 0; index < valueRows.length; index += CONFLICT_QUERY_BATCH_SIZE) {
    ...
    const conflicts = await getConflictingChunksForBatch({
      pointPrelevementId,
    ...
    for (const conflict of conflicts) {
      conflictsByChunkId.set(conflict.chunkId, conflict)
    }
  }

  return [...conflictsByChunkId.values()]
}
