  ...
  return client.$queryRaw`
    ...
    WHERE c."pointPrelevementId" = ${pointPrelevementId}::uuid
      AND (
        ...
        OR (${includeUnattributed}::boolean AND c."preleveurUserId" IS NULL)
      )
...
export async function applyConflictPolicyForIncomingChunkValues({
  ...
  // Le verrou reste détenu jusqu'à l'insertion effectuée par l'appelant transactionnel.
  // Il est partagé avec la reconstruction et la publication des campagnes.
  if (client === prisma) {
    return client.$transaction(tx => applyConflictPolicyForIncomingChunkValues({
      pointPrelevementId,
      preleveurUserId,
      valueRows: rawValueRows,
      requestedPolicy,
      ...
      deferredReplacedSourceIds,
      client: tx,
      findConflictingChunkValues
    }), CONFLICT_TRANSACTION_OPTIONS)
  }
  ...
  const volumeRows = valueRows.filter(row => row.metricTypeCode === METRIC_TYPE_CODES.VOLUME)
  if (client.campaignCoverage) {
    await client.$executeRaw`
      SELECT pg_advisory_xact_lock(hashtext('volumes-from-index'), hashtext(${pointPrelevementId}))
    `
  }

  if (volumeRows.length > 0 && client.campaignCoverage) {
    const coverages = await client.campaignCoverage.findMany({
      where: {
        pointPrelevementId,
        ...(preleveurUserId ? {preleveurUserId} : {}),
        active: true,
        OR: volumeRows.map(row => ({periodStart: {lt: row.periodEnd}, periodEnd: {gt: row.periodStart}}))
      },
      select: {periodStart: true, periodEnd: true}
    })
    const protectedRows = volumeRows.filter(row => coverages.some(coverage => datesOverlap(
      row.periodStart, row.periodEnd, coverage.periodStart, coverage.periodEnd
    )))
    if (protectedRows.length > 0) {
      // Une couverture manquante justifiée protège aussi l'absence de volume.
      // On ne fractionne jamais un ancien volume par prorata pour contourner ce verrou.
      const unprotectedRows = valueRows.filter(row => !protectedRows.includes(row))
      if (effectivePolicy === RESOLUTION_POLICIES.SKIP_NEW_CHUNK || unprotectedRows.length === 0) {
        return {shouldSkip: true, replacedChunkIds: [], valueRowsToInsert: [], skippedValueRows: valueRows, skippedValueCount: valueRows.length}
      }

      const result = await applyConflictPolicyForIncomingChunkValues({
        pointPrelevementId, preleveurUserId, valueRows: unprotectedRows, requestedPolicy,
        replaceComment, replacementSourceId, replacementMetadata, deferredReplacedSourceIds,
        client, findConflictingChunkValues
      })
      return {...result, skippedValueRows: [...protectedRows, ...result.skippedValueRows], skippedValueCount: protectedRows.length + result.skippedValueCount}
    }
  }
...
/** Un remplacement de campagne ne peut retirer que des périodes entièrement couvertes. */
export function classifyCampaignVolumeConflicts({conflicts, periods}) {
  const ordered = periods.map(period => ({start: new Date(period.periodStart), end: new Date(period.periodEnd)}))
  ...
  const partial = []
  for (const conflict of conflicts) {
    const start = new Date(conflict.periodStart)
    const end = new Date(conflict.periodEnd)
    if (merged.some(period => period.start <= start && period.end >= end)) {
      complete.push(conflict)
    } else if (merged.some(period => datesOverlap(start, end, period.start, period.end))) {
      partial.push(conflict)
    }
  }
...
export async function supersedeCampaignVolumeConflicts({conflicts, periods, valueRows, sourceId, metadata, client}) {
  const {complete, partial} = classifyCampaignVolumeConflicts({conflicts, periods})
  if (partial.length > 0) {
    throw new Error('Un volume existant déborde de la période de campagne : remplacement partiel interdit.')
  }
  ...
  if (!hasReplacementAuditClient(client)) {
    throw new Error('La publication exige un historique des remplacements.')
  }

  await createReplacementAudits({
    conflicts: complete,
    valueRows,
    conflictPolicy: 'CAMPAIGN_SUPERSEDE',
    replaceComment: 'Remplacement par une transmission de campagne.',
    replacementSourceId: sourceId,
    replacementMetadata: metadata,
    exactMatchOnly: false,
    client
  })
  await deleteConflictingChunkValues(complete.map(conflict => conflict.chunkValueId), client)
  await refreshPartiallyReplacedChunks({
    chunkIds: uniqueValues(complete.map(conflict => conflict.chunkId)),
    replaceComment: 'Remplacement par une transmission de campagne.',
    client
  })
  return uniqueValues(complete.map(conflict => conflict.sourceId))
}
