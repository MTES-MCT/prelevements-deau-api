...
async function getConflictingChunkValuesForBatch({
  ...
  const overlapCondition = exactMatchOnly
    ? Prisma.sql`${normalizedExistingMetric} = inc."metricTypeCode" AND (
      (cv."periodStart" = inc."periodStart" AND cv."periodEnd" = inc."periodEnd")
      OR (c."calculationStrategy" = 'METER' AND cv."periodStart" < inc."periodEnd" AND cv."periodEnd" > inc."periodStart")
    )`
  ...
  return client.$queryRaw`
    ...
    SELECT DISTINCT
      ...
      c."calculationStrategy" AS "calculationStrategy",
    ...
    WHERE c."pointPrelevementId" = ${pointPrelevementId}::uuid
      AND (${exploitationId}::uuid IS NULL OR c."exploitationId" IS NULL OR c."exploitationId" = ${exploitationId}::uuid)
      AND (
        ...
        OR (c."calculationStrategy" = 'METER' AND ${preleveurUserId}::uuid IS NULL)
      )
...
export async function applyConflictPolicyForIncomingChunkValues({
  ...
  // Il est partagé avec la reconstruction et les publications de compteurs.
  if (client === prisma) {
  ...
  if (conflicts.some(conflict => conflict.calculationStrategy === 'METER')) {
    throw createHttpError(409, 'Ces volumes recouvrent une publication de compteur protégée. Corrigez les relevés ou les affectations du compteur concerné.')
  }
