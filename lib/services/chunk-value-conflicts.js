...
export async function applyConflictPolicyForIncomingChunkValues({
  ...
  const replaceConflicts = async ({conflictsToReplace, replacementValueRows}) => {
    const replacedChunkIds = [...new Set(conflictsToReplace.map(conflict => conflict.chunkId))]
    const affectedSourceIds = [...new Set(conflictsToReplace.map(conflict => conflict.sourceId))]
