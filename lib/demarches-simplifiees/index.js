...
export async function reprocessAllAttachments(demarcheNumber) {
  const dossiers = await Dossier.getDossiers(demarcheNumber)

  for (const dossier of dossiers) {
    console.log(
      `Reprocessing attachments for dossier ${demarcheNumber}/${dossier.number}`
    )
    const attachments = await Dossier.getAttachments(
      demarcheNumber,
      dossier.number
    )

    for (const attachment of attachments) {
      await processAttachment(attachment, dossier.typePrelevement)
    }
  }
}
