...
import {getBeneficiaireByEmail} from '../models/beneficiaire.js'
...
async function experimentalHandleData(data, context) {
  try {
    ...
    const beneficiaire = await getBeneficiaireByEmail(dossier.declarant.email || dossier.usager.email)
    ...
    const regleVolumeJournalier = exploitation.regles.find(r => r.parametre === 'Volume journalier')

    if (regleVolumeJournalier) {
      for (const volumePreleve of volumesPreleves.rows) {
        if (volumePreleve.valeur > regleVolumeJournalier.valeur) {
          console.log(`Volume prélevé (${volumePreleve.valeur} m3) supérieur à la règle de volume journalier (${regleVolumeJournalier.valeur} m3)`)
        }
      }
    }
