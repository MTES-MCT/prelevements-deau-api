...
import {getPreleveurByEmail} from '../models/preleveur.js'
import {getMatchingExploitation} from '../models/exploitation.js'

import {extractDossier} from './extract.js'

import {fetchDossiersGenerator} from '@fabrique/demarches-simplifiees'
...
async function handleNewFile(file, context) {
  ...
  await s3('ds').uploadObject(objectKey, buffer)
...
export async function processAllDossiers() {
  ...
  for await (const rawDossier of fetchDossiersGenerator(demarcheNumber, {includeChamps: true})) {
