...
import * as Preleveur from '../models/declarant.js'
...
export async function consolidateDossiersMaintenance(logger = createLogger()) {
  const dossiers = await Dossier.getUnconsolidatedDossiers()
  logger.log(`Création de ${dossiers.length} jobs de consolidation`)
  for (const dossier of dossiers) {
    try {
      await addJobConsolidateDossier(dossier._id.toString())
    } catch (error) {
      Sentry.captureException(error)
      logger.error(`Erreur lors de la création du job de consolidation pour le dossier ${dossier._id}: ${error.message}`)
    }
  }
}

async function findPreleveur(dossier) {
  return await Preleveur.getDeclarantByEmail(dossier.usager.email)
    || Preleveur.getDeclarantByEmail(dossier.declarant.email)
}

export async function consolidateDossier(dossierId, logger = createLogger()) {
  // Conversion string → ObjectId si nécessaire
  const dossierObjectId = typeof dossierId === 'string' ? parseObjectId(dossierId) : dossierId

  if (!dossierObjectId) {
    logger.log(`ID invalide: ${dossierId}, abandon`)
    return
  }

  const dossier = await Dossier.getDossier(dossierObjectId)
  if (!dossier) {
    logger.log(`Dossier ${dossierId} non trouvé, abandon`)
    return
  }

  if (dossier.consolidatedAt) {
    logger.log(`Dossier ${dossierId} déjà consolidé, abandon`)
    return
  }

  logger.log(`Consolidation du dossier ${dossierId}`)
  ...
  if (preleveur) {
    ...
    logger.log(`Préleveur introuvable pour le dossier ${dossierId}, consolidation sans intégration`)
  }
  ...
  for (const key of Object.keys(seriesByPoint)) {
    ...
    const pointPrelevement = await getPointPrelevement(seqId)

    if (!pointPrelevement) {
      logger.log(`Point de prélèvement introuvable : ${seqId}`)
      result.notFoundPointPrelevements.push(seqId)
      continue
    }

    logger.log(`Traitement du point ${seqId} avec ${seriesByPoint[seqId].length} série(s)`)
...
async function processPointSeries({pointId, pointSeries, preleveur, dossier, logger}) {
  ...
  for (const {id: attachmentId, series: attachmentSeries} of attachmentsMap.values()) {
    ...
    logger.log(`  Traitement attachment ${attachmentId || 'null'} avec ${attachmentSeries.length} série(s)`)
    ...
    if (!volumeSeries) {
      // Pas de série volume => aucune intégration ne devrait exister pour cette PJ
      // Si des intégrations orphelines existent (cas d'un fichier retraité qui n'a plus de volumes),
      // on les supprime pour nettoyer les données
      // IMPORTANT : seulement si attachmentId existe (les séries sans attachment n'ont pas d'intégrations)
      if (attachmentId) {
        const existingIntegrations = await listIntegrationsByAttachment(attachmentId)
        if (existingIntegrations.length > 0) {
          const orphanedDays = existingIntegrations.map(i => i.date)
          await deleteIntegrationsByAttachment(attachmentId, orphanedDays)
          logger.log(`Suppression de ${orphanedDays.length} intégration(s) orpheline(s) pour attachment ${attachmentId} (pas de série volume prélevé)`)
          result.removedDaysCount += orphanedDays.length
        }
      }

      // Mettre à jour toutes les séries de l'attachment pour retirer les integratedDays
      // (pour qu'elles ne soient plus considérées comme problématiques)
      await updateSeriesIntegratedDays(attachmentSeriesIds, [])

      // On passe à la PJ suivante
      continue
    }
    ...
    // 5. Déterminer les jours qui DEVRAIENT être intégrés (basé sur le contenu actuel ET les critères d'intégration)
    // Règles métier : dossier avec statut 'accepte' ET préleveur identifié
    let integrableDays = []
    if (dossier.status === 'accepte' && preleveur) {
      integrableDays = volumeRows.map(row => row.date)
    } else if (dossier.status !== 'accepte') {
      logger.log(`Dossier ${dossier._id} avec statut '${dossier.status}': aucun jour intégrable (statut requis: 'accepte')`)
    } else if (!preleveur) {
      logger.log(`Dossier ${dossier._id} sans préleveur: aucun jour intégrable (préleveur requis)`)
    }

    // 6. Récupérer les intégrations EXISTANTES pour cet attachment
    // IMPORTANT : seulement si attachmentId existe
    let existingDays = []
    if (attachmentId) {
      const existingIntegrations = await listIntegrationsByAttachment(attachmentId)
      existingDays = existingIntegrations.map(i => i.date)
    }
    ...
    if (toRemove.length > 0 && attachmentId) {
      await deleteIntegrationsByAttachment(attachmentId, toRemove)
      logger.log(`Suppression de ${toRemove.length} intégration(s) obsolète(s) pour attachment ${attachmentId}`)
    ...
    // IMPORTANT : ne peut se faire QUE si on a un préleveur ET un attachmentId
    const actuallyAdded = []
    let conflictCount = 0
    if (preleveur && attachmentId) {
      for (const day of toAdd) {
        ...
        if (existingIntegration) {
          logger.log(`Conflit: jour ${day} déjà intégré par attachment ${existingIntegration.attachmentId}`)
          conflictCount++
          continue
        }
    ...
    // 10. Mettre à jour computed.integratedDays des séries (pour TOUS les jours EFFECTIVEMENT intégrés)
    // On doit utiliser les jours qui sont réellement dans integrations_journalieres après toutes les opérations
    // IMPORTANT : seulement si attachmentId existe (sinon pas d'intégrations possibles)
    let allIntegratedDays = []
    if (attachmentId) {
      const currentIntegrations = await listIntegrationsByAttachment(attachmentId)
      allIntegratedDays = currentIntegrations.map(i => i.date)
    }

    // Pour chaque série de la PJ : ne rattacher que les jours intégrés compatibles avec sa plage min/max
    // IMPORTANT : toujours mettre à jour, même si allIntegratedDays est vide, pour garantir l'idempotence
    for (const s of attachmentSeries) {
      const filtered = allIntegratedDays.filter(d => d >= s.minDate && d <= s.maxDate)
