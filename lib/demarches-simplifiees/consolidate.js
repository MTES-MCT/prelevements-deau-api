...
export async function consolidateDossiersMaintenance() {
  ...
  for (const dossier of dossiers) {
    try {
      ...
      console.error(`Erreur lors de la création du job de consolidation pour le dossier ${dossier._id}:`, err)
    }
  }
}
...
export async function consolidateDossier(dossierId) {
  ...
  const dossierObjectId = typeof dossierId === 'string' ? mongo.parseObjectId(dossierId) : dossierId
