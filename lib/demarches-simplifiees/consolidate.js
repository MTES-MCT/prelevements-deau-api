...
// Hash logic supprimée (comparaison différentielle abandonnée)

// Données désormais normalisées : séries + valeurs en collections dédiées
...
async function processPointSeries({seqId, territoire, pointSeries, preleveur, dossier, demarcheNumber, result}) {
  ...
  // On ne charge plus les saisies existantes (collection supprimée)
  // Ancien index des entrées existantes supprimé (collection retirée)
  ...
  // Set des dates conservé auparavant pour suppression différentielle (plus nécessaire)
  ...
  // Plus de suppression d’entrées (collection supprimée)
