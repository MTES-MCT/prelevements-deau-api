...
export async function consolidateDossier(dossierId) {
  ...
  if (preleveur) {
    ...
    console.log(`Préleveur introuvable pour le dossier ${dossierId}, consolidation sans intégration`)
  }
...
async function processPointSeries({pointId, pointSeries, preleveur, dossier}) {
  ...
  for (const {id: attachmentId, series: attachmentSeries} of attachmentsMap.values()) {
    ...
    if (dossier.status === 'accepte' && preleveur) {
      ...
      console.log(`Dossier ${dossier._id} sans préleveur: aucun jour intégrable (préleveur requis)`)
    }
