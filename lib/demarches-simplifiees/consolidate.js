...
export async function consolidateDossier(demarcheNumber, dossierNumber) {
  ...
  if (preleveur && dossier.status === 'accepte') {
    const attachments = await Dossier.getAttachmentsSummary(demarcheNumber, dossierNumber)
    ...
    for (const attachment of attachments) {
      ...
      for (const p of points) {
        console.log(`Point de prélèvement ${p.pointPrelevement} : ${p.dailyValues.length} jours de données trouvées (${p.minDate} => ${p.maxDate}`)
      }
    }
  }
