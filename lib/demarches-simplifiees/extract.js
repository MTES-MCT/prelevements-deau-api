/* eslint-disable complexity, quote-props */
...
function extractDataFromChamps(champs, context) {
  ...
  for (const champ of champs) {
    ...
    const overrideEntry = OVERRIDES[numeroDossier]
    ...
    const value = parseChamp(champ, {definition, numeroDossier: context.numeroDossier})
...
function parseChamp(champ, {definition, numeroDossier}) {
  if (!definition) {
  ...
  if (definition.array && definition.type === 'string' && champ.values) {
    return champ.values.map(value => parseChamp({stringValue: value}, {definition, numeroDossier}))
  }
...
export function extractDossier(dossier) {
  const numeroDossier = dossier.number
  ...
  const extractedAttributes = extractDataFromChamps(dossier.champs, {numeroDossier})
