...
import {calculateTotalVolumePreleve, normalizeErrors, summarizeErrors, parseBuffer, addHashToSeries, compareSeries} from '../attachments/index.js'
...
/* Tests pour addHashToSeries */

test('addHashToSeries - null series', t => {
  const result = addHashToSeries(null)
  t.deepEqual(result, [])
})

test('addHashToSeries - undefined series', t => {
  const result = addHashToSeries(undefined)
  t.deepEqual(result, [])
})

test('addHashToSeries - empty array', t => {
  const result = addHashToSeries([])
  t.deepEqual(result, [])
})

test('addHashToSeries - single series', t => {
  const series = [
    {
      pointName: 'Point A',
  ...
  const result = addHashToSeries(series)
  t.is(result.length, 1)
  t.truthy(result[0].hash)
  t.is(typeof result[0].hash, 'string')
  t.is(result[0].hash.length, 12)
  t.is(result[0].pointName, 'Point A')
  t.is(result[0].parameter, 'volume prélevé')
})

test('addHashToSeries - multiple series', t => {
  const series = [
    {
      pointName: 'Point A',
      parameter: 'volume prélevé',
    ...
    },
    {
      ...
      data: [{date: '2025-01-01', value: 25}]
    }
  ]
  const result = addHashToSeries(series)
  t.is(result.length, 2)
  t.truthy(result[0].hash)
  t.truthy(result[1].hash)
  t.not(result[0].hash, result[1].hash) // Hash différents pour des séries différentes
})

test('addHashToSeries - identical series produce same hash', t => {
  const series1 = [
    {
      pointName: 'Point A',
      parameter: 'volume prélevé',
  ...
  const result1 = addHashToSeries(series1)
  const result2 = addHashToSeries(series2)
  t.is(result1[0].hash, result2[0].hash)
})

test('addHashToSeries - different data produce different hash', t => {
  const series1 = [
    {
      pointName: 'Point A',
      parameter: 'volume prélevé',
  ...
  const series2 = [
    {
      pointName: 'Point A',
      parameter: 'volume prélevé',
      data: [{date: '2025-01-01', value: 43}]
    }
  ]
  const result1 = addHashToSeries(series1)
  const result2 = addHashToSeries(series2)
  t.not(result1[0].hash, result2[0].hash)
})

test('addHashToSeries - preserves all original properties', t => {
  const series = [
    {
      pointName: 'Point A',
      parameter: 'volume prélevé',
  ...
  t.is(result[0].pointName, 'Point A')
  t.is(result[0].parameter, 'volume prélevé')
  t.is(result[0].unit, 'm³')
  t.is(result[0].frequency, 'daily')
  t.deepEqual(result[0].data, [{date: '2025-01-01', value: 42}])
  t.deepEqual(result[0].metadata, {source: 'test'})
  t.truthy(result[0].hash)
})
...
/* Tests pour compareSeries */

test('compareSeries - all new series', t => {
  const existingSeries = []
  const newSeries = [
    {hash: 'abc123', data: []},
    {hash: 'def456', data: []}
  ]
  const result = compareSeries(existingSeries, newSeries)
  t.is(result.toDelete.length, 0)
  t.is(result.toCreate.length, 2)
  t.is(result.unchangedCount, 0)
})

test('compareSeries - all unchanged series', t => {
  const existingSeries = [
    {_id: 'id1', hash: 'abc123'},
    {_id: 'id2', hash: 'def456'}
  ]
...
test('compareSeries - all deleted series', t => {
  const existingSeries = [
    {_id: 'id1', hash: 'abc123'},
    {_id: 'id2', hash: 'def456'}
  ]
  ...
  t.deepEqual(result.toDelete, ['id1', 'id2'])
  t.is(result.toCreate.length, 0)
  t.is(result.unchangedCount, 0)
})

test('compareSeries - mixed scenario', t => {
  const existingSeries = [
    {_id: 'id1', hash: 'abc123'}, // Unchanged
    {_id: 'id2', hash: 'def456'}, // To delete
    {_id: 'id3', hash: 'ghi789'} // Unchanged
  ]
  const newSeries = [
    {hash: 'abc123', data: []}, // Unchanged
    {hash: 'ghi789', data: []}, // Unchanged
    {hash: 'jkl012', data: []} // New
  ]
  const result = compareSeries(existingSeries, newSeries)
  t.is(result.toDelete.length, 1)
  t.deepEqual(result.toDelete, ['id2'])
  t.is(result.toCreate.length, 1)
  t.is(result.toCreate[0].hash, 'jkl012')
  t.is(result.unchangedCount, 2)
})

test('compareSeries - duplicate hashes in new series', t => {
  const existingSeries = [
    {_id: 'id1', hash: 'abc123'}
  ]
  const newSeries = [
    {hash: 'abc123', data: []},
    {hash: 'abc123', data: []} // Duplicate
  ]
  const result = compareSeries(existingSeries, newSeries)
  t.is(result.toDelete.length, 0)
  t.is(result.toCreate.length, 0) // Both duplicates are considered unchanged
  t.is(result.unchangedCount, 2)
})

test('compareSeries - empty inputs', t => {
...
test('compareSeries - series replacement (different hash, same position)', t => {
  const existingSeries = [
    {_id: 'id1', hash: 'abc123'}
  ]
  const newSeries = [
    {hash: 'xyz999', data: []} // Different hash
  ]
  const result = compareSeries(existingSeries, newSeries)
  t.is(result.toDelete.length, 1)
  t.deepEqual(result.toDelete, ['id1'])
  t.is(result.toCreate.length, 1)
  t.is(result.toCreate[0].hash, 'xyz999')
  t.is(result.unchangedCount, 0)
})
