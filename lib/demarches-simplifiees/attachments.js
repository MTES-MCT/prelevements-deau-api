...
export async function processAttachmentsMaintenance() {
  const attachments = await Dossier.getUnprocessedAttachments()
  console.log(`Création de ${attachments.length} jobs de traitement d'attachments`)
  for (const attachment of attachments) {
    try {
      await addJobProcessAttachment(attachment._id.toString())
    } catch (error) {
      console.error(`Erreur lors de la création du job pour l'attachment ${attachment._id}:`, error)
    }
  }
}
...
export async function processAttachment(attachmentId) {
  ...
  const attachmentObjectId = typeof attachmentId === 'string' ? mongo.parseObjectId(attachmentId) : attachmentId

  if (!attachmentObjectId) {
    console.log(`ID invalide: ${attachmentId}, abandon`)
    return
  }

  const attachment = await Dossier.getAttachment(attachmentObjectId)
  if (!attachment) {
    console.log(`Attachment ${attachmentId} non trouvé, abandon`)
    return
  }

  if (attachment.processed) {
    console.log(`Attachment ${attachmentId} déjà traité, abandon`)
    return
  }
