...
export async function deleteDocument(idDocument) {
  return mongo.db.collection('documents').findOneAndUpdate(
...
export async function deleteDocumentFromS3(fileName) {
...
export async function getDocumentsFromPreleveur(idPreleveur) {
  const resultFromExploitations = await mongo.db.collection('exploitations').find(
    {id_preleveur: idPreleveur, deletedAt: {$exists: false}},
    {projection: {documents: 1, _id: 0}}
  ).toArray()

  const flattenResultFromExploitations = resultFromExploitations.flatMap(r => r.documents)

  const resultFromDocuments = await mongo.db.collection('documents').find(
    {id_preleveur: idPreleveur, deletedAt: {$exists: false}}
  ).toArray()

  const results = [...flattenResultFromExploitations, ...resultFromDocuments]
  const documentsUniq = uniqBy(results, 'id_document')

  return documentsUniq
}
