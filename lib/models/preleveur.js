...
import {getExploitationsFromPreleveur, hasActiveExploitationForPreleveur} from './exploitation.js'

export async function decoratePreleveur(preleveur) {
  const exploitations = await getExploitationsFromPreleveur(preleveur._id)
...
export async function deletePreleveur(preleveurId) {
  const hasActiveExploitation = await hasActiveExploitationForPreleveur(preleveurId)
...
export async function getPreleveursFromIds(preleveurIds) {
  return mongo.db.collection('preleveurs').find(
    {
      _id: {$in: preleveurIds},
      deletedAt: {$exists: false}
