...
export async function listAllowedDeclarationTypesForDeclarants(
  declarantUserIds,
  now = new Date(),
  {client = prisma} = {}
) {
  ...
  const links = await client.declarantDeclarationType.findMany({
    where: {
      declarantUserId: {in: uniqueDeclarantUserIds},
  ...
  const declarationTypesByDeclarantId = new Map(
    uniqueDeclarantUserIds.map(declarantUserId => [declarantUserId, new Map()])
  )

  for (const link of links) {
    const declarationType = serializeDeclarationType(link.declarationType)
    const byCode = declarationTypesByDeclarantId.get(link.declarantUserId)

    if (!byCode || !declarationType || byCode.has(declarationType.code)) {
      continue
    }
  ...
  return new Map([...declarationTypesByDeclarantId].map(([declarantUserId, byCode]) => [
    declarantUserId,
    [...byCode.values()]
  ]))
}
...
export async function listAllowedDeclarationTypesForDeclarant(
  ...
  const declarationTypesByDeclarantId = await listAllowedDeclarationTypesForDeclarants(
    [declarantUserId],
    now,
    options
  )

  return declarationTypesByDeclarantId.get(declarantUserId) ?? []
}
