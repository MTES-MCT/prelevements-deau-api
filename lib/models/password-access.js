...
export async function consumePasswordActivation(token, credential, {
  ...
  return client.$transaction(async transaction => {
    ...
    return createSessionToken(
      ...
      undefined,
      {
        authVersion: activation.user.authVersion,
        client: transaction
      }
    )
  })
...
export async function replacePasswordCredential(user, credential, {
  ...
  return client.$transaction(async transaction => {
    ...
    if (sessionToken) {
      await validateSession(user.id, sessionToken, {client: transaction})
    }
    ...
    return createSessionToken(currentUser.id, currentUser.role, undefined, {
