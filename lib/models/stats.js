...
import {
  ...
  countPointsPrelevement,
  getActivePointsByType
} from './points-prelevement.js'

import {getActiveExploitations, getActiveExploitationsForStats} from './exploitation.js'

import mongo from '../util/mongo.js'

export async function getStats(territoire) {
  const activExploitations = await getActiveExploitations(territoire)
  ...
  const activPointsSurface = await getActivePointsByType(activPoints, 'Eau de surface', territoire)

  const activPointsSouterrain = await getActivePointsByType(activPoints, 'Eau souterraine', territoire)
  ...
  return {
    ...
    pointsCount: await countPointsPrelevement(territoire),
...
async function getRegularisationsStats(territoire) {
  ...
  const activExploitations = await getActiveExploitationsForStats(territoire)
...
async function getDebitsReservesStats(territoire) {
  ...
  const activeExploitations = await getActiveExploitationsForStats(territoire)
