...
const SURFACE_WATER_BODY_TYPES = new Set(['SUPERFICIELLE'])

function getExploitationUsageValues(exploitation) {
  const usages = [
    ...(Array.isArray(exploitation.usages) ? exploitation.usages : []),
    ...getExploitationWaterUses(exploitation)
  ]
  const usagesByKey = new Map()

  for (const usage of usages) {
    const key = usage && typeof usage === 'object'
      ? (usage.id ?? usage.code)
      : usage

    if (key && !usagesByKey.has(key)) {
      usagesByKey.set(key, usage)
    }
  }

  return [...usagesByKey.values()]
}
...
export async function getStats(db = prisma) {
  ...
  return {
    ...
    activPointsSurfaceCount: new Set(
      activeExploitations
        .filter(exploitation => SURFACE_WATER_BODY_TYPES.has(exploitation.pointPrelevement?.waterBodyType))
