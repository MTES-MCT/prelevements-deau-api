...
export async function getLibelleCommune(idCommune) {
  const commune = await mongo.db.collection('communes').findOne({
    id: idCommune
  })

  if (!commune) {
    throw createHttpError(404, 'Identifiant commune introuvable.')
  }

  return commune
}

export async function getLibellesCommune() {
  return mongo.db.collection('communes').find().toArray()
}
