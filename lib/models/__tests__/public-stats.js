...
import {
  ...
  loadPublicStats,
  resolvePublicStatsDeploymentMonth,
...
test('les dates de déploiement proviennent seulement de la configuration métier du type et code', t => {
  const dates = {'SAGE:SAGE-test': '2025-03', 'DEPARTEMENT:974': '2026-02'}
  t.is(resolvePublicStatsDeploymentMonth({type: 'SAGE', code: 'SAGE-test'}, dates), '2025-03')
  t.is(resolvePublicStatsDeploymentMonth({type: 'DEPARTEMENT', code: '974'}, dates), '2026-02')
  t.is(resolvePublicStatsDeploymentMonth({type: 'SAGE', code: '974'}, dates), null)
  t.is(resolvePublicStatsDeploymentMonth({type: 'SAGE', code: 'SAGE-test'}), null)
  t.is(resolvePublicStatsDeploymentMonth({createdAt: '2025-03-01', firstMeasurementMonth: '2024-01'}, dates), null)
  for (const value of ['2026-13', '2026-2', '0000-01', '1899-12', 2026]) {
    t.is(resolvePublicStatsDeploymentMonth({type: 'SAGE', code: 'test'}, {'SAGE:test': value}), null)
  }
})
...
test('la réponse conserve les territoires sans remontée du mois et les totaux dédupliqués SQL', async t => {
  const client = statsClient({
    ...
    territories: [
      ...
      },
      {
        ...
        reportingPreleveursCount: 1, agriculture: 1, unknown: 1, reportingUnknown: 1
      },
      {
  ...
  t.is(result.territories.SAGE[0].profiles.reduce((sum, row) => sum + row.reportingCount, 0), 1)
  t.is(result.territories.SAGE[0].profiles[0].reportingCount, 0)
  t.is(result.territories.SAGE[0].profiles[4].reportingCount, 1)
  t.is(result.territories.SAGE[0].deploymentMonth, null)
