...
import {buildPublicStatsActiveUsersQuery, buildPublicStatsDataQuery, getPublicStats, loadPublicStats, summarizePublicStatsActiveUsers} from '../public-stats.js'
...
async function withRolledBackFixtures(operation) {
  ...
  try {
    await client.$transaction(async transaction => {
      ...
      await transaction.$executeRaw`TRUNCATE TABLE "UserActivityCollectionState"`
...
function expectedProfiles() {
  return [
    {key: 'AGRICULTURE', label: 'Agriculteurs', count: 2, reportingCount: 1},
    {key: 'INDUSTRY', label: 'Industriels', count: 2, reportingCount: 1},
    {key: 'DRINKING_WATER', label: 'Gestionnaires d’eau potable', count: 1, reportingCount: 1},
    {key: 'OTHER', label: 'Autres', count: 2, reportingCount: 1},
    {key: 'UNKNOWN', label: 'Non renseigné', count: 0, reportingCount: 0}
  ]
}
...
defineIntegrationTest('getPublicStats calcule les statistiques réelles sans fuite entre territoires ou statuts', async t => {
  ...
  await withRolledBackFixtures(async transaction => {
    ...
    t.deepEqual(result.activeUsers, {
      availableSince: '2026-04-15T12:00:00.000Z',
      collectionStartedAt: null,
      months: [
        {month: '2026-03', administration: null, declarants: null, total: null, status: 'unavailable'},
        {month: '2026-04', administration: null, declarants: null, total: null, status: 'unavailable'},
        {month: '2026-05', administration: 1, declarants: 1, total: 2, status: 'partial'},
        {month: '2026-06', administration: 0, declarants: 1, total: 1, status: 'partial'},
        {month: '2026-07', administration: 1, declarants: 0, total: 1, status: 'partial'},
        {month: '2026-08', administration: 0, declarants: 2, total: 2, status: 'partial'}
      ]
    })
  })
...
defineIntegrationTest('les SAGE chevauchants dédupliquent les points et déclarations mensuelles dans le département', async t => {
  await withRolledBackFixtures(async transaction => {
    const runId = randomUUID()
    const usage = await transaction.sandreWaterUse.create({
      data: {code: `TEST-${runId.slice(0, 8)}`, kind: 'USAGE', label: 'Usage test chevauchement'},
      select: {id: true}
    })
    const firstPreleveur = await createPreleveur(transaction, runId, 'first', 'IRRIGANT')
    const secondPreleveur = await createPreleveur(transaction, runId, 'second', 'ICPE')
    const firstSage = await createZone(transaction, {code: `SAGE-A-${runId}`, name: 'SAGE A', type: 'SAGE'})
    const secondSage = await createZone(transaction, {code: `SAGE-B-${runId}`, name: 'SAGE B', type: 'SAGE'})
    const department = await createZone(transaction, {code: `DEP-${runId}`, name: 'Département', type: 'DEPARTEMENT'})
    const firstPoint = await createPoint(transaction, runId, 'first')
    const sharedPoint = await createPoint(transaction, runId, 'shared')
    const lastPoint = await createPoint(transaction, runId, 'last')
    await linkPointsToZones(transaction, [firstPoint, sharedPoint], [firstSage])
    await linkPointsToZones(transaction, [sharedPoint, lastPoint], [secondSage])
    await linkPointsToZones(transaction, [firstPoint, sharedPoint, lastPoint], [department])

    await Promise.all([
      [firstPoint, firstPreleveur, 'API'],
      [sharedPoint, firstPreleveur, 'API'],
      [sharedPoint, firstPreleveur, 'API'],
      [lastPoint, secondPreleveur, 'BATCH']
    ].map(([point, preleveur, sourceType]) => createSourceChunk(transaction, {
      sourceType,
      pointId: point.id,
      preleveurUserId: preleveur.id,
      usageId: usage.id,
      declarationCreatedAt: new Date('2026-09-10T12:00:00.000Z'),
      periodStart: new Date('2026-08-01T00:00:00.000Z'),
      periodEnd: new Date('2026-09-01T00:00:00.000Z')
    })))

    const result = await getPublicStats({month: TEST_MONTH, now: TEST_NOW, client: transaction})
    t.deepEqual(result.totals, {pointsCount: 3, preleveursCount: 2, sageCount: 2, departmentCount: 1})
    t.deepEqual(result.territories.SAGE.map(({pointsCount, reportingPreleveursCount}) => ({pointsCount, reportingPreleveursCount})), [
      {pointsCount: 2, reportingPreleveursCount: 1},
      {pointsCount: 2, reportingPreleveursCount: 2}
    ])
    t.is(result.territories.DEPARTEMENT[0].pointsCount, 3)
    t.is(result.territories.DEPARTEMENT[0].reportingPreleveursCount, 2)
    t.deepEqual(result.territories.DEPARTEMENT[0].profiles.map(({reportingCount}) => reportingCount), [1, 1, 0, 0, 0])
    t.true(result.territories.SAGE.every(row => row.profiles.reduce((sum, profile) => sum + profile.reportingCount, 0) === row.reportingPreleveursCount))
  })
})

defineIntegrationTest('une période annuelle compte chaque mois couvert, indépendamment du dépôt et sans doublonner ses sources', async t => {
  await withRolledBackFixtures(async transaction => {
    const runId = randomUUID()
    const usage = await transaction.sandreWaterUse.create({
      data: {code: `TEST-${runId.slice(0, 8)}`, kind: 'USAGE', label: 'Usage test période annuelle'},
      select: {id: true}
    })
    const preleveur = await createPreleveur(transaction, runId, 'annual', 'IRRIGANT')
    const zone = await createZone(transaction, {code: `SAGE-${runId}`, name: 'SAGE annuel', type: 'SAGE'})
    const points = await Promise.all(['first', 'second'].map(key => createPoint(transaction, runId, key)))
    await linkPointsToZones(transaction, points, [zone])
    await Promise.all(points.map(point => createSourceChunk(transaction, {
      sourceType: 'API',
      preleveurUserId: preleveur.id,
      pointId: point.id,
      usageId: usage.id,
    ...
    const results = await Promise.all(['2025-12', '2026-05', '2026-06', '2026-07', '2026-08', '2027-01'].map(month =>
      loadPublicStats({month, client: transaction, now: new Date('2027-02-01T12:00:00.000Z')})
    ))
    t.deepEqual(results.map(result => result.territories.SAGE[0].reportingPreleveursCount), [0, 1, 1, 1, 1, 0])
    t.deepEqual(results.map(result => result.channels.find(channel => channel.key === 'THIRD_PARTY').count), [0, 1, 1, 1, 1, 0])
    t.true(results.every(result => result.territories.SAGE[0].preleveursCount === 1))
    t.is(results[1].territories.SAGE[0].profiles[0].reportingCount, 1)
    t.is(results[1].territories.SAGE[0].deploymentMonth, null)
  })
})

defineIntegrationTest('les parts publiées d’un compteur partagé suivent les mois à Paris, sans anciens volumes rejetés ni index bruts', async t => {
  await withRolledBackFixtures(async transaction => {
    const runId = randomUUID()
    const usage = await transaction.sandreWaterUse.create({
      data: {code: `TEST-${runId.slice(0, 8)}`, kind: 'USAGE', label: 'Usage test compteur partagé'},
      select: {id: true}
    })
    const firstOwner = await createPreleveur(transaction, runId, 'first', 'IRRIGANT')
    const secondOwner = await createPreleveur(transaction, runId, 'second', 'ICPE')
    const rejectedOwner = await createPreleveur(transaction, runId, 'superseded', 'AUTRE')
    const rawOnlyOwner = await createPreleveur(transaction, runId, 'raw-only', 'AUTRE')
    const genericOwner = await createPreleveur(transaction, runId, 'generic', 'IRRIGANT')
    const zone = await createZone(transaction, {code: `SAGE-${runId}`, name: 'SAGE compteurs', type: 'SAGE'})
    const points = await Promise.all(['shared', 'second'].map(key => createPoint(transaction, runId, key)))
    await linkPointsToZones(transaction, points, [zone])
    const meter = await transaction.compteur.create({data: {serialNumber: runId}, select: {id: true}})
    await transaction.declarantPointPrelevement.create({data: {
      declarantUserId: rawOnlyOwner.id,
      pointPrelevementId: points[0].id,
      usageId: usage.id,
      status: 'EN_ACTIVITE'
    }})
    // A raw reading cannot turn an unresolved allocation into a declaration.
    await transaction.meterReading.create({data: {
      compteurId: meter.id,
      observedAt: new Date('2026-09-05T12:00:00.000Z'),
      lastFetchedAt: new Date('2026-09-05T12:00:00.000Z'),
      currentMode: 'LIVE'
    }})
    const addMeterChunk = options => createSourceChunk(transaction, {
      sourceType: 'API',
      ...
      periodEnd: new Date('2026-07-31T22:30:00.000Z'),
      value: '0',
      ...options
    })
    await Promise.all([
      addMeterChunk({preleveurUserId: firstOwner.id}),
      addMeterChunk({preleveurUserId: firstOwner.id, pointId: points[1].id}),
      addMeterChunk({preleveurUserId: secondOwner.id}),
      addMeterChunk({
        preleveurUserId: secondOwner.id,
        periodStart: new Date('2026-08-31T21:00:00.000Z'),
        periodEnd: new Date('2026-08-31T22:00:00.000Z')
      }),
      addMeterChunk({preleveurUserId: rejectedOwner.id, instructionStatus: 'REJECTED'}),
      addMeterChunk({preleveurUserId: genericOwner.id, calculationStrategy: 'GENERIC', compteurId: null})
    ])

    const results = await Promise.all(['2026-07', '2026-08', '2026-09'].map(month =>
      loadPublicStats({month, client: transaction, now: new Date('2026-10-01T12:00:00.000Z')})
    ))
    t.deepEqual(results.map(result => result.territories.SAGE[0].reportingPreleveursCount), [1, 2, 0])
    t.deepEqual(results.map(result => result.channels.find(channel => channel.key === 'THIRD_PARTY').count), [1, 2, 0])
    t.deepEqual(results[1].territories.SAGE[0].profiles.map(({reportingCount}) => reportingCount), [1, 1, 0, 0, 0])
    t.is(results[1].territories.SAGE[0].preleveursCount, 5)
    t.is(results[1].territories.SAGE[0].reportingRate, 40)
    t.deepEqual(results[0].territories.SAGE[0].profiles.map(({reportingCount}) => reportingCount), [1, 0, 0, 0, 0])
    t.true(results[2].territories.SAGE[0].profiles.every(({reportingCount}) => reportingCount === 0))
  })
})

defineIntegrationTest('les utilisateurs actifs réunissent les sources sans doublons et arrêtent le repli sur les connexions après le démarrage', async t => {
  await withRolledBackFixtures(async transaction => {
    const runId = randomUUID()
    const changedRole = await createPreleveur(transaction, runId, 'changed-role', 'IRRIGANT')
    const secondPreleveur = await createPreleveur(transaction, runId, 'second', 'ICPE')
    const instructor = await createAgent(transaction, runId, 'INSTRUCTOR')
    const administrator = await createAgent(transaction, runId, 'ADMIN')
    const collectionStartedAt = new Date('2026-01-31T23:05:00.000Z')
    await transaction.userActivityCollectionState.create({data: {id: 'active-users', startedAt: collectionStartedAt}})
    await transaction.userMonthlyActivity.createMany({data: [
      {month: '2026-02', userId: changedRole.id, role: 'ADMIN'},
      {month: '2026-02', userId: secondPreleveur.id, role: 'DECLARANT'},
      {month: '2026-04', userId: secondPreleveur.id, role: 'DECLARANT'},
      {month: '2026-05', userId: changedRole.id, role: 'ADMIN'}
    ]})
    await Promise.all([
      ['2025-12-31T22:30:00.000Z', changedRole, 'ADMIN'],
      ['2025-12-31T23:30:00.000Z', changedRole, 'DECLARANT'],
      ['2026-01-31T23:01:00.000Z', changedRole, 'DECLARANT'],
      ['2026-02-10T12:00:00.000Z', changedRole, 'DECLARANT'],
      ['2026-02-10T12:00:00.000Z', instructor, 'INSTRUCTOR'],
      ['2026-02-10T12:00:00.000Z', administrator, 'ADMIN', 'FAILURE'],
      ['2026-02-11T12:00:00.000Z', administrator, 'SERVICE_ACCOUNT'],
      ['2026-03-10T12:00:00.000Z', administrator, 'ADMIN']
    ].map(([occurredAt, user, role, outcome = 'SUCCESS']) => createAuditEvent(transaction, {
      actionType: 'AUTH.LOGIN_VERIFIED',
    ...
    const [data] = await transaction.$queryRaw(buildPublicStatsActiveUsersQuery('2026-04'))
    const result = summarizePublicStatsActiveUsers({month: '2026-04', ...data})
    t.deepEqual(result, {
      availableSince: '2025-12-31T22:30:00.000Z',
      collectionStartedAt: collectionStartedAt.toISOString(),
      months: [
        {month: '2025-11', administration: null, declarants: null, total: null, status: 'unavailable'},
        {month: '2025-12', administration: 1, declarants: 0, total: 1, status: 'partial'},
        {month: '2026-01', administration: 0, declarants: 1, total: 1, status: 'partial'},
        {month: '2026-02', administration: 2, declarants: 1, total: 3, status: 'partial'},
        {month: '2026-03', administration: 0, declarants: 0, total: 0, status: 'available'},
        {month: '2026-04', administration: 0, declarants: 1, total: 1, status: 'available'}
      ]
    })
  })
})
