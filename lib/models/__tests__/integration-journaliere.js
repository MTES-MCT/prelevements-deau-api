...
test('integration-journaliere: idempotence insertion', async t => {
  ...
  const first = await insertIntegration({preleveurId, pointId}, date, {demarcheNumber: 1, dossierNumber: 10, attachmentId: 'att-1'})
  const second = await insertIntegration({preleveurId, pointId}, date, {demarcheNumber: 1, dossierNumber: 10, attachmentId: 'att-1'})
