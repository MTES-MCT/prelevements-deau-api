...
test('searchDeclarants filtre puis classe uniquement les déclarants autorisés', async t => {
  ...
  t.deepEqual(
    userQueries[0].select.declarant.select.pointPrelevements.where,
    {
      pointPrelevement: {
  ...
  t.deepEqual(
    userQueries[0].select.declarant.select.pointPrelevements.select.collecteurs.where,
