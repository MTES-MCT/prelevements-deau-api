...
test('computeRegularisationsStats tient compte des usages secondaires sans double compter', t => {
  const exploitation = {
    usage: {id: 'usage-6', code: '6', label: 'Énergie'},
    secondaryUsageLinks: [
      {usage: {id: 'usage-4', code: '4', label: 'Industrie'}},
      {usage: {id: 'usage-4', code: '4', label: 'Industrie'}}
    ],
    documents: []
  }
  ...
  const byRegime = new Map(
    computeRegularisationsStats([exploitation], now).map(row => [row.regime, row])
  )

  t.is(byRegime.get('ICPE').nb_exploitations_concernees, 1)
  t.is(byRegime.get('Hydroélectricité').nb_exploitations_concernees, 1)
})
