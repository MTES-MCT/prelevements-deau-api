...
import {exploitationAtDateWhere} from '../services/exploitation-periods.js'

export function activeWindowWhere(now = new Date(), {startNullable = true, endNullable = true} = {}) {
  return exploitationAtDateWhere(now, {
    startNullable,
    endNullable,
...
function getDeclarantMapRelationWhere(declarantUserId) {
  if (!declarantUserId) {
    return undefined
  }
  ...
  return {
    OR: [
      {declarantUserId},
      {collecteurs: {some: {collecteurUserId: declarantUserId}}}
    ]
  }
}
...
async function findPointMapSummaries(where, {
  ...
  const declarantWhere = getDeclarantMapRelationWhere(declarantUserId)
  const points = await client.pointPrelevement.findMany({
    where,
    select: {
      ...POINT_MAP_SUMMARY_SELECT,
      declarants: {
        ...POINT_MAP_SUMMARY_SELECT.declarants,
        ...(declarantWhere ? {where: declarantWhere} : {})
      }
    },
    orderBy: {name: 'asc'}
  })
...
export function getPointMapSummaries(includeDeleted = false, {client = prisma} = {}) {
  return findPointMapSummaries(computeDeletionCondition(includeDeleted), {client})
}
  ...
  await prisma.$transaction(async tx => {
    await tx.$executeRaw`
      ...
      VALUES (
        ...
        CAST(${data.pointKind ?? 'PHYSIQUE'} AS "PointKind"),
        CAST(${data.collectionMode ?? null} AS "PointCollectionMode"),
        CAST(${data.nature ?? null} AS "PointPrelevementNature"),
        CAST(${data.withdrawalType ?? null} AS "PrelevementType"),
        CAST(${data.commissioningDate ?? null} AS DATE),
        ${data.waterAgencyInternalIdentifier ?? null},
        ${data.isReferencePoint ?? null},
        ${data.isWaterBodyConnectedToStream ?? null},
        ${data.isWaterBodyConnectedToGroundwater ?? null},
        ${data.reservoirNominalVolume ?? null},
        ${data.waterBodyIdentifier ?? null},
        ST_SetSRID(ST_MakePoint(${longitude}, ${latitude}), 4326),
        ${data.sourceId ?? null},
