...
function meterScopeWhere({pointIds = [], preleveurId, collecteurId, pointFlowType, exploitationId}) {
  return {
    deletedAt: null,
...
export async function listMeterReadingSeriesOptions({user, sourceId, client = prisma, ...scope}) {
  ...
  return meters.filter(meter => byMeter.has(meter.id)).map(meter => {
    ...
    return {
      id: `index:meter:${meter.id}`, name: 'index', meterId: meter.id, meter, readingSeries: true,
      label: `Index — compteur ${meter.serialNumber ?? meter.identifier ?? 'sans numéro'}`,
