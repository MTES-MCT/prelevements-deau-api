...
import {getExploitationsForDecorating, hasActiveExploitationForPoint, getPointIdsFromPreleveur} from './exploitation.js'
import {getPreleveursFromIds} from './preleveur.js'
...
export async function decoratePointPrelevement(point) {
  const exploitations = await getExploitationsForDecorating(point._id, {
    id_exploitation: 1,
    statut: 1,
    date_debut: 1,
    preleveur: 1,
  ...
  const preleveursIds = chain(exploitations).map(e => e.preleveur).uniqBy(id => id.toString()).value()

  const preleveurs = await getPreleveursFromIds(preleveursIds)
...
export async function deletePointPrelevement(pointId) {
  const hasActiveExploitation = await hasActiveExploitationForPoint(pointId)

  if (hasActiveExploitation) {
    // We need to get the exploitation ID for the error message
...
export async function getPointsFromPreleveur(preleveurId) {
  const pointIds = await getPointIdsFromPreleveur(preleveurId)
...
export async function countPointsPrelevement(territoire) {
  return mongo.db.collection('points_prelevement').countDocuments({
    deletedAt: {$exists: false},
    ...(territoire && {territoire})
  })
}

export async function getActivePointsByType(pointIds, typeMilieu, territoire) {
  return mongo.db.collection('points_prelevement').find({
    _id: {$in: pointIds},
