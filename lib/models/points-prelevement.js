...
export async function decoratePointPrelevement(point) {
  ...
  const preleveursIds = uniq(exploitations.map(e => e.id_preleveur))

  const preleveurs = await mongo.db.collection('preleveurs').find({
    id_preleveur: {$in: preleveursIds}
  }).toArray()
...
export async function getStats(territoire) {
  ...
  const termineesExploitations = await mongo.db.collection('exploitations').find({
    statut: 'Terminée',
    deletedAt: {$exists: false},
    ...(territoire && {territoire})
  }).toArray()
  const abandonneesExploitations = await mongo.db.collection('exploitations').find({
    statut: 'Abandonnée',
    deletedAt: {$exists: false},
    ...(territoire && {territoire})
  }).toArray()
  const nonRenseigneExploitations = await mongo.db.collection('exploitations').find({
  ...
  for (const e of activExploitations) {
    activPreleveurs.push(e.id_preleveur)
  ...
  return {
    ...
    termineesExploitations: termineesExploitations.length,
    abandonneesExploitations: abandonneesExploitations.length,
    nonRenseigneExploitations: nonRenseigneExploitations.length,
