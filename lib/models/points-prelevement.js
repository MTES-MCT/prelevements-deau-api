...
export async function getPointPrelevement(idPoint) {
  ...
  const meContinentalesBv = storage.meContinentalesBv.find(m => m.code_dce === point.code_me_continentales_bv)
  const bvBdCarthage = storage.bvBdCarthage.find(b => b.code_cours === point.code_bv_bdcarthage)
  const meso = storage.meso.find(m => m.code === point.code_meso)
