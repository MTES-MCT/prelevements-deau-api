...
import {synchronizeDeclarantPrimaryContactEmail} from './user-email-verification.js'
...
function normalizeDeclarant(declarant) {
  ...
  const normalizeExploitation = exploitation => {
    if (!exploitation) {
      return exploitation
    }

    const documentsById = new Map((exploitation.documents ?? [])
    ...
    const normalized = {
      ...exploitation,
      ...(Object.hasOwn(exploitation, 'documents') || Object.hasOwn(exploitation, 'documentLinks')
        ? {documents: [...documentsById.values()]}
        : {})
    }
    delete normalized.documentLinks

    return Object.hasOwn(exploitation, 'usage')
      || Object.hasOwn(exploitation, 'secondaryUsageLinks')
      || Object.hasOwn(exploitation, 'secondaryUsages')
      ? serializeExploitationUsageFields(normalized)
      : normalized
  }
  ...
  return {
    ...
    loginEmail: declarant.user?.email ?? null,
    contactEmails: serializeDeclarantContactEmails(declarant),
    firstName: declarant.user?.firstName ?? null,
    lastName: declarant.user?.lastName ?? null,
    ...(Array.isArray(declarant.pointPrelevements)
      ? {pointPrelevements: declarant.pointPrelevements.map(normalizeExploitation)}
      : {}),
    ...(Array.isArray(declarant.collecteurExploitations)
      ? {
        collecteurExploitations: declarant.collecteurExploitations.map(link => ({
          ...link,
          exploitation: normalizeExploitation(link.exploitation)
        }))
      }
      : {})
  }
...
export async function getDeclarantSearchExploitations(
  ...
  const exploitationSearchColumns = includeSearchDocuments
    ? Prisma.sql`,
      concat_ws(
        ...
        exploitation_usage_search.human_text,
      ...
      concat_ws(
        ...
        point."codeAIOT",
        point."codePTP",
        exploitation_usage_search.identifier_text,
        ...
        coalesce(
          (
            ...
            WHERE contact."declarantUserId" = exploitation_declarant."userId"
          ),
          CASE
            WHEN lower(exploitation_declarant_user.email::text) NOT LIKE '%@import.local'
              THEN exploitation_declarant_user.email::text
            ELSE NULL
          END,
          ''
        )
  ...
  const exploitationSearchJoins = includeSearchDocuments
    ? Prisma.sql`
      ...
      LEFT JOIN LATERAL (
        ...
        FROM exploitation_usages exploitation_usage
        WHERE exploitation_usage.exploitation_id = exploitation.id
      ) exploitation_usage_search ON true
    `
    : Prisma.empty
  ...
  return client.$queryRaw(Prisma.sql`
    ...
    exploitation_usages AS MATERIALIZED (
      SELECT
        exploitation.id AS exploitation_id,
        water_use.id,
        water_use.code,
        water_use.label,
        water_use.mnemonic
      ...
      SELECT
        secondary_usage."exploitationId" AS exploitation_id,
        water_use.id,
        water_use.code,
        water_use.label,
        water_use.mnemonic
      ...
      JOIN "DeclarantPointPrelevementSecondaryUsage" secondary_usage
        ON secondary_usage."exploitationId" = scoped_exploitation.exploitation_id
      JOIN "SandreWaterUse" water_use
    ...
    declarant_exploitation_summaries AS MATERIALIZED (
      SELECT
        ...
        array_agg(
          DISTINCT exploitation_summary."pointPrelevementId"
          ORDER BY exploitation_summary."pointPrelevementId"
        ) AS "pointPrelevementIds",
        array_agg(
    ...
    declarant_usages AS MATERIALIZED (
      SELECT
        candidate_exploitation.declarant_id,
        exploitation_usage.code AS "usageCode",
        max(exploitation_usage.label) AS "usageLabel",
        count(DISTINCT exploitation_usage.exploitation_id) AS occurrence
      FROM candidate_exploitations candidate_exploitation
      JOIN exploitation_usages exploitation_usage
        ON exploitation_usage.exploitation_id = candidate_exploitation.exploitation_id
      GROUP BY
        candidate_exploitation.declarant_id,
        exploitation_usage.code
    ),
    ...
    SELECT
      ...
      declarant_summary."pointPrelevementIds",
...
function attachDeclarantSearchExploitations(records, exploitationRows) {
  ...
  for (const row of exploitationRows) {
    byDeclarantId.set(row.declarantUserId, {
      ...
      pointPrelevementIds: row.pointPrelevementIds ?? [],
...
function getDeclarantInMemorySearchDocument(user) {
  ...
  return {
    ...
    identifierText: [
      user.id,
      ...getEffectiveDeclarantContactEmails(user),
      declarant.siret,
...
function normalizeSearchRecord(user, zoneLinks, now) {
  ...
  const usages = exploitationSummary
    ...
    : exploitations
      .flatMap(exploitation => getExploitationWaterUses(exploitation)
        .map(usage => ({...usage, occurrence: 1})))
  ...
  return {
    ...
    emailStatus: hasDeclarantContactEmail(user) ? 'WITH_EMAIL' : 'WITHOUT_EMAIL',
    pointCount: exploitationSummary
      ? uniqueValues(exploitationSummary.pointPrelevementIds ?? []).length
      : uniqueValues(exploitations.map(exploitation => exploitation.pointPrelevementId)).length,
...
async function buildSearchResult({
  ...
  const decorateItem = record => {
    ...
    const searchSummary = {
      ...
      ...(filters.format === 'compact' ? {pointCount: record.pointCount} : {})
    }

    return Object.keys(searchSummary).length > 0
      ? {...item, searchSummary}
      : item
  }
...
export async function searchDeclarants(user, searchFilters = {}, {
  ...
  const instructorScope = user?.role === 'INSTRUCTOR'
    ? await withRequestPerformancePhase(
      'search_scope',
      async () => getInstructorDeclarantListScope(user.id, {client, now})
    )
    : null
  ...
  const includeSearchDocuments = Boolean(filters.query)
  const corpus = await cache.getOrLoad({
    scope: {
      kind: 'declarants',
      includeSearchDocuments,
      user: {id: user?.id, role: user?.role},
      rights: instructorScope
        ? {
          declarantUserIds: instructorScope.declarantUserIds,
          declarantZoneIds: instructorScope.declarantZoneIds,
          exploitationZoneIds: instructorScope.exploitationZoneIds
        }
        : {administrator: true}
    },
    async loader() {
      const baseRecords = await withRequestPerformancePhase(
        'search_base',
        async () => client.user.findMany({
          where: baseWhere,
          select: getDeclarantSearchSelect({includeSearchDocuments})
        })
      )
      ...
      const [exploitationRows, zoneLinksByDeclarantId] = await Promise.all([
        withRequestPerformancePhase(
          'search_exploitations',
          async () => getDeclarantSearchExploitations(
            candidateIds,
            instructorScope?.exploitationZoneIds,
            {client, includeSearchDocuments}
          )
        ),
        withRequestPerformancePhase(
          'search_zones',
          async () => getZoneLinksForSearchRecords(
            baseRecords,
      ...
      return {
        searchRecords: attachDeclarantSearchExploitations(
          baseRecords,
          exploitationRows
        ),
        zoneLinksByDeclarantId: [...zoneLinksByDeclarantId]
      }
    }
  })
  const {searchRecords} = corpus
  const zoneLinksByDeclarantId = new Map(corpus.zoneLinksByDeclarantId)
  const searchDocumentsById = new Map(searchRecords.map(record => [
    record.id,
    getDeclarantInMemorySearchDocument(record)
  ]))
  const hydrate = ids => withRequestPerformancePhase('search_hydrate', async () => {
    if (ids.length === 0) {
      return []
    }
  ...
  return withRequestPerformancePhase('search_build', async () => buildSearchResult({
    ...
    now,
    includeExploitationRelations,
    rank: (rankedCandidateIds, query) => withRequestPerformancePhase(
      'search_rank',
      () => rankSearchDocuments(
        rankedCandidateIds
...
export async function searchCollecteurPreleveurs(
  collecteurUserId,
  searchFilters = {},
  {cache = searchCorpusCache, client = prisma, now = new Date()} = {}
) {
  const baseRecords = await cache.getOrLoad({
    scope: {
      kind: 'collector-preleveurs',
      user: {id: collecteurUserId, role: 'COLLECTEUR'}
    },
    phases: {
      hit: 'collector_cache_hit',
      load: 'collector_cache_load',
      miss: 'collector_cache_miss'
    },
    loader: async () => withRequestPerformancePhase(
      'collector_base',
      async () => getCollecteurPreleveurSearchRecords(
        collecteurUserId,
        {client}
      )
    )
  ...
  const hydrate = ids => withRequestPerformancePhase('collector_hydrate', async () => {
  ...
  return withRequestPerformancePhase('collector_build', async () => buildSearchResult({
    baseRecords,
    filters,
    zoneLinksByDeclarantId: new Map(),
    client,
    now,
    rank: (candidateIds, query, options) => withRequestPerformancePhase(
      'collector_rank',
      async () => rankCollecteurPreleveurIds(
...
export async function updateDeclarantById(declarantUserId, changes, {
  client = prisma,
  synchronizePrimaryContactEmail = synchronizeDeclarantPrimaryContactEmail
} = {}) {
  ...
  try {
    await client.$transaction(async tx => {
      ...
      const primaryEmailChanges = Object.hasOwn(userData, 'email')
        && normalizeEmail(existingUser.email) !== userData.email

      if (primaryEmailChanges) {
        ...
        await Promise.all([
          tx.authToken.deleteMany({
            where: {userId: declarantUserId}
          }),
          tx.passwordActivation.deleteMany({
            where: {userId: declarantUserId}
          }),
          tx.sessionToken.deleteMany({
            where: {
              OR: [
                {userId: declarantUserId},
      ...
      if (primaryEmailChanges) {
        await synchronizePrimaryContactEmail(tx, {
          userId: declarantUserId,
    ...
    if (error?.code === 'P2002'
      || errorMessage.includes('User_email_not_alias')
      || errorMessage.includes('User_email_reserved')
      || errorMessage.includes('UserEmailIdentity_compatible_claims_check')) {
      throw createHttpError(409, 'Email déjà utilisé ou en cours de validation.')
    }
...
function groupCollecteurPreleveurs(links) {
  ...
  for (const link of links) {
    ...
    current.declarant.collecteurExploitations.push({
      ...
      exploitation: serializeExploitationUsageFields(link.exploitation)
    })
    const pointPrelevementIds = new Set(
      current.declarant.collecteurExploitations
        .map(item => item.exploitation?.pointPrelevementId)
        .filter(Boolean)
    )
    current.declarant._count.pointPrelevements = pointPrelevementIds.size > 0
      ? pointPrelevementIds.size
      : current.declarant.collecteurExploitations.length
...
function serializeCollecteurDeclarationTargets(users) {
  return users
    .filter(user => user.declarant)
    .map(user => ({
      ...user,
...
export async function getCollecteurDeclarationTargets(collecteurUserId, {
  client = prisma
} = {}) {
  ...
  return serializeCollecteurDeclarationTargets(users)
}
