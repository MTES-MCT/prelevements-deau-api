...
const SEARCH_EXPLOITATION_SELECT = Object.freeze({
  id: true,
  status: true,
  usage: {
    select: {
...
function exploitationSearchWhere(exploitationZoneIds) {
  return Array.isArray(exploitationZoneIds)
    ? {
      pointPrelevement: {
        zones: {
          some: {zoneId: {in: exploitationZoneIds}}
        }
      }
    }
    : undefined
...
function getDeclarantSearchSelect(exploitationZoneIds) {
  const exploitationWhere = exploitationSearchWhere(exploitationZoneIds)
    ...
    declarant: {
      select: {
        ...
        pointPrelevements: {
          ...
          select: SEARCH_EXPLOITATION_SELECT
        },
        collecteurExploitations: {
          ...(exploitationWhere ? {where: {exploitation: exploitationWhere}} : {}),
          select: {
            exploitation: {
              select: SEARCH_EXPLOITATION_SELECT
            }
...
function createDeclarantFacets(records) {
  return {
...
async function buildSearchResult({
  ...
  return {
    ...
    facets: createDeclarantFacets(records)
  }
}
...
export async function searchDeclarants(user, searchFilters = {}, {
  ...
  const baseRecords = await client.user.findMany({
    where: baseWhere,
    select: getDeclarantSearchSelect(instructorScope?.exploitationZoneIds)
  })
  const zoneLinksByDeclarantId = await getZoneLinksForSearchRecords(
    baseRecords,
