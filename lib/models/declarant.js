...
function getDeclarantSearchSelect() {
  return {
    id: true,
    email: true,
    firstName: true,
...
function getSearchExploitationZonePredicate(exploitationZoneIds) {
  ...
  return Prisma.sql`AND EXISTS (
    ...
    WHERE point_zone."pointPrelevementId" = exploitation."pointPrelevementId"
      AND point_zone."zoneId" IN (${Prisma.join(
        exploitationZoneIds.map(zoneId => Prisma.sql`${zoneId}::uuid`)
      )})
  )`
}
...
export async function getDeclarantSearchExploitations(
  ...
  return client.$queryRaw(Prisma.sql`
    ...
    SELECT
      candidate_exploitation.declarant_id AS "declarantUserId",
      exploitation.id,
      exploitation.status,
      ...
      coalesce((
        ...
        WHERE point_zone."pointPrelevementId" = point.id
      ), '[]'::jsonb) AS zones,
      EXISTS (
        SELECT 1
        FROM "DeclarantCollecteurExploitation" active_collector_link
        ...
        WHERE active_collector_link."exploitationId" = exploitation.id
      ) AS "hasCollecteur",
      EXISTS (
        SELECT 1
        FROM "DeclarantPointPrelevementConnector" connector
        WHERE connector."declarantPointPrelevementId" = exploitation.id
      ) AS "hasConnector"
...
function attachDeclarantSearchExploitations(records, exploitationRows) {
  ...
  for (const row of exploitationRows) {
    const current = byDeclarantId.get(row.declarantUserId) ?? []
    current.push({
      id: row.id,
      status: row.status,
      usage: row.usageCode
        ? {code: row.usageCode, label: row.usageLabel}
        : null,
      pointPrelevement: {
        waterBodyType: row.waterBodyType,
        zones: (row.zones ?? []).map(zone => ({zone}))
      },
      collecteurs: row.hasCollecteur ? [{}] : [],
      connectors: row.hasConnector ? [{}] : []
    })
    byDeclarantId.set(row.declarantUserId, current)
  }
  ...
  return records.map(record => ({
    ...
    declarant: {
      ...record.declarant,
      searchExploitations: byDeclarantId.get(record.id) ?? []
    }
  }))
}
...
function normalizeSearchRecord(user, zoneLinks, now) {
  ...
  const exploitations = Array.isArray(declarant.searchExploitations)
    ? declarant.searchExploitations
    : [
  ...
  for (const exploitation of exploitations) {
    for (const link of exploitation.pointPrelevement?.zones ?? []) {
      if (link.zone) {
        zoneDetails.set(link.zone.id, link.zone)
      }
    }

    if (exploitation.usage?.code) {
      const code = getRootWaterUseCode(exploitation.usage.code)

      if (code) {
        usageDetails.set(code, getWaterUse(code) ?? exploitation.usage)
      }
    }
  }
  ...
  return {
    ...
    collecteurStatus: exploitations.some(exploitation => exploitation.collecteurs?.length > 0)
      ? 'WITH_COLLECTEUR'
      : 'WITHOUT_COLLECTEUR',
    connectorStatus: exploitations.some(exploitation => exploitation.connectors?.length > 0)
      ? 'WITH_CONNECTOR'
      : 'WITHOUT_CONNECTOR',
    zoneIds: zones,
    zoneDetails,
    ...
    exploitationStatuses: uniqueValues(exploitations.map(exploitation => exploitation.status))
  }
}
...
async function buildSearchResult({
  ...
  return {
    items: pageRecords.map(record => itemById.get(record.id)).filter(Boolean),
    total,
    page: filters.page,
...
function getDeclarantSearchScopesById(records) {
  return new Map(records.map(item => [
    ...
    {
      declarantId: item.id,
      exploitationIds: uniqueValues(Array.isArray(item.declarant?.searchExploitations)
        ? item.declarant.searchExploitations.map(exploitation => exploitation.id)
        : [
...
export async function searchDeclarants(user, searchFilters = {}, {
  ...
  const scopesByDeclarantId = getDeclarantSearchScopesById(searchRecords)
  ...
  return buildSearchResult({
    ...
    now,
    includeExploitationRelations,
    rank: (candidateIds, query, options) => rankScopedDeclarantIds(
      candidateIds.map(id => scopesByDeclarantId.get(id)).filter(Boolean),
      query,
      options
    ),
    hydrate: ids => ids.length === 0
      ? []
      : client.user.findMany({
        where: {AND: [baseWhere, {id: {in: ids}}]},
        include: getDeclarantListInclude({
...
export async function searchCollecteurPreleveurs(
  ...
  const baseRecords = await getCollecteurPreleveurs(collecteurUserId, {client})
  const filters = {
  ...
  return buildSearchResult({
    ...
    hydrate: ids => ids.map(id => byId.get(id)).filter(Boolean)
  })
}
...
export async function getCollecteurPreleveurs(collecteurUserId, {client = prisma} = {}) {
