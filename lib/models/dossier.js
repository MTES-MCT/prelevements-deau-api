...
import {addJobProcessAttachment, addJobConsolidateDossier} from '../queues/jobs.js'
...
export async function markDossierForReconsolidation(dossierId) {
  ...
  // Créer immédiatement un job de consolidation au lieu d'attendre la maintenance
  if (result) {
...
export async function createAttachment({dossierId, ds, territoire, typePrelevement, storageKey}) {
  const result = await mongo.db.collection('dossier_attachments').insertOne({
    dossierId,
    ds,
    territoire,
    typePrelevement,
  ...
  // Créer un job pour traiter ce nouvel attachment
  await addJobProcessAttachment(result.insertedId.toString())
}
...
export async function updateAttachment(attachmentId, changes) {
  ...
  if (changes.processed) {
    ...
    // Créer un job pour consolider ce dossier
    await addJobConsolidateDossier(dossierId.toString())
  }
}
...
export async function markAttachmentForReprocessing(attachmentId) {
  const result = await mongo.db.collection('dossier_attachments').findOneAndUpdate(
    {_id: attachmentId},
    {$set: {processed: false}, $unset: {result: 1, validationStatus: 1, processingError: 1}},
    {returnDocument: 'after'}
  )

  // Créer immédiatement un job de traitement au lieu d'attendre la maintenance
  if (result) {
