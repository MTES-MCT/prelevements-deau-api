...
export async function createDossier(data, files, validationReport) {
  ...
  const dossier = {
    ...
    isValid: Object.keys(validationReport).length === 0,
    validationReport,
    files,
    createdAt: new Date(),
    updatedAt: new Date()
...
export async function updateDossier(mongoId, data, files, validationReport) {
  const dossierData = omit(data, dossierDatatoOmit)
