...
export async function getDossiers(query = {}) {
  return mongo.db.collection('dossiers').find(query).sort({dateDepot: -1}).toArray()
}
...
export async function getUnconsolidatedDossiers() {
  ...
  return mongo.db.collection('dossiers')
    .find(query, {projection: {_id: 1}}) // eslint-disable-line unicorn/no-array-method-this-argument
    .toArray()
}
...
export async function createAttachment({dossierId, typePrelevement, storageKey}) {
  const attachment = {
    _id: new ObjectId(),
    dossierId,
    typePrelevement,
