...
import {getFileS3Key} from '../util/demarches-simplifies/index.js'
...
export async function getDossierFile(dossierId, checksum) {
  ...
  const file = dossier.files.find(f => f.checksum === checksum) || null

  if (!file) {
    return null
  }

  const key = getFileS3Key(dossier.number, file.filename)
