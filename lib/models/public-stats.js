...
import {PUBLIC_STATS_DEPLOYMENT_MONTHS} from '../constants/public-stats-deployment-months.js'
...
export function listPublicStatsMonths(firstMonth, lastMonth) {
  ...
  const months = []
  // YYYY-MM values compare lexicographically, not numerically.
...
export function resolvePublicStatsDeploymentMonth({type, code}, deploymentMonths = PUBLIC_STATS_DEPLOYMENT_MONTHS) {
  const month = deploymentMonths[`${type}:${code}`]
  return typeof month === 'string' && MONTH_PATTERN.test(month) && month >= MIN_SUPPORTED_MONTH ? month : null
}
  ...
  return Prisma.sql`
    WITH live_preleveurs AS MATERIALIZED (
      ...
      SELECT c.id, c."pointPrelevementId" AS "pointId", c."preleveurUserId",
        c."minDate", c."maxDate", c."calculationStrategy",
        declared_preleveur."userId" AS "declarationPreleveurId",
        CASE
      ...
      -- METER publications store physical UTC instants in timestamp columns;
      -- legacy GENERIC values already store business calendar dates.
      SELECT cv."chunkId", cv."metricTypeCode",
        CASE WHEN c."calculationStrategy" = 'METER'
          THEN (cv."periodStart" AT TIME ZONE 'UTC') AT TIME ZONE 'Europe/Paris'
          ELSE cv."periodStart" END AS "periodStart",
        CASE WHEN c."calculationStrategy" = 'METER'
          THEN (cv."periodEnd" AT TIME ZONE 'UTC') AT TIME ZONE 'Europe/Paris'
          ELSE cv."periodEnd" END AS "periodEnd"
      FROM "ChunkValue" cv
      ...
      SELECT DISTINCT z.id, z.name, z.type, z.code
      FROM "Zone" z
      ...
      SELECT z.id, z.name, z.type, z.code,
        ...
        COUNT(DISTINCT r."userId") FILTER (WHERE d."preleveurType" = 'IRRIGANT') AS "reportingAgriculture",
        COUNT(DISTINCT r."userId") FILTER (WHERE d."preleveurType" = 'ICPE') AS "reportingIndustry",
        COUNT(DISTINCT r."userId") FILTER (WHERE d."preleveurType" = 'GESTIONNAIRE_AEP') AS "reportingDrinkingWater",
        COUNT(DISTINCT r."userId") FILTER (WHERE d."preleveurType" = 'AUTRE') AS "reportingOther",
        COUNT(DISTINCT r."userId") FILTER (WHERE d."preleveurType" IS NULL) AS "reportingUnknown"
      ...
      GROUP BY z.id, z.name, z.type, z.code, points.count
...
export function buildPublicStatsActiveUsersQuery(month) {
  const firstMonth = shiftPublicStatsMonth(month, -5)
  const nextMonth = shiftPublicStatsMonth(month, 1)

  return Prisma.sql`
    WITH collection_state AS (
      SELECT "startedAt",
        TO_CHAR(("startedAt" AT TIME ZONE 'UTC') AT TIME ZONE 'Europe/Paris', 'YYYY-MM') AS month
      FROM "UserActivityCollectionState" WHERE id = 'active-users'
    ), first_event AS (
      SELECT MIN("occurredAt") AS "availableSince"
      FROM "AuditEvent"
      WHERE "actionType" IN (
      ...
      SELECT DISTINCT ON (month, "subjectUserId")
        TO_CHAR(("occurredAt" AT TIME ZONE 'UTC') AT TIME ZONE 'Europe/Paris', 'YYYY-MM') AS month,
        "subjectUserId" AS "userId", "subjectUserRole" AS role
      FROM "AuditEvent"
      WHERE outcome = 'SUCCESS' AND "subjectUserId" IS NOT NULL
        AND "actionType" IN ('AUTH.LOGIN_VERIFIED', 'AUTH.PASSWORD_LOGIN_VERIFIED', 'AUTH.PASSWORD_ACTIVATED')
        AND "occurredAt" >= (${`${firstMonth}-01`}::timestamp AT TIME ZONE 'Europe/Paris') AT TIME ZONE 'UTC'
        AND "occurredAt" < (${`${nextMonth}-01`}::timestamp AT TIME ZONE 'Europe/Paris') AT TIME ZONE 'UTC'
      ORDER BY month, "subjectUserId", "occurredAt" DESC, id DESC
    ), activity_candidates AS (
      SELECT month, "userId", role::text, 0 AS priority
      FROM "UserMonthlyActivity"
      WHERE month >= ${firstMonth} AND month < ${nextMonth}
      UNION ALL
      SELECT month, "userId", role, 1 AS priority
      FROM successful_connections
      WHERE NOT EXISTS (SELECT 1 FROM collection_state)
        OR month <= (SELECT month FROM collection_state)
    ), unique_activity AS (
      SELECT DISTINCT ON (month, "userId") month, "userId", role
      FROM activity_candidates
      ORDER BY month, "userId", priority
    ), counts AS (
      SELECT month,
        COUNT(*) FILTER (WHERE role IN ('ADMIN', 'INSTRUCTOR')) AS administration,
        COUNT(*) FILTER (WHERE role = 'DECLARANT') AS declarants
      FROM unique_activity
      WHERE role IN ('ADMIN', 'INSTRUCTOR', 'DECLARANT')
      GROUP BY month
    )
    SELECT LEAST(
        (SELECT "availableSince" FROM first_event),
        (SELECT "startedAt" FROM collection_state)
      ) AS "availableSince",
      (SELECT "startedAt" FROM collection_state) AS "collectionStartedAt",
      COALESCE((SELECT JSON_AGG(counts) FROM counts), '[]'::json) AS months
  `
}

export function summarizePublicStatsActiveUsers({
  month, availableSince = null, collectionStartedAt = null, months = []
}) {
  const firstRecordedAt = availableSince ? new Date(availableSince) : null
  const collectionStartedDate = collectionStartedAt ? new Date(collectionStartedAt) : null
  const collectionStartedMonth = collectionStartedDate ? getParisMonth(collectionStartedDate) : null
  const byMonth = new Map(months.map(row => [row.month, row]))

  return {
    months: Array.from({length: 6}, (_, index) => {
      const key = shiftPublicStatsMonth(month, index - 5)
      const row = byMonth.get(key)
      const complete = collectionStartedMonth !== null && key > collectionStartedMonth
      const collectionMonth = key === collectionStartedMonth
      // A historical absence of successful logins does not prove an absence of usage.
      const unavailable = !complete && !collectionMonth && !row
      const administration = unavailable ? null : Number(row?.administration ?? 0)
      const declarants = unavailable ? null : Number(row?.declarants ?? 0)
      return {
        month: key,
        ...
        declarants,
        total: unavailable ? null : administration + declarants,
        status: unavailable ? 'unavailable' : (complete ? 'available' : 'partial')
      }
    }),
    availableSince: firstRecordedAt?.toISOString() ?? collectionStartedDate?.toISOString() ?? null,
    collectionStartedAt: collectionStartedDate?.toISOString() ?? null
  }
}

function serializeTerritory(row) {
  const profileCounts = [row.agriculture, row.industry, row.drinkingWater, row.other, row.unknown]
  const reportingProfileCounts = [row.reportingAgriculture, row.reportingIndustry, row.reportingDrinkingWater, row.reportingOther, row.reportingUnknown]
  const preleveursCount = Number(row.preleveursCount)
  const reportingPreleveursCount = Number(row.reportingPreleveursCount)
  return {
    id: row.id,
    name: row.name,
    deploymentMonth: resolvePublicStatsDeploymentMonth(row),
    ...
    profiles: PROFILE_DEFINITIONS.map(({key, label}, index) => ({
      key, label, count: Number(profileCounts[index] ?? 0), reportingCount: Number(reportingProfileCounts[index] ?? 0)
    }))
  }
}

export async function loadPublicStats({month, client = prisma, now = new Date()}) {
  const [[data], [connections], [activeUsers]] = await Promise.all([
    client.$queryRaw(buildPublicStatsDataQuery(month)),
    client.$queryRaw(buildPublicStatsConnectionsQuery(month)),
    client.$queryRaw(buildPublicStatsActiveUsersQuery(month))
  ])
  ...
  return {
    ...
    activeUsers: summarizePublicStatsActiveUsers({month, ...activeUsers})
  }
}
