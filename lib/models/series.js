...
/**
 * Calcule la couverture temporelle en heures basée sur le nombre de valeurs et la fréquence.
 * @param {Array} values - Valeurs infra-journalières
 * @param {string} frequency - Fréquence (ex: '15 minutes', '1 hour', '1 second')
 * @returns {number} - Couverture en heures (peut être 23, 24 ou 25 pour les changements d'heure)
 */
function calculateCoverageHours(values, frequency) {
  // Mapper la fréquence vers un nombre d'heures par valeur
  ...
  const hoursPerValue = frequencyToHours[frequency]

  if (!hoursPerValue) {
    // Fréquence inconnue, estimer en comptant les valeurs
    // On suppose que les valeurs sont uniformément réparties
    return Math.min(values.length / 4, 25) // Max 25h pour changement d'heure
  }

  return Math.min(values.length * hoursPerValue, 25)
}
