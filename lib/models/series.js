...
import {SUB_DAILY_FREQUENCIES} from '../parameters-config.js'
...
function calculateCoverageHours(values, frequency) {
  ...
  const minTime = min(timeValues)
  const maxTime = max(timeValues)
...
export async function insertSeriesWithValues({attachmentId, dossierId, series}) {
  if (!dossierId) {
  ...
  const seriesDocs = series.map(s => ({
    ...
    hasSubDaily: SUB_DAILY_FREQUENCIES.includes(s.frequency),
...
//  - attachmentId : liste toutes les séries d'une pièce jointe (ignores autres critères sauf startDate/endDate)
//  - pointIds : filtre sur computed.point (accepte un tableau d'ObjectIds ou un seul ObjectId dans un tableau)
//  - preleveurId : filtre sur computed.preleveur
//  - startDate / endDate : limite temporelle sur les jours intégrés (integratedDays) ou sur minDate/maxDate selon onlyIntegratedDays
//  - onlyIntegratedDays : si true, l'intervalle startDate/endDate est appliqué sur la présence de jours dans computed.integratedDays.
...
export function buildOverlapFilter({startDate, endDate, onlyIntegratedDays}) {
...
export function buildAttachmentQuery({attachmentId, startDate, endDate}) {
  const query = {attachmentId}

  if (startDate) {
    query.maxDate = {$gte: startDate}
  }

  if (endDate) {
    query.minDate = query.minDate ? {...query.minDate, $lte: endDate} : {$lte: endDate}
  }

  return query
}

export function buildPointPreleveurQuery({pointIds, preleveurId, parameter, startDate, endDate, onlyIntegratedDays}) {
  const query = {}

  // Support de pointIds (tableau d'ObjectIds)
  if (pointIds && Array.isArray(pointIds) && pointIds.length > 0) {
    query['computed.point'] = pointIds.length === 1 ? pointIds[0] : {$in: pointIds}
  }
  ...
  if (startDate || endDate) {
    if (onlyIntegratedDays) {
      ...
      query['computed.integratedDays'] = {$elemMatch: elemMatch}
    } else {
      if (startDate) {
        query.maxDate = {$gte: startDate}
      }
  ...
  } else if (onlyIntegratedDays) {
    // Si onlyIntegratedDays est true mais qu'il n'y a pas de contrainte de dates,
    // on vérifie simplement l'existence du champ
    query['computed.integratedDays'] = {
      $exists: true,
      $type: 'array',
      $not: {$size: 0}
    }
...
export async function listSeries({
  ...
  // Cas 1: attachmentId fourni -> on ignore pointIds/preleveurId et on retourne directement les séries de la pièce jointe.
  const overlapFilter = buildOverlapFilter({startDate, endDate, onlyIntegratedDays})

  if (attachmentId) {
    const raw = await col.find(buildAttachmentQuery({attachmentId, startDate, endDate})).project(projection).toArray()
    return overlapFilter(raw)
  }

  // Cas 2: recherche par pointIds / preleveurId (au moins un des deux doit être fourni)
  const baseQuery = buildPointPreleveurQuery({pointIds, preleveurId, parameter, startDate, endDate, onlyIntegratedDays})

  if (!((pointIds && pointIds.length > 0) || preleveurId)) {
  ...
  // Fallback: si filtrage par pointIds demandé mais aucune série (computed.point non encore peuplé),
  // retenter via champ brut pointPrelevement
  if (pointIds && pointIds.length > 0 && candidates.length === 0) {
    const fallbackQuery = {...baseQuery}
    delete fallbackQuery['computed.point']

    fallbackQuery.pointPrelevement = pointIds.length === 1 ? pointIds[0] : {$in: pointIds}
...
export async function getSeriesValuesInRange(seriesId, {startDate, endDate, useAggregates = false}) {
  ...
  // Mais on ne doit jamais utiliser dailyAggregates pour les séries super-daily (mensuelles, annuelles)
...
// Met à jour computed.integratedDays avec une liste de dates intégrées.
// Utilise $set pour remplacer complètement le tableau (garantit l'idempotence).
// Accepte un tableau vide pour forcer la mise à jour à [].
export async function updateSeriesIntegratedDays(seriesIds, dates) {
  ...
  // Si dates est vide, on doit quand même mettre à jour pour garantir l'idempotence (set à [])
  if (dates.length === 0) {
    const {matchedCount, modifiedCount} = await mongo.db.collection('series').updateMany(
      {_id: {$in: seriesIds}},
      {
  ...
  const {matchedCount, modifiedCount} = await mongo.db.collection('series').updateMany(
    ...
    {
      $set: {
        'computed.integratedDays': uniqueDates,
        'computed.updatedAt': new Date()
      }
    }
  )
...
/**
 * Réinitialise les jours intégrés de toutes les séries
 * @returns {Promise<{matched: number, modified: number}>} Nombre de séries affectées
 */
export async function resetAllIntegratedDays() {
  const {matchedCount, modifiedCount} = await mongo.db.collection('series').updateMany(
    {},
    {
      $set: {
