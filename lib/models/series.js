...
export async function getSeriesByDossier(demarcheNumber, dossierNumber) {
  return mongo.db.collection('series').find({demarcheNumber, dossierNumber}).toArray()
}
...
// Retour: métadonnées de séries sans champs sensibles (attachmentId, demarcheNumber, dossierNumber, extras, computed.integratedDays).
