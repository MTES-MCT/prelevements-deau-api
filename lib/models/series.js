...
export function buildAttachmentQuery({attachmentId, territoire, startDate, endDate}) {
...
export function buildPointPreleveurQuery({territoire, pointIds, preleveurId, parameter, startDate, endDate, onlyIntegratedDays}) {
...
export async function listSeries({
  ...
  if (attachmentId) {
    const raw = await col.find(buildAttachmentQuery({attachmentId, territoire, startDate, endDate})).project(projection).toArray()
    return overlapFilter(raw)
  }

  // Cas 2: recherche par pointIds / preleveurId (au moins un des deux doit être fourni)
  const baseQuery = buildPointPreleveurQuery({territoire, pointIds, preleveurId, parameter, startDate, endDate, onlyIntegratedDays})
