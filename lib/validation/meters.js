...
export const meterReplaySchema = Joi.object({
  ...
  if (new Date(value.to) <= new Date(value.from) || new Date(value.to) - new Date(value.from) > 32 * 86_400_000) {
    return helpers.message('La fenêtre de rejeu doit être positive et ne pas dépasser 32 jours.')
  }
