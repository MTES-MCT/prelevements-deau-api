...
const importedUuid = '1b50db42-ca4b-5dd2-afc0-491ab70a704f'
...
test('validateDocumentCreation accepte plusieurs exploitations uniques', t => {
  const value = validateDocumentCreation({
    nature: NATURES_VALIDES[0],
    signatureDate: '2026-06-30',
    declarantPointPrelevementIds: [uuid, secondUuid]
  })

  t.deepEqual(value.declarantPointPrelevementIds, [uuid, secondUuid])
})

test('validateDocumentCreation normalise une exploitation multipart unique', t => {
  const value = validateDocumentCreation({
    nature: NATURES_VALIDES[0],
    signatureDate: '2026-06-30',
    declarantPointPrelevementIds: uuid
  })

  t.deepEqual(value.declarantPointPrelevementIds, [uuid])
})

test('les documents peuvent être créés ou rattachés aux exploitations v4 et v5', t => {
  for (const association of [
    {declarantPointPrelevementId: importedUuid},
    {declarantPointPrelevementIds: [uuid, importedUuid]}
  ]) {
    const created = validateDocumentCreation({nature: NATURES_VALIDES[0], signatureDate: '2026-06-30', ...association})
    const changed = validateDocumentChanges(association)
    for (const [key, value] of Object.entries(association)) {
      t.deepEqual(created[key], value)
      t.deepEqual(changed[key], value)
    }
  }

  t.deepEqual(validateDocumentChanges({declarantPointPrelevementIds: importedUuid}), {declarantPointPrelevementIds: [importedUuid]})
})

test('les rattachements de documents refusent les UUID invalides, hors v4/v5 et dupliqués', t => {
  for (const id of ['not-a-uuid', importedUuid.replace('-5dd2-', '-1dd2-'), importedUuid.replace('-5dd2-', '-3dd2-'), importedUuid.replace('-5dd2-', '-7dd2-')]) {
    t.throws(() => validateDocumentChanges({declarantPointPrelevementId: id}), {name: 'ValidationError'})
    t.throws(() => validateDocumentChanges({declarantPointPrelevementIds: [id]}), {name: 'ValidationError'})
  }

  t.throws(() => validateDocumentChanges({declarantPointPrelevementIds: [importedUuid, importedUuid]}), {name: 'ValidationError'})
})

test('validateDocumentCreation refuse de mélanger les interfaces singulière et plurielle', t => {
  const error = t.throws(() => validateDocumentCreation({
    nature: NATURES_VALIDES[0],
    signatureDate: '2026-06-30',
    declarantPointPrelevementId: uuid,
    declarantPointPrelevementIds: [secondUuid]
  }), {name: 'ValidationError'})

  t.true(error.details.some(detail => detail.type === 'object.oxor'))
})
