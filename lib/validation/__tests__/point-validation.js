...
test('validateCreation exige nom, type de milieu et géométrie', t => {
  const error = t.throws(() => validateCreation({}), {name: 'ValidationError'})
  t.deepEqual(error.details.map(detail => detail.path).sort(), ['coordinates', 'name', 'waterBodyType'])
})
