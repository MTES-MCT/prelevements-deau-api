...
const IMPORTED_ID = '1b50db42-ca4b-5dd2-afc0-491ab70a704f'
...
test('validateRegleCreation impose une fréquence pour un volume', t => {
  const payload = validRulePayload()
  delete payload.frequency

  const error = assertValidationError(t, () => validateRegleCreation(payload))

  t.true(error.details.some(detail => detail.message === 'La fréquence est obligatoire pour le paramètre "volume".'))
})
...
test('validateRegleCreation n’impose pas de fréquence pour un débit', t => {
  ...
  t.is(value.parameter, 'débit')
  t.false(Object.hasOwn(value, 'frequency'))
})

test('validateRegleCreation rejette les UUID malformés', t => {
  const payload = {
    ...validRulePayload(),
...
test('les règles acceptent des exploitations v4 et v5 en création et édition', t => {
  const exploitationIds = [EXPLOITATION_ID, IMPORTED_ID]
  t.deepEqual(validateRegleCreation({...validRulePayload(), exploitationIds}).exploitationIds, exploitationIds)
  t.deepEqual(validateRegleChanges({exploitationIds}), {exploitationIds})
})

test('les règles refusent les exploitations invalides ou hors UUID v4/v5', t => {
  for (const id of ['not-a-uuid', IMPORTED_ID.replace('-5dd2-', '-1dd2-'), IMPORTED_ID.replace('-5dd2-', '-3dd2-'), IMPORTED_ID.replace('-5dd2-', '-7dd2-')]) {
    assertValidationError(t, () => validateRegleChanges({exploitationIds: [id]}))
  }
})

test('les identifiants de documents associés aux règles restent UUID v4', t => {
  t.deepEqual(validateRegleChanges({documentId: DOCUMENT_ID}), {documentId: DOCUMENT_ID})
  const error = assertValidationError(t, () => validateRegleChanges({documentId: IMPORTED_ID}))
  t.true(error.details.some(detail => detail.path === 'documentId'))
})
