...
test('regleSchemaCreation / frequence obligatoire pour volume prélevé', t => {
  const input = regleValideBase()
  delete input.frequence
  const error = t.throws(() => validateRegleCreation(input), {instanceOf: ValidationError})
  t.true(error.details.some(d => d.message === 'La fréquence est obligatoire pour le paramètre "volume prélevé".'))
})

test('regleSchemaCreation / frequence invalide', t => {
  const input = {...regleValideBase(), frequence: '1 week'}
  const error = t.throws(() => validateRegleCreation(input), {instanceOf: ValidationError})
  t.true(error.details.some(d => d.message === 'La fréquence est invalide.'))
})

test('regleSchemaCreation / frequence non requise pour débit prélevé', t => {
  const input = {
