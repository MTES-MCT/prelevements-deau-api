...
const pointSchemaCreation = Joi.object().keys({
  ...
  code_dblisa: Joi.string().allow(null).messages({
    'string.base': 'Le code DBLISA doit être une chaine de caractères.',
    'string.empty': 'Le code DBLISA ne peux pas être vide.'
  }),
  ...
  bss: Joi.object().keys({
    ...
    lien: Joi.uri().allow(null).messages({
