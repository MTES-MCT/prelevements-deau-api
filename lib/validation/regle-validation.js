...
const LEGACY_PARAMETERS = new Map([
...
function normalizeRegleParameter(regle) {
  if (!regle || !Object.hasOwn(regle, 'parameter')) {
    return regle
  }

  return {
    ...regle,
    parameter: LEGACY_PARAMETERS.get(regle.parameter) ?? regle.parameter
  }
}
...
const REGLE_FIELDS = {
  ...
  exploitationIds: Joi.array().items(resourceIdSchema)
}
...
export const regleSchemaCreation = Joi.object().keys({
  ...
  frequency: Joi.when('parameter', {
    is: 'volume',
    then: REGLE_FIELDS.frequency.required().messages({
      'any.required': 'La fréquence est obligatoire pour le paramètre "volume".'
    }),
...
export function validateRegleCreation(regle) {
  return validatePayload(normalizeRegleParameter(regle), regleSchemaCreation)
}

export function validateRegleChanges(changes) {
  return validatePayload(normalizeRegleParameter(changes), regleSchemaEdition)
}
