...
export const TYPES_MILIEU_VALIDES = [
  'Eau de surface',
  'Eau souterraine',
  'Eau de transition'
]

export const PRECISIONS_GEOM_VALIDES = [
  'Repérage carte',
...
function validateTypeMilieu(typeMilieu, helpers) {
  if (!TYPES_MILIEU_VALIDES.includes(typeMilieu)) {
...
function validatePrecisionGeom(precision_geom, helpers) {
  if (!PRECISIONS_GEOM_VALIDES.includes(precision_geom)) {
...
const POINT_FIELDS = {
  ...
  remarque_interne: Joi.string().trim().min(3).max(500),
...
const pointSchemaCreation = Joi.object().keys({
  ...
  remarque_interne: addStringMessages(POINT_FIELDS.remarque_interne.allow(null), 'remarque_interne')
}).messages({
