...
import {COLLECTION_MODES} from '../services/manual-collection.js'

export const WATER_BODY_TYPES = ['SUPERFICIELLE', 'SOUTERRAIN', 'TRANSITION']

export const WATER_BODY_FIELDS = [
...
const POINT_FIELDS = {
  ...
  codeINSEE: CODE_TEXT,
  codeROE: CODE_TEXT,
  waterBodyType: Joi.any().custom((value, helpers) => {
    if (value === null) {
      return helpers.error('any.required')
    }

    if (typeof value !== 'string' || !WATER_BODY_TYPES.includes(value)) {
  ...
  reservoirNominalVolume: Joi.number().positive(),
  waterBodyIdentifier: CODE_TEXT,
...
const sharedCreationFields = {
  collectionMode: Joi.string().valid(...COLLECTION_MODES).allow(null),
  ...
  reservoirNominalVolume: POINT_FIELDS.reservoirNominalVolume.allow(null).messages({
    'number.base': 'Le volume nominal de la retenue doit être un nombre.',
    'number.positive': 'Le volume nominal de la retenue doit être strictement positif.'
  }),
  waterBodyIdentifier: nullableTextField('waterBodyIdentifier', 'Identifiant du plan d’eau'),
...
const pointSchemaEdition = Joi.object().keys({
  ...
  waterBodyType: POINT_FIELDS.waterBodyType,
...
export function normalizeWaterBodyConnections(value, currentNature = null) {
  ...
  const updatesConnections = Object.hasOwn(normalized, 'nature')
    || WATER_BODY_FIELDS.some(field => Object.hasOwn(normalized, field))
  ...
  if (effectiveNature !== 'PLAN_EAU') {
    for (const field of WATER_BODY_FIELDS) {
