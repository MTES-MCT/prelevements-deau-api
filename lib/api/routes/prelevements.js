...
prelevementRoutes.get('/exploitation/:id', w(async (req, res) => {
  const exploitation = await getExploitation(req.params.id)

  res.send(exploitation)
}))

prelevementRoutes.get('/beneficiaire/:id', w(async (req, res) => {
  const beneficiaire = await getBeneficiaire(req.params.id)

  res.send(beneficiaire)
}))

prelevementRoutes.get('/regle/:id', w(async (req, res) => {
  const regle = await getRegle(req.params.id)

  res.send(regle)
}))

prelevementRoutes.get('/document/:id', w(async (req, res) => {
