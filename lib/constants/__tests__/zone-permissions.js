...
test('le catalogue contient 67 droits uniques et conserve 25 droits historiques de lecture par défaut', t => {
  t.is(ZONE_PERMISSION_CODES.length, 67)
  t.is(new Set(ZONE_PERMISSION_CODES).size, 67)
...
test('les migrations aboutissent exactement au catalogue et aux droits historiques attendus', async t => {
  ...
  t.deepEqual(migrationCodes, ZONE_PERMISSION_CODES.filter(code => !code.startsWith('campaign.')))
  t.true(ZONE_PERMISSION_CODES.filter(code => code.startsWith('campaign.')).every(code => !LEGACY_INSTRUCTOR_ZONE_PERMISSIONS.includes(code)))
