...
function validateAndExtractParameters(dataSheet, dataRows, {startDate, endDate, errorCollector}) {
  ...
  for (const paramIndex of [2, 3, 4, 5, 6, 7, 8]) {
    ...
    parameters.push({paramIndex, ...fields})
  }
...
function validateAndExtractParamFields(dataSheet, colIndex, {errorCollector}) {
  ...
  const definitions = [
    {fieldName: 'nom_parametre', type: 'string', row: 1, required: true},
    {fieldName: 'type', type: 'string', row: 2, required: true},
    {fieldName: 'frequence', type: 'string', row: 3, required: true},
    {fieldName: 'unite', type: 'string', row: 4, required: true},
  ...
  for (const {fieldName, type, row, required} of definitions) {
    ...
    } else if (fieldName === 'date_debut' && value !== undefined) {
