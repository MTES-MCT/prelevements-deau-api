...
import {handleDossier, handlePoint, handlePreleveur, handleExploitation, handleDocument} from './resolvers.js'
...
async function createRoutes() {
  ...
  app.param('storageHash', attachmentParam)
  app.get('/dossiers/:dossierId/files/:storageHash', w(getAttachmentDetail))
  app.get('/dossiers/:dossierId/files/:storageHash/integrations', w(getAttachmentIntegrations))
  app.get('/dossiers/:dossierId/files/:storageHash/series', w(listSeriesForAttachment))
  app.get('/dossiers/:dossierId/files/:storageHash/download', w(downloadAttachment))
  ...
  // Recherche séries métadonnées (preleveurId, pointId, from, to)
  app.get('/series', w(async (req, res, next) => {
    if (!req.isAdmin) {
      return next(createHttpError(403, 'Vous devez être authentifié'))
    }

    return listSeriesMetadataSearch(req, res)
  }))
  ...
  // Série: valeurs (dates optionnelles)
  app.get('/series/:seriesId/values', w(getSeriesValuesHandler))
  // Série: métadonnées seules
  app.get('/series/:seriesId', w(getSeriesMetadataHandler))
