...
import {eachDayOfInterval, subDays, differenceInDays} from 'date-fns'
...
async function createRoutes() {
  ...
  app.route('/preleveurs/:preleveurId/documents')
    ...
    .post(w(checkPermissionOnPreleveur), w(async (req, res) => {
      const document = await createDocument(req.body, req.preleveur._id, req.territoire)
  ...
  app.route('/preleveurs/:preleveurId/documents/upload')
    .post(w(checkPermissionOnPreleveur), upload.single('document'), w(async (req, res) => {
      const {file, preleveur} = req
      const documentInfos = await uploadDocument(file, preleveur._id)
