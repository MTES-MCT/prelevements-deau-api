...
import {createUpload} from './util/upload.js'
import {
  listCollectionCampaignsHandler, getCollectionCampaignSummaryHandler, getCollectionCampaignHandler,
  listCollectionCandidatesHandler, createCollectionCampaignHandler, updateCollectionCampaignHandler,
  transitionCollectionCampaignHandler, listCollectionResponsesHandler, getCollectionResponseHandler,
  saveCollectionResponseDraftHandler, submitCollectionResponseHandler, approveCollectionMeterHandler,
  getCollectionResultsHandler, exportCollectionResultsHandler, getCollectionMeterReviewHandler
} from './handlers/collection-campaigns.js'
...
import {getPublicStatsHandler} from './handlers/public-stats.js'
import {
...
import {
  ...
  deactivateAdminAgentHandler,
  getAdminAgentHandler,
  listAdminAgentsHandler,
  replaceAdminAgentEmailHandler,
  restoreAdminAgentHandler,
  sendAdminAgentAccountCreationNotificationHandler,
  updateAdminAgentProfileHandler
} from './handlers/admin-agents.js'
...
import {postUserActivityHandler} from './handlers/user-activity.js'
import {
...
import {
  getMeterStreamContextHandler,
  ingestMeterBatchHandler,
  getExploitationMeterAllocationsHandler,
  getExploitationMeterReadingsHandler,
  getMeterAllocationSettingsHandler,
  updateMeterAllocationSettingsHandler,
  searchMeterAllocationTargetsHandler
} from './handlers/meters.js'
import {
...
export function createRoutes({
  ...
  publicStatsHandler = getPublicStatsHandler,
  userActivityHandler = postUserActivityHandler
} = {}) {
  ...
  // Agrégats publics uniquement : ne pas appliquer les contrôles de session,
  // y compris lorsqu'un visiteur présente un ancien token expiré.
  // L'alias doit être déclaré ici : le montage racine précède le montage /api.
  app.get(['/stats/public', '/api/stats/public'], rateLimit({
    windowMs: 60_000,
    limit: 60,
    standardHeaders: true,
    legacyHeaders: false,
    message: {code: 429, message: 'Trop de demandes de statistiques. Réessayez dans une minute.'}
  }), publicStatsHandler)
  ...
  app.use('/campaigns', ensureHumanSession, ensureRole('ADMIN', 'DECLARANT'))
  app.get('/campaigns/summary', getCollectionCampaignSummaryHandler)
  app.get('/campaigns/candidates', ensureRole('ADMIN'), listCollectionCandidatesHandler)
  app.route('/campaigns').get(listCollectionCampaignsHandler).post(ensureRole('ADMIN'), ensureNotImpersonating, createCollectionCampaignHandler)
  app.route('/campaigns/:campaignId').get(getCollectionCampaignHandler)
    .patch(ensureRole('ADMIN'), ensureNotImpersonating, updateCollectionCampaignHandler)
    .delete(ensureRole('ADMIN'), ensureNotImpersonating, transitionCollectionCampaignHandler('delete'))
  app.post('/campaigns/:campaignId/open', ensureRole('ADMIN'), ensureNotImpersonating, transitionCollectionCampaignHandler('open'))
  app.post('/campaigns/:campaignId/close', ensureRole('ADMIN'), ensureNotImpersonating, transitionCollectionCampaignHandler('close'))
  app.post('/campaigns/:campaignId/archive', ensureRole('ADMIN'), ensureNotImpersonating, transitionCollectionCampaignHandler('archive'))
  app.get('/campaigns/:campaignId/responses', listCollectionResponsesHandler)
  app.get('/campaigns/:campaignId/results', getCollectionResultsHandler)
  app.get('/campaigns/:campaignId/export', exportCollectionResultsHandler)
  // Like quick declarations, a human impersonation keeps the preleveur's rights.
  // Ownership is checked by the service; the administrator remains the audit actor.
  app.route('/campaigns/:campaignId/responses/:responseId').get(getCollectionResponseHandler)
    .put(ensureRole('DECLARANT'), saveCollectionResponseDraftHandler)
  app.post('/campaigns/:campaignId/responses/:responseId/submit', ensureRole('DECLARANT'), submitCollectionResponseHandler)
  app.post('/campaigns/:campaignId/meters/:compteurId/approve', ensureRole('ADMIN'), ensureNotImpersonating, approveCollectionMeterHandler)
  app.get('/campaigns/:campaignId/meters/:compteurId/review', ensureRole('ADMIN'), getCollectionMeterReviewHandler)
  ...
  app.get('/service-accounts/meter-streams', ensureServiceAccountAuthenticated, getMeterStreamContextHandler)
  app.post('/service-accounts/meter-readings/ingestions', ensureServiceAccountAuthenticated, ingestMeterBatchHandler)
  ...
  app.post('/admin/meter-readings/ingestions', ensureRole('ADMIN'), ensureNotImpersonating, ingestMeterBatchHandler)

  app.route('/admin/agents')
    .get(ensureRole('ADMIN'), ensureNotImpersonating, listAdminAgentsHandler)
    .post(ensureRole('ADMIN'), ensureNotImpersonating, createAdminAgentHandler)
  ...
  app.patch(
    '/admin/agents/:agentId/profile',
    ensureRole('ADMIN'),
    ensureNotImpersonating,
    updateAdminAgentProfileHandler
  )
  app.put(
    '/admin/agents/:agentId/email',
    ensureRole('ADMIN'),
    ensureNotImpersonating,
    replaceAdminAgentEmailHandler
  )
  ...
  app.post(
    '/admin/agents/:agentId/restore',
    ensureRole('ADMIN'),
    ensureNotImpersonating,
    restoreAdminAgentHandler
  )
  app.post(
    '/admin/agents/:agentId/notifications/account-creation',
    ensureRole('ADMIN'),
    ensureNotImpersonating,
    sendAdminAgentAccountCreationNotificationHandler
  )
  ...
  app.post('/users/me/activity', ensureHumanSession, userActivityHandler)
  app.patch(
    '/users/me/profile',
    ensureAuthenticated,
  ...
  app.get('/exploitations/:exploitationId/meter-allocations', authorizeExploitation('read', 'exploitation.detail.read'), getExploitationMeterAllocationsHandler)
  app.get('/exploitations/:exploitationId/meters/:meterId/readings', ensureRole('ADMIN'), getExploitationMeterReadingsHandler)
  app.get('/meters/allocation-targets', ensureRole('ADMIN'), searchMeterAllocationTargetsHandler)
  app.get('/exploitations/:exploitationId/meters/:meterId/allocation-settings', ensureRole('ADMIN'), getMeterAllocationSettingsHandler)
  app.put('/exploitations/:exploitationId/meters/:meterId/allocation-settings', ensureRole('ADMIN'), ensureNotImpersonating, updateMeterAllocationSettingsHandler)
