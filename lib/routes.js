...
import {getAttachmentObjectKey} from '@fabrique/demarches-simplifiees'
...
async function createRoutes() {
  ...
  app.get('/dossiers/:dossierId', w(async (req, res) => {
    const attachments = await Dossier.getAttachments(req.dossier.demarcheNumber, req.dossier.number)
    res.send({
      ...req.dossier,
      files: attachments
    })
  ...
  app.get('/dossiers/:dossierId/files/:storageKey', w(async (req, res) => {
    ...
    const objectKey = getAttachmentObjectKey(req.dossier.demarcheNumber, attachment.dossierNumber, attachment.storageKey)
