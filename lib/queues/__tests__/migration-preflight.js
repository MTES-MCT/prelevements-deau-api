...
async function legacyTarget(queueName = 'sync-monitoring-stations-daily') {
...
test('la migration préserve nom, données et tentatives puis retire seulement la définition ciblée', async t => {
  ...
  t.deepEqual(events, [
    {action: 'create', id: 'sync-monitoring-stations-daily', repeat: {every: 1000}, template: {
...
test('une cible ambiguë est refusée avant toute écriture de conversion', async t => {
  const target = await legacyTarget()
  const scenarios = [
    {...target, schedulerId: 'migrated-sync-monitoring-stations-daily'},
