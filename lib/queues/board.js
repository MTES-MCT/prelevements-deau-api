...
export function createBullBoardAuthMiddleware(password) {
  return (req, res, next) => {
 ...
 * @param {object} options - Files injectées pour les tests.
...
export function createBullBoardRouter(basePath, password, options = {}) {
  ...
  router.use(createBullBoardAuthMiddleware(password))
