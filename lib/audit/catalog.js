...
/* eslint-disable max-params -- Les entrées restent volontairement lisibles sur une ligne. */
...
function campaignAction(method, path, type, label, options = {}) {
  return action(method, path, type, 'CAMPAIGN', label, {
    // Record the operation and resource ids, never questionnaire contents,
    // free-text names, contact details or raw query filters.
    safeBodyFields: ['name', 'type', 'opensOn', 'closesOn', 'collecteurUserId', 'exploitationIds', 'sourceId',
      'data', 'revision', 'allocations', 'confirmHistorical', 'expectedHash', 'canonicalResponseId'],
    safeBodyValueFields: [],
    target: {type: 'COLLECTION_CAMPAIGN', param: 'campaignId', captureLabel: false},
    ...options
  })
}
...
export const AUDIT_ACTIONS = Object.freeze([
  ...
  action('POST', '/auth/password', 'AUTH.PASSWORD_LOGIN_VERIFIED', 'AUTHENTICATION', 'Connexion avec un mot de passe'),
  action('POST', '/auth/password/activate', 'AUTH.PASSWORD_ACTIVATED', 'AUTHENTICATION', 'Activation d’un mot de passe'),
  action('POST', '/auth/password/change', 'AUTH.PASSWORD_CHANGED', 'AUTHENTICATION', 'Modification du mot de passe'),
  action('POST', '/auth/email-verifications/confirm', 'ACCOUNT.EMAIL_VERIFICATION_CONFIRMED', 'ACCOUNT', 'Validation d’une nouvelle adresse email'),
  ...
  action('GET', '/admin/password-accesses', 'AUTH.PASSWORD_ACCESSES_VIEWED', 'AUTHENTICATION', 'Consultation des accès par mot de passe'),
  action('POST', '/admin/password-accesses', 'AUTH.PASSWORD_ACTIVATION_ISSUED', 'AUTHENTICATION', 'Création d’un lien d’activation de mot de passe', {
    subject: {body: 'userId'},
    target: {type: 'USER', body: 'userId'}
  }),
  action('DELETE', '/admin/password-accesses/:userId', 'AUTH.PASSWORD_ACCESS_REVOKED', 'AUTHENTICATION', 'Révocation d’un accès par mot de passe', {
    subject: {param: 'userId'},
    target: {type: 'USER', param: 'userId'}
  }),

  action('GET', '/admin/agents', 'ACCOUNT.AGENTS_VIEWED', 'ACCOUNT', 'Consultation de la liste des agents'),
  action('POST', '/admin/agents', 'ACCOUNT.AGENT_CREATED', 'ACCOUNT', 'Création d’un agent'),
  action('GET', '/admin/agents/:agentId', 'ACCOUNT.AGENT_VIEWED', 'ACCOUNT', 'Consultation d’un agent', {
    subject: {param: 'agentId'},
    target: {type: 'USER', param: 'agentId'}
  }),
  action('PATCH', '/admin/agents/:agentId/profile', 'ACCOUNT.AGENT_PROFILE_UPDATED', 'ACCOUNT', 'Modification du profil d’un agent', {
    subject: {param: 'agentId'},
    target: {type: 'USER', param: 'agentId'}
  }),
  action('PUT', '/admin/agents/:agentId/email', 'ACCOUNT.AGENT_EMAIL_CHANGED', 'ACCOUNT', 'Modification de l’adresse de connexion d’un agent', {
    subject: {param: 'agentId'},
    target: {type: 'USER', param: 'agentId'}
  }),
  action('DELETE', '/admin/agents/:agentId', 'ACCOUNT.AGENT_DISABLED', 'ACCOUNT', 'Désactivation d’un agent', {
    subject: {param: 'agentId'},
    target: {type: 'USER', param: 'agentId'}
  }),
  action('POST', '/admin/agents/:agentId/restore', 'ACCOUNT.AGENT_RESTORED', 'ACCOUNT', 'Réactivation d’un agent', {
    subject: {param: 'agentId'},
    target: {type: 'USER', param: 'agentId'}
  }),
  action('POST', '/admin/agents/:agentId/notifications/account-creation', 'ACCOUNT.AGENT_ACCOUNT_NOTIFICATION_SENT', 'ACCOUNT', 'Envoi d’une invitation à un agent', {
    subject: {param: 'agentId'},
    target: {type: 'USER', param: 'agentId'}
  }),
  ...
  campaignAction('POST', '/campaigns', 'CAMPAIGN.CREATED', 'Création d’une campagne', {target: {type: 'COLLECTION_CAMPAIGN', captureLabel: false}}),
  campaignAction('PATCH', '/campaigns/:campaignId', 'CAMPAIGN.UPDATED', 'Modification d’une campagne'),
  campaignAction('DELETE', '/campaigns/:campaignId', 'CAMPAIGN.DELETED', 'Suppression d’une campagne brouillon'),
  campaignAction('POST', '/campaigns/:campaignId/open', 'CAMPAIGN.OPENED', 'Lancement d’une campagne'),
  campaignAction('POST', '/campaigns/:campaignId/close', 'CAMPAIGN.CLOSED', 'Clôture d’une campagne'),
  campaignAction('POST', '/campaigns/:campaignId/archive', 'CAMPAIGN.ARCHIVED', 'Archivage d’une campagne'),
  campaignAction('PUT', '/campaigns/:campaignId/responses/:responseId', 'CAMPAIGN.RESPONSE_DRAFT_SAVED', 'Enregistrement d’un brouillon de réponse', {target: {type: 'COLLECTION_RESPONSE', param: 'responseId'}}),
  campaignAction('POST', '/campaigns/:campaignId/responses/:responseId/submit', 'CAMPAIGN.RESPONSE_SUBMITTED', 'Soumission d’une réponse de campagne', {target: {type: 'COLLECTION_RESPONSE', param: 'responseId'}}),
  campaignAction('POST', '/campaigns/:campaignId/meters/:compteurId/approve', 'CAMPAIGN.METER_APPROVED', 'Validation de la publication d’un compteur de campagne', {target: {type: 'COMPTEUR', param: 'compteurId'}}),
  campaignAction('GET', '/campaigns/:campaignId/responses', 'CAMPAIGN.RESPONSES_VIEWED', 'Consultation des réponses d’une campagne', {requireUuidParams: ['campaignId']}),
  campaignAction('GET', '/campaigns/:campaignId/responses/:responseId', 'CAMPAIGN.RESPONSE_VIEWED', 'Consultation d’une réponse de campagne', {target: {type: 'COLLECTION_RESPONSE', param: 'responseId'}, requireUuidParams: ['campaignId', 'responseId']}),
  campaignAction('GET', '/campaigns/:campaignId/results', 'CAMPAIGN.RESULTS_VIEWED', 'Consultation des résultats d’une campagne', {requireUuidParams: ['campaignId']}),
  campaignAction('GET', '/campaigns/:campaignId/export', 'CAMPAIGN.RESULTS_EXPORTED', 'Export des résultats d’une campagne', {requireUuidParams: ['campaignId']}),
  campaignAction('GET', '/campaigns/:campaignId/meters/:compteurId/review', 'CAMPAIGN.METER_REVIEW_VIEWED', 'Consultation de la répartition d’un compteur de campagne', {target: {type: 'COMPTEUR', param: 'compteurId'}, requireUuidParams: ['campaignId', 'compteurId']}),
  ...
  action('POST', '/service-accounts/meter-readings/ingestions', 'INGESTION.METER_READINGS_INGESTED', 'DATA_INGESTION', 'Ingestion d’observations physiques normalisées'),
  action('POST', '/admin/meter-readings/ingestions', 'INGESTION.METER_READINGS_ADMIN_INGESTED', 'DATA_INGESTION', 'Ingestion administrative d’observations physiques normalisées'),
  action('GET', '/service-accounts/meter-streams', 'INGESTION.METER_STREAMS_VIEWED', 'DATA_INGESTION', 'Consultation des flux physiques autorisés'),
  action('GET', '/exploitations/:exploitationId/meters/:meterId/allocation-settings', 'EXPLOITATION.METER_ALLOCATION_VIEWED', 'EXPLOITATION', 'Consultation de la répartition complète d’un compteur', {
    target: {type: 'EXPLOITATION', param: 'exploitationId'}, requireUuidParams: ['exploitationId', 'meterId']
  }),
  action('PUT', '/exploitations/:exploitationId/meters/:meterId/allocation-settings', 'EXPLOITATION.METER_ALLOCATION_UPDATED', 'EXPLOITATION', 'Modification datée de la répartition d’un compteur', {
    target: {type: 'EXPLOITATION', param: 'exploitationId'}, requireUuidParams: ['exploitationId', 'meterId']
  }),
  action('GET', '/exploitations/:exploitationId/meters/:meterId/readings', 'INGESTION.METER_READINGS_VIEWED', 'DATA_INGESTION', 'Consultation des index physiques globaux', {
    target: {type: 'EXPLOITATION', param: 'exploitationId'},
    requireUuidParams: ['exploitationId', 'meterId']
  }),
  ...
  action('PATCH', '/collecteurs/:collecteurId/exploitations', 'COLLECTEUR.EXPLOITATIONS_UPDATED', 'EXPLOITATION', 'Modification en masse des exploitations d’un collecteur', {
    subject: {param: 'collecteurId'},
    target: {type: 'DECLARANT', param: 'collecteurId'}
  }),
  ...
  action('PUT', '/declarants/:declarantId/contact-emails', 'DECLARANT.CONTACT_EMAILS_UPDATED', 'DECLARANT', 'Modification des emails de contact d’un déclarant', {
    subject: {param: 'declarantId'},
    target: {type: 'DECLARANT', param: 'declarantId'}
  }),
  ...
  action('PATCH', '/users/me/profile', 'ACCOUNT.PROFILE_UPDATED', 'ACCOUNT', 'Modification des informations personnelles'),
  action('POST', '/users/me/email-change', 'ACCOUNT.PRIMARY_EMAIL_CHANGE_REQUESTED', 'ACCOUNT', 'Demande de changement de l’adresse email de connexion'),
  action('POST', '/users/me/email-aliases', 'ACCOUNT.EMAIL_ALIAS_ADDITION_REQUESTED', 'ACCOUNT', 'Demande d’ajout d’une adresse email de connexion'),
  action('DELETE', '/users/me/email-aliases/:emailAliasId', 'ACCOUNT.EMAIL_ALIAS_REMOVED', 'ACCOUNT', 'Suppression d’une adresse email de connexion', {target: {type: 'EMAIL_ALIAS', param: 'emailAliasId'}}),
  action('POST', '/users/me/email-verifications/:verificationId/resend', 'ACCOUNT.EMAIL_VERIFICATION_RESENT', 'ACCOUNT', 'Nouvel envoi d’un email de validation', {target: {type: 'EMAIL_VERIFICATION', param: 'verificationId'}}),
  action('DELETE', '/users/me/email-verifications/:verificationId', 'ACCOUNT.EMAIL_VERIFICATION_CANCELLED', 'ACCOUNT', 'Annulation d’une validation d’adresse email', {target: {type: 'EMAIL_VERIFICATION', param: 'verificationId'}}),
  ...
  action('POST', '/zones/:zoneId/instructeurs', 'ZONE.AGENT_ADDED', 'ZONE', 'Ajout d’un agent à une zone', {target: {type: 'ZONE', param: 'zoneId'}, subject: {body: 'instructorUserId'}}),
  ...
  action('GET', '/admin/audit-events/:eventId', 'AUDIT.EVENT_VIEWED', 'AUDIT', 'Consultation du détail d’un événement d’audit', {
    target: {type: 'AUDIT_EVENT', param: 'eventId'},
    requireUuidParams: ['eventId']
  }),
  action('GET', '/audit-history/:resourceType/:resourceId', 'AUDIT.RESOURCE_HISTORY_VIEWED', 'AUDIT', 'Consultation de l’historique d’une ressource', {
    target: {type: 'AUDIT_RESOURCE', param: 'resourceId'},
    requireUuidParams: ['resourceId'],
    safeParamFields: ['resourceType']
  })
])
