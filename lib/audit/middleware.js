...
import {
  buildAuditMutations,
  captureAuditResponseBody,
  captureFinalAuditMutation,
...
function buildSafeMetadata(request, auditAction, params, networkContext) {
  const body = auditAction.safeBodyFields
    ? Object.fromEntries(Object.entries(request.body ?? {}).filter(([key]) => auditAction.safeBodyFields.includes(key)))
    : request.body
  ...
  const requestedValues = pickSafeValues(body, auditAction.safeBodyValueFields ?? SAFE_BODY_VALUE_FIELDS)
...
function captureResponseTarget(request, response, auditAction) {
  const capture = body => {
    captureAuditResponseBody(request, body)
    ...
    if (target) {
      ...
      request.auditContext.target = {
        ...
        label: auditAction.target.captureLabel === false ? null : target.name || target.label || null
      }
...
export function createAuditMiddleware({client = prisma} = {}) {
  return async (request, response, next) => {
    ...
    try {
      ...
      request.auditContext.metadata.mutationBeforeSnapshotUnavailable = true
      Sentry.captureException(error, {
        tags: {component: 'audit', phase: 'mutation-before'}
      })
    }
    ...
    const finalize = async incomplete => {
      ...
      try {
        ...
        const outcome = getAuditOutcome(response.statusCode, incomplete)
        if (outcome === 'SUCCESS') {
          try {
            await captureFinalAuditMutation(request, auditAction, client)
          } catch (error) {
            request.auditContext.metadata.mutationAfterSnapshotUnavailable = true
            Sentry.captureException(error, {
              tags: {component: 'audit', phase: 'mutation-after'}
            })
          }
        }

        const mutations = outcome === 'SUCCESS'
          ? buildAuditMutations(request, auditAction)
          : []
        const persist = async transactionClient => {
          await transactionClient.auditEvent.update({
          ...
          await Promise.all(mutations.map(mutation =>
            transactionClient.auditMutation.create({
              data: {
