...
import {CAMPAIGN_AUDIT_LABELS, loadCampaignAuditSnapshot} from './campaigns.js'
...
const ENTITY_FIELDS = Object.freeze({
  POINT: [
    ...
    'isWaterBodyConnectedToGroundwater', 'reservoirNominalVolume', 'waterBodyIdentifier',
    'otherNames', 'names', 'identifiers',
  ...
  COLLECTION_CAMPAIGN: [
    'type', 'status', 'opensOn', 'closesOn', 'closedAt', 'collecteurUserId', 'createdByUserId',
    'exploitationCount', 'populationHash'
  ],
  COLLECTION_RESPONSE: [
    'campaignId', 'exploitationId', 'preleveurUserId', 'revision',
    'firstSubmittedAt', 'lastSubmittedAt', 'declarationId', 'publicationStatus'
  ],
  COLLECTION_METER: [
    'campaignId', 'compteurId', 'streamIds', 'enabled', 'streamStateHash',
    'allocationVersionCount', 'allocationStateHash', 'activePublicationCount', 'publicationIds',
    'responseCount', 'responseStateHash'
  ],
...
export const AUDIT_MUTATION_PROFILES = Object.freeze({
  ...
  'CAMPAIGN.CREATED': profile('CREATE', 'COLLECTION_CAMPAIGN'),
  'CAMPAIGN.UPDATED': profile('UPDATE', 'COLLECTION_CAMPAIGN', {idParam: 'campaignId'}),
  'CAMPAIGN.DELETED': profile('DELETE', 'COLLECTION_CAMPAIGN', {idParam: 'campaignId'}),
  'CAMPAIGN.OPENED': profile('UPDATE', 'COLLECTION_CAMPAIGN', {idParam: 'campaignId'}),
  'CAMPAIGN.CLOSED': profile('UPDATE', 'COLLECTION_CAMPAIGN', {idParam: 'campaignId'}),
  'CAMPAIGN.ARCHIVED': profile('UPDATE', 'COLLECTION_CAMPAIGN', {idParam: 'campaignId'}),
  'CAMPAIGN.RESPONSE_DRAFT_SAVED': profile('UPDATE', 'COLLECTION_RESPONSE', {idParam: 'responseId'}),
  'CAMPAIGN.RESPONSE_SUBMITTED': profile('UPDATE', 'COLLECTION_RESPONSE', {idParam: 'responseId'}),
  'CAMPAIGN.METER_APPROVED': profile('UPDATE', 'COLLECTION_METER', {idParam: 'compteurId'}),
...
function flattenEntity(entityType, value) {
  ...
  if (entityType === 'USER_PROFILE' || entityType === 'AGENT_ACCOUNT') {
    return {
      ...value.declarant,
      ...value.instructor,
      ...value.user,
...
async function loadMutationEntity(client, entityType, entityId, request) {
  ...
  if (Object.hasOwn(CAMPAIGN_AUDIT_LABELS, entityType)) {
    return loadCampaignAuditSnapshot(client, entityType, entityId, getRequestParams(request))
  }
...
function getEntityLabel(entityType, after, before, entityId) {
  if (Object.hasOwn(CAMPAIGN_AUDIT_LABELS, entityType)) return CAMPAIGN_AUDIT_LABELS[entityType]
...
function getRedactedChangedFields(request) {
  return Object.keys(request.body ?? {})
    .filter(field => REDACTED_FIELDS.has(field)
      || (request.auditAction?.type?.startsWith('CAMPAIGN.') && ['name', 'data', 'allocations'].includes(field)))
    .sort()
}
...
function getMutationScopes(request, entityType, entityId, entityLabel, after, before) {
  ...
  const declarantId = params.declarantId
    ...
    ?? value.declarant?.id
    ?? (entityType === 'COLLECTION_RESPONSE' ? value.preleveurUserId : null)
  ...
  const exploitationId = params.exploitationId
    ?? value.declarantPointPrelevementId
    ?? (entityType === 'COLLECTION_RESPONSE' ? value.exploitationId : null)
  ...
  scopes.push(
    ...
    scope('POINT', pointId),
    scope('COLLECTION_CAMPAIGN', params.campaignId ?? value.campaignId),
    scope('COMPTEUR', params.compteurId ?? value.compteurId)
  )
