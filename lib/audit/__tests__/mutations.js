...
test('buildAuditMutations historise les caractéristiques du plan d’eau et leur effacement', t => {
  const fields = {reservoirNominalVolume: 12000.75, waterBodyIdentifier: '00042'}
  const emptyFields = {reservoirNominalVolume: null, waterBodyIdentifier: null}

  for (const [before, after] of [[emptyFields, fields], [fields, emptyFields]]) {
    const [mutation] = buildAuditMutations(pointUpdateRequest({
      before: {id: POINT_ID, ...before},
      after: {id: POINT_ID, ...after}
    }), {type: 'POINT.UPDATED'})

    t.deepEqual(mutation.changedFields, ['reservoirNominalVolume', 'waterBodyIdentifier'])
    t.deepEqual(mutation.before, before)
    t.deepEqual(mutation.after, after)
  }
})
...
test('buildAuditMutations historise la correction du code comptage sans modifier l’identité', t => {
  const request = {
    auditAction: {params: {exploitationId: EXPLOITATION_ID}},
    auditContext: {
      mutationBefore: {id: EXPLOITATION_ID, countingCode: '001'},
      mutationAfter: {id: EXPLOITATION_ID, countingCode: '002'}
    },
    body: {countingCode: '002', expectedCountingCode: '001'},
    params: {}
  }
  const [mutation] = buildAuditMutations(request, {type: 'EXPLOITATION.UPDATED'})
  t.deepEqual(mutation.changedFields, ['countingCode'])
  t.deepEqual(mutation.before, {countingCode: '001'})
  t.deepEqual(mutation.after, {countingCode: '002'})
})
