...
const EXEMPT_MUTATIONS = new Set([
  ...
  // Un marqueur mensuel agrégé, pas un événement métier ou de sécurité :
  // ne pas ajouter de journal détaillé contenant l’IP et le contexte de navigation.
...
const SENSITIVE_READS = [
  ...
  '/service-accounts/meter-streams',
  '/exploitations/11111111-1111-4111-8111-111111111111/meters/22222222-2222-4222-8222-222222222222/readings',
  ...
  '/campaigns/11111111-1111-4111-8111-111111111111/responses',
  '/campaigns/11111111-1111-4111-8111-111111111111/responses/22222222-2222-4222-8222-222222222222',
  '/campaigns/11111111-1111-4111-8111-111111111111/results',
  '/campaigns/11111111-1111-4111-8111-111111111111/export',
  '/campaigns/11111111-1111-4111-8111-111111111111/meters/22222222-2222-4222-8222-222222222222/review'
]

function extractMutationRoutes(source) {
  const routes = new Set()
  const directRoutePattern = /app\.(post|put|patch|delete)\(\s*['`]([^'`]+)['`]/g
  const chainedRoutePattern = /app\.route\(\s*['`]([^'`]+)['`]\s*\)([\s\S]*?)(?=^[\t ]*(?:app\.|\/\*|\/\/|return app))/gm
...
test('les POST explicitement exemptés ne produisent pas de journal d’audit détaillé', t => {
