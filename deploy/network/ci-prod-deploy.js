...
const MIGRATION_COMMAND = ['node', 'scripts/network/prod-migration-service.js']
...
export function readProdConfiguration(environment) {
  requireCondition(UUID.test(PROD.privateNetworkId ?? '') && UUID.test(PROD.vpcId ?? ''), 'Cible réseau prod non configurée : aucune écriture autorisée.')
  requireCondition(environment.SCW_DEFAULT_PROJECT_ID === PROD.projectId, 'Projet Scaleway différent du projet PE attendu.')
  requireCondition(environment.SCW_REGION === 'fr-par', 'La région prod doit être fr-par.')
  requireCondition(environment.GITHUB_REF === 'refs/heads/prod', 'Seule la branche prod peut exécuter ce déploiement.')
  requireCondition(SHA.test(environment.GITHUB_SHA ?? ''), 'La release doit être un SHA Git complet.')
  requireCondition(environment.IMAGE_REF?.startsWith(`${PROD.registry}@sha256:`)
    && /^[\da-f]{64}$/.test(environment.IMAGE_REF.slice(`${PROD.registry}@sha256:`.length)), 'Une image immuable du registre API PE est obligatoire.')
  requireCondition(environment.SCW_SERVERLESS_CONTAINER_ID_PROD_API === PROD.apiId
    && environment.SCW_SERVERLESS_CONTAINER_ID_PROD_WORKER === PROD.workerId, 'Les identifiants API/worker ne correspondent pas à prod.')
  requireCondition(UUID.test(environment.SCW_PROD_MIGRATION_CONTAINER_ID ?? '')
    && UUID.test(environment.SCW_PROD_MIGRATION_NAMESPACE_ID ?? ''), 'Identifiants du service de migration prod manquants.')
  requireCondition(environment.SCW_SECRET_KEY?.length > 0, 'Clé CI Scaleway manquante.')
  requireCondition(environment.PROD_MIGRATION_INVOKE_SECRET?.length >= 32, 'Secret d’invocation prod absent ou trop court.')
  return {
    release: environment.GITHUB_SHA,
    image: environment.IMAGE_REF,
    key: environment.SCW_SECRET_KEY,
    migrationSecret: environment.PROD_MIGRATION_INVOKE_SECRET,
    migrationId: environment.SCW_PROD_MIGRATION_CONTAINER_ID,
    migrationNamespaceId: environment.SCW_PROD_MIGRATION_NAMESPACE_ID
  }
}

function assertNamespace(namespace, id, name) {
  requireCondition(namespace.id === id && namespace.name === name
    && namespace.project_id === PROD.projectId && namespace.region === 'fr-par', 'Namespace prod inattendu : déploiement interrompu.')
}
...
function assertContainer(container, {id, name, namespaceId, privacy}) {
  requireCondition(container.id === id && container.name === name && container.namespace_id === namespaceId
    && container.region === 'fr-par' && container.private_network_id === PROD.privateNetworkId
    && container.privacy === privacy, 'Identité, réseau ou confidentialité du conteneur prod inattendus.')
}
...
export async function deployProd(configuration, {fetch: fetchRequest = globalThis.fetch, wait = sleep, log = console.log} = {}) {
  const {release, image, key, migrationSecret, migrationId, migrationNamespaceId} = configuration
  const headers = {'X-Auth-Token': key}
  const definitions = [
    {id: PROD.apiId, name: 'prod-prelevement-deau-api', namespaceId: PROD.namespaceId, privacy: 'public'},
    {id: PROD.workerId, name: 'prod-partageons-leau-api-workers', namespaceId: PROD.namespaceId, privacy: 'public'},
    {id: migrationId, name: PROD.migrationName, namespaceId: migrationNamespaceId, privacy: 'private'}
  ]
  ...
  const [namespace, migrationNamespace, privateNetwork, ...before] = await Promise.all([
    getNamespace(PROD.namespaceId),
    getNamespace(migrationNamespaceId),
    jsonRequest(`https://api.scaleway.com/vpc/v2/regions/fr-par/private-networks/${PROD.privateNetworkId}`, {headers}),
    ...definitions.map(definition => getContainer(definition.id))
  ])
  assertNamespace(namespace, PROD.namespaceId, PROD.namespaceName)
  assertNamespace(migrationNamespace, migrationNamespaceId, PROD.migrationNamespaceName)
  requireCondition(privateNetwork.id === PROD.privateNetworkId && privateNetwork.project_id === PROD.projectId
    && privateNetwork.vpc_id === PROD.vpcId, 'Le Private Network ne correspond pas à prod.')
  for (const [index, container] of before.entries()) {
    assertContainer(container, definitions[index])
    requireCondition(container.status === 'ready', 'Un conteneur prod n’est pas prêt avant déploiement.')
  }
  ...
  requireCondition(migrationBefore.max_scale === 1 && migrationBefore.min_scale <= 1
    ...
    && migrationBefore.environment_variables?.APP_ENV === 'prod', 'Configuration du service de migration prod incorrecte.')
  ...
  async function waitReady(definition, expectedEnvironment) {
    for (let attempt = 0; attempt < 120; attempt++) {
      ...
      requireCondition(!['error', 'locked', 'deleting'].includes(current.status), 'Scaleway signale un échec de déploiement prod.')
    ...
    throw new Error('Le conteneur prod n’a pas confirmé sa disponibilité dans les dix minutes.')
  }
  ...
  await patchContainer(PROD.apiId, {
    image,
  ...
  await patchContainer(PROD.workerId, {image, command: ['node', 'worker.js']})
  ...
  requireCondition(apiHealth.ok === true, 'La sonde API prod a échoué après le déploiement.')
  const namespaceAfter = await getNamespace(PROD.namespaceId)
  ...
  const result = {environment: 'prod', release, image, migrationOperationId: verified.operation.id, containers: [api.id, worker.id, migrationId]}
