...
export function readDemoConfiguration(environment) {
  requireCondition(environment.SCW_DEFAULT_PROJECT_ID === DEMO.projectId, 'Projet Scaleway différent du projet PE attendu.')
  requireCondition(environment.SCW_REGION === 'fr-par', 'La région demo doit être fr-par.')
  requireCondition(environment.GITHUB_REF === 'refs/heads/demo', 'Seule la branche demo peut exécuter ce déploiement.')
  requireCondition(SHA.test(environment.GITHUB_SHA ?? ''), 'La release doit être un SHA Git complet.')
  requireCondition(environment.IMAGE_REF?.startsWith(`${DEMO.registry}@sha256:`)
    && /^[\da-f]{64}$/.test(environment.IMAGE_REF.slice(`${DEMO.registry}@sha256:`.length)), 'Une image immuable du registre API PE est obligatoire.')
  requireCondition(environment.SCW_SERVERLESS_CONTAINER_ID_DEMO_API === DEMO.apiId
    && environment.SCW_SERVERLESS_CONTAINER_ID_DEMO_WORKER === DEMO.workerId, 'Les identifiants API/worker ne correspondent pas à demo.')
  requireCondition(UUID.test(environment.SCW_DEMO_MIGRATION_CONTAINER_ID ?? '')
    && UUID.test(environment.SCW_DEMO_MIGRATION_NAMESPACE_ID ?? ''), 'Identifiants du service de migration demo manquants.')
  requireCondition(environment.SCW_SECRET_KEY?.length > 0, 'Clé CI Scaleway manquante.')
  requireCondition(environment.DEMO_MIGRATION_INVOKE_SECRET?.length >= 32, 'Secret d’invocation demo absent ou trop court.')
  return {
    release: environment.GITHUB_SHA,
    image: environment.IMAGE_REF,
    key: environment.SCW_SECRET_KEY,
    migrationSecret: environment.DEMO_MIGRATION_INVOKE_SECRET,
    migrationId: environment.SCW_DEMO_MIGRATION_CONTAINER_ID,
    migrationNamespaceId: environment.SCW_DEMO_MIGRATION_NAMESPACE_ID
  }
}

function assertNamespace(namespace, id, name) {
  requireCondition(namespace.id === id && namespace.name === name
    && namespace.project_id === DEMO.projectId && namespace.region === 'fr-par', 'Namespace demo inattendu : déploiement interrompu.')
}
...
function assertContainer(container, {id, name, namespaceId, privacy}) {
  requireCondition(container.id === id && container.name === name && container.namespace_id === namespaceId
    && container.region === 'fr-par' && container.private_network_id === DEMO.privateNetworkId
    && container.privacy === privacy, 'Identité, réseau ou confidentialité du conteneur demo inattendus.')
}
...
export async function deployDemo(configuration, {fetch: fetchRequest = globalThis.fetch, wait = sleep, log = console.log} = {}) {
  const {release, image, key, migrationSecret, migrationId, migrationNamespaceId} = configuration
  const headers = {'X-Auth-Token': key}
  const definitions = [
    {id: DEMO.apiId, name: 'demo-prelevement-deau-api', namespaceId: DEMO.namespaceId, privacy: 'public'},
    {id: DEMO.workerId, name: 'demo-prelevement-deau-api-worker', namespaceId: DEMO.namespaceId, privacy: 'public'},
    {id: migrationId, name: DEMO.migrationName, namespaceId: migrationNamespaceId, privacy: 'private'}
  ]
  ...
  const [namespace, migrationNamespace, privateNetwork, ...before] = await Promise.all([
    getNamespace(DEMO.namespaceId),
    getNamespace(migrationNamespaceId),
    jsonRequest(`https://api.scaleway.com/vpc/v2/regions/fr-par/private-networks/${DEMO.privateNetworkId}`, {headers}),
    ...definitions.map(definition => getContainer(definition.id))
  ])
  assertNamespace(namespace, DEMO.namespaceId, DEMO.namespaceName)
  assertNamespace(migrationNamespace, migrationNamespaceId, DEMO.migrationNamespaceName)
  requireCondition(privateNetwork.id === DEMO.privateNetworkId && privateNetwork.project_id === DEMO.projectId
    && privateNetwork.vpc_id === DEMO.vpcId, 'Le Private Network ne correspond pas à demo.')
  for (const [index, container] of before.entries()) {
    assertContainer(container, definitions[index])
    requireCondition(container.status === 'ready', 'Un conteneur demo n’est pas prêt avant déploiement.')
  }
  ...
  requireCondition(migrationBefore.max_scale === 1 && migrationBefore.min_scale <= 1
    ...
    && migrationBefore.environment_variables?.APP_ENV === 'demo', 'Configuration du service de migration demo incorrecte.')
  ...
  async function waitReady(definition, expectedEnvironment) {
    for (let attempt = 0; attempt < 120; attempt++) {
      ...
      requireCondition(!['error', 'locked', 'deleting'].includes(current.status), 'Scaleway signale un échec de déploiement demo.')
    ...
    throw new Error('Le conteneur demo n’a pas confirmé sa disponibilité dans les dix minutes.')
  }
  ...
  await patchContainer(DEMO.apiId, {
    image,
  ...
  await patchContainer(DEMO.workerId, {image, command: ['node', 'worker.js']})
  ...
  requireCondition(apiHealth.ok === true, 'La sonde API demo a échoué après le déploiement.')
  const namespaceAfter = await getNamespace(DEMO.namespaceId)
  ...
  const result = {environment: 'demo', release, image, migrationOperationId: verified.operation.id, containers: [api.id, worker.id, migrationId]}
