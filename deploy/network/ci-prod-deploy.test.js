...
import {PROD, deployProd, readProdConfiguration} from './ci-prod-deploy.js'

test('le nom de l’exécuteur respecte la limite Scaleway sans changer le namespace', t => {
  t.is(PROD.migrationName, 'prod-api-migrations')
  t.true(PROD.migrationName.length <= 34)
  t.is(PROD.migrationNamespaceName, 'prod-partageons-leau-migrations')
})
...
const IMAGE = `${PROD.registry}@sha256:${'3'.repeat(64)}`
const OPERATION_ID = 'cccccccc-cccc-cccc-cccc-cccccccccccc'
const AUTH = 'test-scaleway-key'
const INVOKE_SECRET = 's'.repeat(32)
const configuration = () => readProdConfiguration({
  GITHUB_REF: 'refs/heads/prod', GITHUB_SHA: RELEASE, IMAGE_REF: IMAGE,
  SCW_REGION: 'fr-par', SCW_DEFAULT_PROJECT_ID: PROD.projectId,
  SCW_SERVERLESS_CONTAINER_ID_PROD_API: PROD.apiId,
  SCW_SERVERLESS_CONTAINER_ID_PROD_WORKER: PROD.workerId,
  SCW_PROD_MIGRATION_CONTAINER_ID: MIGRATION_ID,
  SCW_PROD_MIGRATION_NAMESPACE_ID: MIGRATION_NAMESPACE_ID,
  SCW_SECRET_KEY: AUTH, PROD_MIGRATION_INVOKE_SECRET: INVOKE_SECRET
})

function fixture(options = {}) {
  const namespace = (id, name) => ({
    id, name, region: 'fr-par', project_id: PROD.projectId,
    environment_variables: {KEEP_NAMESPACE: 'unchanged'}, secret_environment_variables: {}
  })
  const namespaces = {
    [PROD.namespaceId]: namespace(PROD.namespaceId, PROD.namespaceName),
    [MIGRATION_NAMESPACE_ID]: namespace(MIGRATION_NAMESPACE_ID, PROD.migrationNamespaceName)
  }
  const container = (id, name, namespaceId) => ({
    id, name, namespace_id: namespaceId, region: 'fr-par',
    private_network_id: PROD.privateNetworkId, privacy: 'public', status: 'ready',
    image: `${PROD.registry}:prod-${OLD_RELEASE}`,
  ...
  const containers = {
    [PROD.apiId]: container(PROD.apiId, 'prod-prelevement-deau-api', PROD.namespaceId),
    [PROD.workerId]: container(PROD.workerId, 'prod-partageons-leau-api-workers', PROD.namespaceId),
    [MIGRATION_ID]: {
      ...container(MIGRATION_ID, PROD.migrationName, MIGRATION_NAMESPACE_ID),
      privacy: 'private', min_scale: 0, max_scale: 1, port: 8080, timeout: '1200s',
      command: ['node', 'scripts/network/prod-migration-service.js'], args: [],
      environment_variables: {APP_ENV: 'prod', MIGRATION_RELEASE_SHA: OLD_RELEASE, KEEP_CONTAINER: 'unchanged'},
  ...
  const fetch = async (input, request = {}) => {
    ...
    if (url.hostname === 'api.scaleway.com') {
      ...
      if (url.pathname.includes('/private-networks/')) {
        return json({id: PROD.privateNetworkId, project_id: PROD.projectId, vpc_id: PROD.vpcId})
      }
      ...
      if (method === 'PATCH') {
        ...
        if (id === MIGRATION_ID && options.businessDrift) {
          containers[PROD.apiId].environment_variables.KEEP_CONTAINER = 'modified concurrently'
        }
        ...
        if (id === PROD.workerId && options.settingsDrift) {
          containers[PROD.workerId].max_scale = 999
        }
      }
...
test('la configuration refuse branche, cible ou image non prod', t => {
  t.is(configuration().image, IMAGE)
  const env = {
    GITHUB_REF: 'refs/heads/testing', GITHUB_SHA: RELEASE, IMAGE_REF: IMAGE,
    SCW_REGION: 'fr-par', SCW_DEFAULT_PROJECT_ID: PROD.projectId
  }
  t.throws(() => readProdConfiguration(env), {message: /branche prod/})
  t.throws(() => readProdConfiguration({...env, SCW_DEFAULT_PROJECT_ID: 'another-project'}), {message: /Projet Scaleway/})
  t.throws(() => readProdConfiguration({...env, GITHUB_REF: 'refs/heads/prod', IMAGE_REF: `${PROD.registry}:prod-latest`}), {message: /image immuable/})
})

test('migration puis API et worker utilisent le même digest sans réécrire les secrets', async t => {
  const fake = fixture()
  const result = await deployProd(configuration(), fake)
  const patches = fake.calls.filter(call => call.method === 'PATCH')
  t.deepEqual(patches.map(call => call.url.split('/').at(-1)), [MIGRATION_ID, PROD.apiId, PROD.workerId])
  ...
  t.deepEqual(patches[0].body.environment_variables, {
    APP_ENV: 'prod', KEEP_CONTAINER: 'unchanged', MIGRATION_RELEASE_SHA: RELEASE
  })
...
for (const status of [502, 503, 504]) {
  test(`un GET ${status} après redéploiement migration est retenté sans répéter les écritures`, async t => {
    ...
    const waits = []
    await deployProd(configuration(), {...fake, async wait(duration) {
      waits.push(duration)
    }})

    t.deepEqual(waits, [5000])
    t.is(fake.calls.filter(call => call.method === 'POST').length, 1)
    t.deepEqual(fake.calls.filter(call => call.method === 'PATCH').map(call => call.url.split('/').at(-1)), [MIGRATION_ID, PROD.apiId, PROD.workerId])
  })
}
...
test('une lecture 503 persistante est bornée et bloque avant tout déploiement métier', async t => {
  ...
  const waits = []
  const error = await t.throwsAsync(deployProd(configuration(), {...fake, async wait(duration) {
    waits.push(duration)
...
test('un PATCH 503 n’est jamais retenté', async t => {
  ...
  const error = await t.throwsAsync(deployProd(configuration(), fake), {message: /PATCH \/containers\/v1\/regions\/fr-par\/containers\/.*HTTP 503/})
...
test('un POST 503 est suivi seulement d’une lecture de statut, jamais d’un second POST', async t => {
  ...
  await t.throwsAsync(deployProd(configuration(), fake), {message: /opération non identifiable/})
...
test('un namespace hors projet bloque avant toute écriture', async t => {
  ...
  await t.throwsAsync(deployProd(configuration(), fake), {message: /Namespace prod inattendu/})
  t.true(fake.calls.every(call => call.method === 'GET'))
})

test('un mauvais réseau métier bloque avant toute écriture', async t => {
  const fake = fixture()
  fake.containers[PROD.workerId].private_network_id = null
  await t.throwsAsync(deployProd(configuration(), fake), {message: /réseau/})
  t.true(fake.calls.every(call => call.method === 'GET'))
})
...
test('un exécuteur public est refusé avant toute écriture', async t => {
  ...
  await t.throwsAsync(deployProd(configuration(), fake), {message: /confidentialité/})
  t.true(fake.calls.every(call => call.method === 'GET'))
})

test('les credentials GitHub réels doivent pouvoir invoquer le service privé', async t => {
  const fake = fixture({ciDenied: true})
  await t.throwsAsync(deployProd(configuration(), fake), {message: /HTTP 403/})
  t.true(fake.calls.every(call => call.method === 'GET'))
})
...
test('une destination arbitraire ne reçoit jamais les credentials', async t => {
  ...
  await t.throwsAsync(deployProd(configuration(), fake), {message: /Endpoint Scaleway/})
  t.true(fake.calls.every(call => new URL(call.url).hostname === 'api.scaleway.com'))
})

test('les domaines historiques functions restent compatibles pour les applications prod', async t => {
  const fake = fixture()
  fake.containers[PROD.apiId].public_endpoint = `https://${PROD.apiId}.functions.fnc.fr-par.scw.cloud`
  const result = await deployProd(configuration(), fake)
  t.is(result.image, IMAGE)
  t.true(fake.calls.some(call => call.url === `https://${PROD.apiId}.functions.fnc.fr-par.scw.cloud/healthz`))
})

test('un registre Prisma avec migration inachevée bloque avant toute écriture', async t => {
  const fake = fixture({unfinished: true})
  await t.throwsAsync(deployProd(configuration(), fake), {message: /migrations PostgreSQL/})
  t.true(fake.calls.every(call => call.method === 'GET'))
})
...
test('un échec de migration bloque les deux applications et ne relance pas Prisma', async t => {
  ...
  await t.throwsAsync(deployProd(configuration(), fake), {message: /réussite/})
  t.is(fake.calls.filter(call => call.method === 'POST').length, 1)
  t.deepEqual(fake.calls.filter(call => call.method === 'PATCH').map(call => call.url.split('/').at(-1)), [MIGRATION_ID])
})
...
test('réponse perdue : une nouvelle opération confirmée réussie permet de continuer sans second POST', async t => {
  ...
  await deployProd(configuration(), fake)
  t.is(fake.calls.filter(call => call.method === 'POST').length, 1)
...
for (const lostResponse of ['before', 'running']) {
  test(`réponse perdue ${lostResponse} : état inconnu ou actif bloque tout déploiement métier`, async t => {
    ...
    await t.throwsAsync(deployProd(configuration(), fake))
    t.is(fake.calls.filter(call => call.method === 'POST').length, 1)
    t.is(fake.calls.filter(call => call.method === 'PATCH').length, 1)
    t.true(fake.calls.at(-1).url.endsWith('/status'))
  })
...
test('une dérive de noms de secrets arrête le déploiement sans les réinjecter', async t => {
  ...
  await t.throwsAsync(deployProd(configuration(), fake), {message: /liste des secrets/})
  t.false(fake.calls.some(call => call.method === 'POST'))
})
...
test('une modification concurrente des variables métier bloque avant leur déploiement', async t => {
  ...
  await t.throwsAsync(deployProd(configuration(), fake), {message: /Configuration métier modifiée/})
  t.is(fake.calls.filter(call => call.method === 'PATCH').length, 1)
})

test('le workflow prod bloque sur le service privé et transmet le digest du build', async t => {
  const workflow = await readFile(new URL('../../.github/workflows/deploy-prod.yml', import.meta.url), 'utf8')
  t.true(workflow.includes('branches: ["prod"]'))
  ...
  t.true(workflow.indexOf('Scan exact candidate image before any deployment') < workflow.indexOf('run: node deploy/network/ci-prod-deploy.js'))
  t.regex(workflow, /run: bash \.github\/scripts\/smoke-image\.sh "\$IMAGE_REF"\n/)
  t.true(workflow.indexOf('Smoke test native modules from the scanned image') < workflow.indexOf('run: node deploy/network/ci-prod-deploy.js'))
  t.regex(workflow, /id: build/)
  t.regex(workflow, /@\$\{\{ steps\.build\.outputs\.digest \}\}/)
  t.regex(workflow, /run: node deploy\/network\/ci-prod-deploy\.js/)
  t.notRegex(workflow, /jobs definition start|SCW_JOB_DEFINITION_ID|continue-on-error/)
  t.true(workflow.indexOf('Publish environment alias only after verified deployment') > workflow.indexOf('run: node deploy/network/ci-prod-deploy.js'))
})

test('une mauvaise release après mise à jour bloque avant toute migration', async t => {
  const fake = fixture({wrongRelease: true})
  await t.throwsAsync(deployProd(configuration(), fake), {message: /release attendue/})
  t.false(fake.calls.some(call => call.method === 'POST'))
})

test('une variable de migration perdue bloque avant toute exécution', async t => {
  const fake = fixture({environmentLost: true})
  await t.throwsAsync(deployProd(configuration(), fake), {message: /variables ordinaires/})
  t.false(fake.calls.some(call => call.method === 'POST'))
})

test('les réglages métier sont conservés et toute dérive est détectée', async t => {
  const fake = fixture({settingsDrift: true})
  await t.throwsAsync(deployProd(configuration(), fake), {message: /configuration du conteneur/})
})

test('la santé du worker est requise avant publication du succès', async t => {
  const fake = fixture({workerUnhealthy: true})
  await t.throwsAsync(deployProd(configuration(), fake), {message: /sonde worker/})
})
