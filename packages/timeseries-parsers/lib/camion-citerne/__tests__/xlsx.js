...
import {extractCamionCiterne} from '../index.js'
...
test('extractCamionCiterne - valid file', async t => {
  const filePath = path.join(testFilesPath, 'valid.xlsx')
  const fileContent = await fs.readFile(filePath)
  const {errors, data} = await extractCamionCiterne(fileContent)
  t.deepEqual(errors, [])
  t.deepEqual(data, {
    series: [
      {
...
test('extractCamionCiterne - multi points valid file', async t => {
...
test('extractCamionCiterne - incorrect file format', async t => {
...
test('extractCamionCiterne - corrupted file', async t => {
...
test('extractCamionCiterne - empty file', async t => {
...
test('extractCamionCiterne - incorrect headers', async t => {
...
test('extractCamionCiterne - no data', async t => {
...
test('extractCamionCiterne - duplicate dates', async t => {
...
test('extractCamionCiterne - only necessary points', async t => {
  const filePath = path.join(testFilesPath, 'only-necessary-points.xlsx')
  const fileContent = await fs.readFile(filePath)
  const {errors} = await extractCamionCiterne(fileContent)
  t.is(errors.length, 0)
})

test('extractCamionCiterne - duplicates points', async t => {
