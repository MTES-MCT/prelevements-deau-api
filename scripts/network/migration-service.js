...
export function getMigrationConfiguration(environment) {
  if (environment.APP_ENV !== 'demo') {
    throw new Error('Le service de migration est réservé à APP_ENV=demo.')
  }

  const databaseTarget = validateDemoAdminDatabaseUrl(environment.DATABASE_URL)
  if (databaseTarget.hostname !== '172.16.12.2' || databaseTarget.port !== '5432') {
...
export function createMigrationService({
  ...
  async function migrate() {
    ...
    try {
      ...
      await readStatus(configuration.databaseUrl)
      if (stopping) {
  ...
  const server = createServer(async (request, response) => {
    if (request.method === 'GET' && request.url === '/healthz') {
      json(response, stopping ? 503 : 200, {ok: !stopping, service: 'demo-migrations', release: configuration.release})
      return
    }
