...
export function getMigrationConfiguration(environment, {appEnv, validateDatabaseUrl}) {
  if (environment.APP_ENV !== appEnv) {
    throw new Error(`Le service de migration est réservé à APP_ENV=${appEnv}.`)
  }
...
export function createMigrationService({
  ...
  const configuration = getMigrationConfiguration(environment, target)
  let stopping = false
  let activeOperation
  let operation
  let cancelChild
  ...
  async function migrate() {
    ...
    try {
      ...
      if (target.requireCleanLedger && (!Array.isArray(before.unfinished) || before.unfinished.length > 0)) {
  ...
  const server = createServer(async (request, response) => {
    if (request.method === 'GET' && request.url === '/healthz') {
      json(response, stopping ? 503 : 200, {ok: !stopping, service: `${target.appEnv}-migrations`, release: configuration.release})
      return
    }
    ...
    if (operation?.state === 'unknown' || (target.blockFailedReplay && operation?.state === 'failed')) {
...
export function startMigrationService(createService) {
  try {
