...
export function getMigrationConfiguration(environment) {
  if (environment.APP_ENV !== 'testing') {
    throw new Error('Le service de migration est réservé à APP_ENV=testing.')
  }

  validateTestingAdminDatabaseUrl(environment.DATABASE_URL)
...
export function createMigrationService({
  ...
  const server = createServer(async (request, response) => {
    if (request.method === 'GET' && request.url === '/healthz') {
      json(response, stopping ? 503 : 200, {ok: !stopping, service: 'testing-migrations', release: configuration.release})
      return
    }
