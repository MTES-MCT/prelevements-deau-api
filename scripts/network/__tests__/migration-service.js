...
import {createMigrationService, getMigrationConfiguration} from '../migration-service.js'
...
const DATABASE_URL = 'postgresql://demo_admin:database-secret@172.16.12.2:5432/prelevements_demo'
...
const DATABASE_STATUS = {
  identity: {databaseName: 'prelevements_demo', databaseUser: 'demo_admin', tls: true},
  expectedCount: 1,
  pending: [],
  unfinished: [],
  migrations: []
...
test('Prisma : traduit verify-full en TLS requis, validation stricte et CA explicite', t => {
  ...
  t.is(parsed.username, 'demo_admin')
...
test('refuse un environnement, une cible, une identité, un TLS ou un secret non conformes', t => {
  const changes = [
    ...
    {DATABASE_URL: DATABASE_URL.replace('prelevements_demo', 'prelevements_prod')},
    {DATABASE_URL: DATABASE_URL.replace('demo_admin', 'postgres')},
    {DATABASE_URL: DATABASE_URL.replace('172.16.12.2:5432', '51.15.219.67:24881')},
    {DATABASE_URL: DATABASE_URL.replace('172.16.12.2:5432', '163.172.7.73:17063')},
    {DATABASE_URL: DATABASE_URL.replace('172.16.12.2:5432', 'rw-ea5a07db-05df-4869-9e57-fa5f5c6c81cc.rdb.fr-par.scw.cloud:17063')},
...
test('healthz ne connecte pas PostgreSQL et ne lance aucune migration', async t => {
  ...
  t.deepEqual(response.body, {ok: true, service: 'demo-migrations', release: RELEASE})
})
