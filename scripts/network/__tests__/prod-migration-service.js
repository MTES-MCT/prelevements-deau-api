...
import {createMigrationService, getMigrationConfiguration} from '../prod-migration-service.js'
...
const HOST = PROD_DATABASE_ENDPOINT.host
const RELEASE = 'a'.repeat(40)
const SECRET = 'secret-of-this-migration-service-'.repeat(2)
const DATABASE_URL = `postgresql://prod-partageons-leau-api:database-secret@${HOST}:5432/prod-partageons-leau-api`
...
const DATABASE_STATUS = {
  identity: {databaseName: 'prod-partageons-leau-api', databaseUser: 'prod-partageons-leau-api', tls: true},
  expectedCount: 1,
  pending: [],
  unfinished: [],
  migrations: []
...
test('Prisma : traduit verify-full en TLS requis, validation stricte et CA explicite', t => {
  ...
  t.is(parsed.username, 'prod-partageons-leau-api')
...
test('refuse un environnement, une cible, une identité, un TLS ou un secret non conformes', t => {
  const changes = [
    ...
    {DATABASE_URL: DATABASE_URL.replace('/prod-partageons-leau-api', '/prelevements_prod')},
    {DATABASE_URL: DATABASE_URL.replace('prod-partageons-leau-api', 'postgres')},
    ...
    {DATABASE_URL: DATABASE_URL.replace(`${HOST}:5432`, '172.16.16.3:5432')},
...
test('healthz ne connecte pas PostgreSQL et ne lance aucune migration', async t => {
  ...
  t.deepEqual(response.body, {ok: true, service: 'prod-migrations', release: RELEASE})
})
...
test('un registre inachevé ou invérifiable bloque même un POST direct avant Prisma', async t => {
  for (const unfinished of [['20260906_failed'], undefined]) {
    const service = createService(t, {
      readStatus: async () => ({...DATABASE_STATUS, unfinished}),
      spawn() {
        t.fail('Aucun processus ne doit démarrer avec un registre inachevé')
      }
    })
    const response = await postMigration(service)
    t.is(response.status, 503)
    t.is(response.body.state, 'failed')
    t.is(response.body.operation.errorCode, 'MIGRATION_LEDGER_REQUIRES_REVIEW')
    const replay = await postMigration(service)
    t.is(replay.status, 409)
    t.is(replay.body.errorCode, 'MIGRATION_REQUIRES_REVIEW')
  }
