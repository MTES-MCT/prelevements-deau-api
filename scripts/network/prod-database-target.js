const EXPECTED_DATABASE_NAME = 'prod-partageons-leau-api'
const EXPECTED_DATABASE_USER = 'prod-partageons-leau-api'
const EXPECTED_SSL_MODE = 'verify-full'
const EXPECTED_SSL_ROOT_CERT = '/usr/local/share/ca-certificates/scw-postgres-ca.crt'
export const PROD_DATABASE_ENDPOINT = Object.freeze({host: '172.16.20.3', port: '5432'})
...
export function validateProdAdminDatabaseUrl(databaseUrl) {
  if (!EXPECTED_DATABASE_NAME || !EXPECTED_DATABASE_USER || !PROD_DATABASE_ENDPOINT.host) {
    throw new Error('Cible PostgreSQL prod non configurée : aucune migration autorisée.')
  }
  ...
  const endpoint = PROD_DATABASE_ENDPOINT.host === parsedDatabaseUrl.hostname.toLowerCase()
    ? PROD_DATABASE_ENDPOINT
    : undefined
  if (!endpoint) {
    throw new Error('Refus : DATABASE_URL ne cible pas l’instance PostgreSQL prod attendue.')
  }

  if (parsedDatabaseUrl.port !== endpoint.port) {
    throw new Error(`Refus : le port PostgreSQL prod doit être ${endpoint.port}.`)
  }
...
export async function assertConnectedProdAdminDatabase(database) {
  ...
  if (failedCheck) {
    throw new Error(
      `Refus : l’identité PostgreSQL connectée ne correspond pas à prod (${failedCheck[1]}).`
    )
  }
