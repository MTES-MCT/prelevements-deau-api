...
export async function readWorkbook(filename, definitions) {
  ...
  const result = {}
  for (const [name, {start, headers}] of Object.entries(definitions)) {
    ...
    for (const [column, expected] of Object.entries(headers)) {
      if (clean(cellValue(sheet.getRow(1).getCell(Number(column)))) !== expected) throw new Error(`Structure inattendue : ${name}, colonne ${column}`)
    }
    ...
    for (let rowNumber = start; rowNumber <= sheet.rowCount; rowNumber++) {
      ...
      if (values.some(value => clean(value))) result[name].push({row: rowNumber, values})
    }
  }
...
export const EPIDROPT_SHEETS = {
  'Points prélèvement': {start: 3, headers: {2: 'Nom du point *', 25: 'Numéro de série du compteur'}},
...
export function buildManifest({epidropt, rives, overrides = {}, inputs = {}}) {
  const issues = []
  ...
  for (const [name, rows] of group(epidropt['Points prélèvement'].filter(r => clean(r.values[1])), r => clean(r.values[1]))) {
    ...
    const point = points.get(key) ?? {
      key, id: override.id ?? stableId(key), names: [], references: [], coordinates: geometry,
      data: {name, flowType: 'PRELEVEMENT', waterBodyType, pointKind: 'PHYSIQUE', nature: {nappe: 'NAPPE', "cours d'eau": 'COURS_EAU', "plan d'eau": 'PLAN_EAU'}[normalized(v[8])] ?? null,
        withdrawalType: normalized(v[9]) === 'souterrain' ? 'SOUTERRAIN' : 'CONTINENTAL',
        waterAgencyInternalIdentifier: nullable(v[11]), codeBSS: nullable(v[19]), codeBNPE: nullable(v[20]), codeAIOT: nullable(v[21]), codeEUMasseDEau: nullable(v[22]), codePTP: nullable(v[23]), codeOPR: nullable(v[25]), codeBDLISA: nullable(v[26]), codeBDCarthage: nullable(v[27]), codeBDTopage: nullable(v[28]), codeSISEAUX: nullable(v[31]), managementUnit: nullable(v[17]), managementSubUnit: nullable(v[18]), locationDescription: place?.label || null},
  ...
  for (const {row, values: v} of epidropt.Exploitations) {
    if (!clean(v[1])) continue
    ...
    const ownerKey = override.declarantKey ?? (owners.size === 1 ? [...owners][0] : null)
    ...
    const key = `exploitation:${point.key}:${owner.key}`
    ...
    const exploitation = previous ?? {key, id: override.id ?? stableId(key), sourceId: `dropt-epidropt:exploitation:${digest(key).slice(0, 32)}`, pointId: point.id, declarantId: owner.id, usageCode, aliases: [], source: []}
  ...
  for (const {row, values: v} of epidropt['Points prélèvement']) {
    const point = pointsByName.get(clean(v[1]))
  ...
  const manifest = {formatVersion: FORMAT_VERSION, scope: SCOPE, inputs, overridesHash: digest(overrides),
    ...
    contracts: [...contracts.values()], issues}
